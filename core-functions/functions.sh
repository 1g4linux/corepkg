...
# be enabled in the shell, the behaviour of core-functions as a whole shall
# be unspecified.
...
case ${KSH_VERSION} in 'Version AJM 93'*)
	...
	warn "core-functions does not currently support ksh93"
	return 1
esac
...
if [ "${YASH_VERSION}" ] && [ -o posixlycorrect ]; then
	...
	warn "core-functions does not support yash in posixlycorrect mode"
	return 1
fi

case $- in *[eu]*)
	# https://lists.gnu.org/archive/html/help-bash/2020-04/msg00049.html
	warn "core-functions supports neither the errexit option nor the nounset option; unexpected behaviour may ensue"
esac
...
#
# Takes the first parameter as the path of a core-functions module then
...
# It was introduced by core-functions-1.7 with an initial value of 1.
# shellcheck disable=2034
GENFUN_API_LEVEL=2

# If genfun_basedir is unset, set genfun_prefix to the value of EPREFIX, as it
# was at the time of installing core-functions, before setting genfun_basedir
...
if [ ! "${genfun_basedir+set}" ]; then
	genfun_prefix=
	genfun_basedir=${genfun_prefix}/lib/corepkg/functions
fi
...
if [ ! "${GENFUN_MODULES+set}" ]; then
	...
	# parallel implementations in core-functions. Declare ours only if the
...
for _ in "${genfun_basedir}/functions"/*.sh; do
	if ! test -e "$_"; then
		warn "no core-functions modules were found (genfun_basedir might be set incorrectly)"
		false
