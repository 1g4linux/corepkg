...
# Prevent aliases from causing corepkg to act inappropriately.
# Make sure it's before everything so we don't mess aliases that follow.
unalias -a

# Make sure this isn't exported to scripts we execute.
unset BASH_COMPAT
declare -F ___in_corepkg_iuse >/dev/null && export -n -f ___in_corepkg_iuse
...
__check_bash_version() {
	...
	# Corepkg uses features that are only available in >=bash-4.4.
	dependent="Corepkg"
	maj=4
	min=4
...
if [[ ${EBUILD_PHASE} != depend ]] ; then
	...
	funcs+=" best_version has_version corepkgq"
	for x in ${funcs} ; do
...
# This is just a temporary workaround for corepkg-9999 users since
# earlier corepkg versions do not detect a version change in this case
...
addread /
addread "${PORTAGE_TMPDIR}/corepkg"
addwrite "${PORTAGE_TMPDIR}/corepkg"
...
inherit() {
	...
	if [[ ${ECLASS_DEPTH} -gt 1 ]]; then
		...
		if [[ -n ${ECLASS} && -n ${!__export_funcs_var} ]] ; then
			...
			eqawarn "EXPORT_FUNCTIONS after inherit(s). Corepkg behavior may change in future."
		fi
	fi
...
__source_all_bashrcs() {
	...
	# The user's bashrc is the ONLY non-Corepkg bit of code
...
if [[ ${EBUILD_PHASE} = depend ]] ; then
	...
	declare -r "${corepkg_readonly_metadata[@]}" "${corepkg_readonly_vars[@]}"
