...
install_qa_check() {
	...
	paths+=(
		# Corepkg built-in checks
		"${PORTAGE_OVERRIDE_EPREFIX}"/usr/lib/corepkg/install-qa-check.d
	...
	if [[ -d ${ED%/}/usr/share/info ]]; then
		# Corepkg regenerates this on the installed system.
	...
	# decrease the space used by corepkg build directories until the
...
postinst_qa_check() {
	...
	paths+=(
		# Corepkg built-in checks
		"${PORTAGE_OVERRIDE_EPREFIX}"/usr/lib/corepkg/${PORTAGE_QA_PHASE}-qa-check.d
...
preinst_suid_scan() {
	...
	if contains_word suidctl "${FEATURES}"; then
		local i sfconf x
		sfconf=${PORTAGE_CONFIGROOT}etc/corepkg/suidctl.conf
...
__generate_packdebug() {
	...
	# have sources.
	mkdir -p "${PORTAGE_TMPDIR}"/corepkg/${CATEGORY}/${PF}/image/${EPREFIX}/usr/lib/debug || die
...
install_hooks() {
	local hooks_dir="${PORTAGE_CONFIGROOT}etc/corepkg/hooks/install"
	local fp
	local ret=0
