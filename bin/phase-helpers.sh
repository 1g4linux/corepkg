...
dedup_symlink() {
	local target_dir=${1:-${D}}
	local helper="${PORTAGE_BIN_PATH}/dedup_symlink.sh"

	if [[ -x ${helper} ]]; then
		"${helper}" "${target_dir}" || ewarn "dedup_symlink.sh failed for ${target_dir}"
	fi
}
...
use() {
	...
	elif declare -F ___in_corepkg_iuse >/dev/null &&
		[[ -n ${EBUILD_PHASE} && -n ${PORTAGE_INTERNAL_CALLER} ]] ; then
		if ! ___in_corepkg_iuse "${u}"; then
...
econf() {
	...
	if [[ -x "${ECONF_SOURCE}/configure" ]]; then
		if [[ -n ${CONFIG_SHELL} && \
			...
			cp -p "${ECONF_SOURCE}/configure" "${ECONF_SOURCE}/configure._corepkg_tmp_.${pid}" || die
			sed -i \
				-e "1s:^#![[:space:]]*/bin/sh:#!${CONFIG_SHELL}:" \
				"${ECONF_SOURCE}/configure._corepkg_tmp_.${pid}" \
				|| die "Substition of shebang in '${ECONF_SOURCE}/configure' failed"

			# Preserve timestamp, see bug #440304
			touch -r "${ECONF_SOURCE}/configure" "${ECONF_SOURCE}/configure._corepkg_tmp_.${pid}" || die
			mv -f "${ECONF_SOURCE}/configure._corepkg_tmp_.${pid}" "${ECONF_SOURCE}/configure" || die
		fi
...
___best_version_and_has_version_common() {
	...
	case ${root_arg} in
		...
		--host-root)
			...
			if ___eapi_has_prefix_variables; then
				# Since corepkgq requires the root argument be consistent
	...
	if [[ -n ${PORTAGE_IPC_DAEMON} ]] ; then
		...
		cmd+=("${PORTAGE_BIN_PATH}"/corepkgq-wrapper "${FUNCNAME[1]}" "${root}" "${atom}")
	fi
	...
	case "${retval}" in
		...
		*)
			if [[ -n ${PORTAGE_IPC_DAEMON} ]]; then
				...
				die "${FUNCNAME[1]}: unexpected corepkgq exit code: ${retval}"
			fi
			;;
	esac
}
...
corepkgq() {
	die "corepkgq is not allowed in ebuild scope"
}
...
if ___eapi_has_einstalldocs; then
	einstalldocs() (
		...
		if [[ ! ${DOCS@A} ]]; then
			for f in README* ChangeLog AUTHORS TODO CHANGES \
...
if ___eapi_has_eapply_user; then
	...
	eapply_user() {
		...
		tagfile=${T}/.corepkg_user_patches_applied
		[[ -f ${tagfile} ]] && return
		>> "${tagfile}"

		basedir=${PORTAGE_CONFIGROOT%/}/etc/corepkg/patches
