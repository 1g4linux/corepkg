...
# to communicate with corepkg's main python process.
...
try:
    ...
    class EbuildIpc:
        ...
        def communicate(self, args):
            ...
            lock_obj = corepkg.locks.lockfile(self.ipc_lock_file, unlinkfile=True)
        ...
        def _no_daemon_msg(self):
            corepkg.util.writemsg_level(
                corepkg.localization._("ebuild-ipc: daemon process not detected\n"),
                level=logging.ERROR,
                noiselevel=-1,
            )
        ...
        def _receive_reply(self, input_fd):
                ...
                if not eof:
                    if self._daemon_is_alive():
                        self._timeout_retry_msg(
                            start_time, corepkg.localization._("during read")
                        )
                    else:
                        pipe_reader.cancel()
                        self._no_daemon_msg()
            ...
            if not buf:
                corepkg.util.writemsg_level(
                    f"ebuild-ipc: {corepkg.localization._('read failed')}\n",
                    level=logging.ERROR,
                    noiselevel=-1,
                )
                ...
                try:
                    ...
                    if out:
                        corepkg.util.writemsg_stdout(out, noiselevel=-1)

                    if err:
                        corepkg.util.writemsg(err, noiselevel=-1)
        ...
        def _communicate(self, args):
            ...
            msg = corepkg.localization._("during write")
