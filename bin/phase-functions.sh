...
# <corepkg-2.1.4 since they assume that a newly installed version
# of ebuild.sh will work for pkg_postinst, pkg_prerm, and pkg_postrm
# when corepkg is upgrading itself.

corepkg_readonly_metadata=(
...
corepkg_readonly_vars=(
	...
	corepkg_mutable_filtered_vars corepkg_saved_readonly_vars
	corepkg_readonly_metadata corepkg_readonly_vars
)

corepkg_saved_readonly_vars=(
	A CATEGORY PVR PF PN PR PV P
)

# Variables that corepkg sets but doesn't mark readonly.
...
corepkg_mutable_filtered_vars=( AA HOSTNAME )
...
#   * some specific variables that affect corepkg or sandbox behavior
...
__preprocess_ebuild_env() {
	local _corepkg_filter_opts="--filter-features --filter-locale --filter-path --filter-sandbox"
	...
	__filter_readonly_variables ${_corepkg_filter_opts} < "${T}"/environment \
		>> "${T}/environment.filtered" || return $?

	unset _corepkg_filter_opts
...
__dyn_prepare() {
	...
	if ___eapi_has_eapply_user && [[ ! -f ${T}/.corepkg_user_patches_applied ]]; then
...
__ebuild_main() {
	...
	*)
	...
	if [[ $1 != @(clean|help|info|nofetch) ]]; then
		...
		chgrp "${PORTAGE_GRPNAME:-corepkg}" "${T}/environment"
