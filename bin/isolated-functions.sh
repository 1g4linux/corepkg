...
if [[ -v PORTAGE_EBUILD_EXTRA_SOURCE ]]; then
	...
	# process and its child processes. There, for example corepkg
...
__dump_trace() {
	...
	# The __qa_call() function and anything before it are corepkg internals
...
debug-print() {
	...
	if [[ -w ${T} ]] ; then
		...
		# Let the corepkg user own/write to this file
		chgrp "${PORTAGE_GRPNAME:-corepkg}" "${T}/eclass-debug.log"
