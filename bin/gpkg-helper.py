...
def command_compose(args):
    ...
    try:
        gpkg_file = corepkg.gpkg.gpkg(corepkg.settings, basename, binpkg_path)
        metadata = gpkg_file._generate_metadata_from_dir(metadata_dir)
        gpkg_file.compress(image_dir, metadata)
    except corepkg.exception.CompressorOperationFailed:
...
def main(argv):
    if argv and isinstance(argv[0], bytes):
        for i, x in enumerate(argv):
            argv[i] = corepkg._unicode_decode(x, errors="strict")
