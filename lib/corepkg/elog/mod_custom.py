...
# via corepkg.process.run_coroutine_exitfuncs().
_proc_refs = None


def _get_procs() -> list[tuple[corepkg.process.MultiprocessingProcess, asyncio.Future]]:
    """
    Return list of (proc, asyncio.ensure_future(proc.wait())) which is not
    inherited from the parent after fork.
    """
    global _proc_refs
    if _proc_refs is None or _proc_refs.pid != corepkg.getpid():
        _proc_refs = types.SimpleNamespace(pid=corepkg.getpid(), procs=[])
        corepkg.process.atexit_register(_async_finalize)
    return _proc_refs.procs


def process(mysettings, key, logentries, fulltext):
    elogfilename = corepkg.elog.mod_save.process(mysettings, key, logentries, fulltext)
    ...
    if not mysettings.get("PORTAGE_ELOG_COMMAND"):
        ...
        proc = corepkg.process.spawn_bash(mylogcmd, returnproc=True)
        procs = _get_procs()
...
def finalize():
    """
    NOTE: This raises CorepkgException if there are any processes
    still running, so it's better to use _async_finalize instead
    (invoked via corepkg.process.run_coroutine_exitfuncs() in
