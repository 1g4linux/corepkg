...
import corepkg
from corepkg import eapi_is_valid_for_runtime
from corepkg import os
from corepkg.cache.cache_errors import CacheError
from corepkg.eclass_cache import hashed_path
...
def action_metadata(settings, portdb, myopts, porttrees=None):
    ...
    onProgress = None
    if not quiet:
        progressBar = corepkg.output.TermProgressBar()
        ...
        def sigwinch_handler(signum, frame):
            lines, progressBar.term_columns = corepkg.output.get_term_size()
    ...
    for cp in cp_all:
        for tree_data in porttrees_data:
            ...
            for cpv in portdb.cp_list(cp, mytree=tree_data.path):
                ...
                if not eapi:
                    eapi = "8"
                eapi_supported = eapi_is_valid_for_runtime(eapi)
