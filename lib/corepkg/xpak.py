...
def xpak(rootdir, outfile=None):
    ...
    if corepkg.utf8_mode and not isinstance(rootdir, bytes):
    ...
    def recompose_mem(self, xpdata, break_hardlinks=True):
        ...
        if break_hardlinks and self.filestat and self.filestat.st_nlink > 1:
            with tempfile.NamedTemporaryFile(
                ...
                prefix=f"{os.path.basename(self.file)}.{corepkg.getpid()}",
            ) as safe_temp:
                tmp_fname = safe_temp.name
            copyfile(self.file, tmp_fname)
            try:
                corepkg.util.apply_stat_permissions(self.file, self.filestat)
            except corepkg.exception.OperationNotPermitted:
                pass
