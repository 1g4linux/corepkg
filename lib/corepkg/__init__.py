...
from corepkg import installation
...
try:
    ...
    # wrapped versions are imported by corepkg internals.
    import os
    ...
    sys.stderr.write(
        "!!! itself and thus corepkg is not able to continue processing.\n\n"
    )
...
try:
    import corepkg.proxy.lazyimport
    import corepkg.proxy as proxy

    proxy.lazyimport.lazyimport(
        globals(),
        "corepkg.cache.cache_errors:CacheError",
        "corepkg.checksum",
        "corepkg.checksum:perform_checksum,perform_md5,prelink_capable",
        "corepkg.cvstree",
        "corepkg.data",
        "corepkg.data:lchown,ostype,corepkg_gid,corepkg_uid,secpass,"
        + "uid,userland,userpriv_groups,wheelgid",
        "corepkg.dbapi",
        "corepkg.dbapi.bintree:bindbapi,binarytree",
        "corepkg.dbapi.cpv_expand:cpv_expand",
        "corepkg.dbapi.dep_expand:dep_expand",
        "corepkg.dbapi.porttree:close_portdbapi_caches,FetchlistDict,"
        + "corepkgtree,portdbapi",
        "corepkg.dbapi.vartree:dblink,merge,unmerge,vardbapi,vartree",
        "corepkg.dbapi.virtual:fakedbapi",
        "corepkg.debug",
        "corepkg.dep",
        "corepkg.dep:best_match_to_list,dep_getcpv,dep_getkey,"
        + "flatten,get_operator,isjustname,isspecific,isvalidatom,"
        + "match_from_list,match_to_list",
        "corepkg.dep.dep_check:dep_check,dep_eval,dep_wordreduce,dep_zapdeps",
        "corepkg.eclass_cache",
        "corepkg.elog",
        "corepkg.exception",
        "corepkg.getbinpkg",
        "corepkg.locks",
        "corepkg.locks:lockdir,lockfile,unlockdir,unlockfile",
        "corepkg.mail",
        "corepkg.manifest:Manifest",
        "corepkg.output",
        "corepkg.output:bold,colorize",
        "corepkg.package.ebuild.doebuild:doebuild,"
        + "doebuild_environment,spawn,spawnebuild",
        "corepkg.package.ebuild.config:autouse,best_from_dict,"
        + "check_config_instance,config",
        "corepkg.package.ebuild.deprecated_profile_check:deprecated_profile_check",
        "corepkg.package.ebuild.digestcheck:digestcheck",
        "corepkg.package.ebuild.digestgen:digestgen",
        "corepkg.package.ebuild.fetch:fetch",
        "corepkg.package.ebuild.getmaskingreason:getmaskingreason",
        "corepkg.package.ebuild.getmaskingstatus:getmaskingstatus",
        "corepkg.package.ebuild.prepare_build_dirs:prepare_build_dirs",
        "corepkg.process",
        "corepkg.process:atexit_register,run_exitfuncs",
        "corepkg.update:dep_transform,fixdbentries,grab_updates,"
        + "parse_updates,update_config_files,update_dbentries,"
        + "update_dbentry",
        "corepkg.util",
        "corepkg.util:atomic_ofstream,apply_secpass_permissions,"
        ...
        "corepkg.util.digraph:digraph",
        "corepkg.util.env_update:env_update",
        "corepkg.util.ExtractKernelVersion:ExtractKernelVersion",
        "corepkg.util.listdir:cacheddir,listdir",
        "corepkg.util.movefile:movefile",
        "corepkg.util.mtimedb:MtimeDB",
        "corepkg.versions",
        "corepkg.versions:best,catpkgsplit,catsplit,cpv_getkey,"
    ...
    sys.stderr.write("\n\n")
    sys.stderr.write(
        "!!! Failed to complete corepkg imports. There are internal modules for\n"
    )
    sys.stderr.write(
        "!!! corepkg and failure here indicates that you have a problem with your\n"
    )
    sys.stderr.write(
        "!!! installation of corepkg. Please try a rescue corepkg located in the ebuild\n"
    )
    sys.stderr.write(
        "!!! repository under '/var/db/repos/bp/app-core/corepkg/files/' (default).\n"
    )
    sys.stderr.write(
        "!!! There is a README.RESCUE file that details the steps required to perform\n"
    )
    sys.stderr.write("!!! a recovery of corepkg.\n")
    sys.stderr.write(f"    {e}\n\n")
    raise
...
#      and then corepkg would be forced to rename those files to match
#      sys.getfilesystemencoding(), possibly breaking things.
#
#   3) Automatic translation between encodings can lead to nonsensical
#      file names when the source encoding is unknown by corepkg.
#
...
_not_installed = os.path.isfile(
    os.path.join(PORTAGE_BASE_PATH, ".corepkg_not_installed")
)

# Api consumers included in corepkg should set this to True.
...
_supported_eapis = frozenset(
    chain(
        (str(x) for x in range(corepkg.const.EAPI + 1)),
        _testing_eapis,
        _deprecated_eapis,
    )
)

_runtime_single_eapi = "8"
...
def normalize_eapi(eapi):
    eapi = "" if eapi is None else str(eapi).strip()
    if not eapi:
        eapi = _runtime_single_eapi
    return eapi


def eapi_is_valid_for_runtime(eapi):
    return normalize_eapi(eapi) == _runtime_single_eapi


def unsupported_eapi_message(eapi):
    actual_eapi = normalize_eapi(eapi)
    return (
        f"unsupported EAPI: '{actual_eapi}' "
        f"(expected EAPI='{_runtime_single_eapi}')"
    )
...
def _movefile(src, dest, **kwargs):
    """Calls movefile and raises a CorepkgException if an error occurs."""
    if movefile(src, dest, **kwargs) is None:
        raise corepkg.exception.CorepkgException(f"mv '{src}' '{dest}'")
...
def create_trees(
    ...
    for myroot, mysettings in myroots:
        trees[myroot] = corepkg.util.LazyItemsDict(trees.get(myroot, {}))
        ...
        trees[myroot].addLazySingleton("porttree", corepkgtree, settings=mysettings)
        trees[myroot].addLazySingleton(
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        VERSION, commits_since_tag, commit, dirty = re.fullmatch(
                            "corepkg-([0-9.]*)-([0-9]*)-(g[0-9a-z]*)(-dirty)?",
                            result.stdout.strip(),
                        ).groups()
...
class _LegacyGlobalProxy(proxy.objectproxy.ObjectProxy):
    ...
    def _get_target(self):
        ...
        from corepkg._legacy_globals import _get_legacy_global
...
def _disable_legacy_globals():
    ...
    for k in _legacy_global_var_names:
        globals().pop(k, None)
    corepkg.data._initialized_globals.clear()
