...
from corepkg import _encodings, _unicode_decode, _unicode_encode
from corepkg import os
from corepkg.const import PRIVATE_PATH
from corepkg.dep import _slot_separator
from corepkg.localization import _
from corepkg.util import grabfile
from corepkg.versions import pkgsplit, vercmp
...
def get_applied_glsas(settings):
    ...
    @param	settings: corepkg config instance
    @rtype:		list
...
def get_glsa_list(myconfig):
    ...
    @type	myconfig: corepkg.config
    @param	myconfig: Corepkg settings instance
...
def makeAtom(pkgname, versionNode):
    ...
    I{versionNode} a (syntactical) valid corepkg atom.
...
def match(atom, dbapi, match_type="default"):
    """
    wrapper that calls revisionMatch() or corepkg.dbapi.dbapi.match() depending on
    the given atom.

    @type	atom: string
    @param	atom: a <~ or >~ atom or a normal corepkg atom that contains the atom to match against
    @type	dbapi: corepkg.dbapi.dbapi
    @param	dbapi: one of the corepkg databases to use as information source
...
def getMinUpgrade(vulnerableList, unaffectedList, portdbapi, vardbapi, minimize=True):
    ...
    @type	portdbapi:	corepkg.dbapi.porttree.portdbapi
    @param	portdbapi:	Ebuild repository
    @type	vardbapi:	corepkg.dbapi.vartree.vardbapi
