...
import corepkg
from corepkg import os, _unicode_encode
from corepkg.const import MERGING_IDENTIFIER, EPREFIX, PRIVATE_PATH, VDB_PATH
...
class TrackingFile:
    ...
    def __init__(self, tracking_path):
        ...
        """
        self._tracking_path = (
            tracking_path if corepkg.utf8_mode else _unicode_encode(tracking_path)
        )
    ...
    def save(self, failed_pkgs):
        ...
        corepkg.util.write_atomic(tracking_path, "\n".join(lines))
...
class MergesHandler:
    ...
    def _get_pkg_atoms(self, failed_pkgs, pkg_atoms, pkg_invalid_entries):
        ...
        portdb = corepkg.db[corepkg.root]["porttree"].dbapi
        for failed_pkg in failed_pkgs:
            # validate pkg name
    ...
    def _emerge_pkg_atoms(self, module_output, pkg_atoms, yes=False):
        ...
        """
        # TODO: rewrite code to use corepkg's APIs instead of a subprocess
        env = {
    ...
    def fix(self, **kwargs):
        ...
        vardb = corepkg.db[corepkg.root]["vartree"].dbapi
        still_failed_pkgs = {}
        for pkg, mtime in failed_pkgs.items():
