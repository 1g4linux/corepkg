...
import corepkg
from corepkg import _encodings, os, shutil
...
from corepkg.util.hooks import perform_hooks
...
def diffstatusoutput(cmd, file1, file2):
    ...
    args = (corepkg._unicode_encode(x, errors="strict") for x in args)
    proc = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    output = corepkg._unicode_decode(proc.communicate()[0])
...
def read_config(mandatory_opts):
    ...
    if corepkg._not_installed:
        config_path = os.path.join(
            corepkg.PORTAGE_BASE_PATH, "cnf", "dispatch-conf.conf"
        )
    else:
