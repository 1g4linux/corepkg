...
class ForkProcess(SpawnProcess):
    ...
    def _spawn(
    ...
    ) -> corepkg.process.MultiprocessingProcess:
        """
        ...
        if self.__class__._run is ForkProcess._run:
            ...
            kwargs = None
            warnings.warn(
                'corepkg.util._async.ForkProcess.ForkProcess._run is deprecated in favor of the "target" parameter',
                UserWarning,
                stacklevel=2,
            )
        ...
        try:
            ...
            if stdin_fd is not None and stdin_fd == corepkg._get_stdin().fileno():
        ...
        if _registered_run_exitfuncs != corepkg.getpid():
            corepkg.process._atexit_register_run_exitfuncs()
            _registered_run_exitfuncs = corepkg.getpid()

        return corepkg.process.MultiprocessingProcess(proc)
    ...
    def _bootstrap(child_connection, have_send_handle, fd_pipes, target, args, kwargs):
        ...
        if fd_pipes:
            ...
            corepkg.process._setup_pipes(fd_pipes, close_fds=False)
