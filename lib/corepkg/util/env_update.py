...
import corepkg
from corepkg import os, _encodings, _unicode_decode, _unicode_encode
from corepkg.checksum import prelink_capable
from corepkg.data import ostype
from corepkg.exception import ParseError
...
def env_update(
    ...
    corepkg will use NEEDED.ELF.2 data to automatically create them
    ...
    @param target_root: root that is passed to the ldconfig -r option,
            defaults to corepkg.settings["ROOT"].
    ...
    if vardbapi is None:
        if isinstance(env, config):
            ...
            if target_root is None:
                eprefix = corepkg.settings["EPREFIX"]
                target_root = corepkg.settings["ROOT"]
                target_eroot = corepkg.settings["EROOT"]
            else:
                eprefix = corepkg.const.EPREFIX
                target_eroot = os.path.join(target_root, eprefix.lstrip(os.sep))
                target_eroot = target_eroot.rstrip(os.sep) + os.sep
            if hasattr(corepkg, "db") and target_eroot in corepkg.db:
                vardbapi = corepkg.db[target_eroot]["vartree"].dbapi
            else:
                settings = config(
...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    if prev_mtimes is None:
        prev_mtimes = corepkg.mtimedb["ldpath"]
    if env is None:
    ...
    if prelink_capable:
        ...
        newprelink = atomic_ofstream(os.path.join(prelink_d, "corepkg.conf"))
        newprelink.write(
