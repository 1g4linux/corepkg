...
import corepkg
from corepkg.util import _unicode_decode, writemsg_level
...
async def async_check_locale(silent=False, env=None):
    ...
    proc = multiprocessing.Process(
        ...
        args=(silent, env, None if env is None else corepkg._native_string(mylocale)),
    )
    proc.start()
    proc = corepkg.process.MultiprocessingProcess(proc)
    await proc.wait()
