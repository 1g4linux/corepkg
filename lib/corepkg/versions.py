# versions.py -- core Corepkg functionality
...
from corepkg.exception import InvalidData
...
def vercmp(ver1: str, ver2: str, silent: int = 1) -> Optional[int]:
    ...
    @param pkg1: version to compare with (see ver_regexp in corepkg.versions.py)
    @type pkg1: string (example: "2.1.2-r3")
    @param pkg2: version to compare againts (see ver_regexp in corepkg.versions.py)
    ...
    4. None if ver1 or ver2 are invalid (see ver_regexp in corepkg.versions.py)
    """
    ...
    # NOTE: Behavior changed in r2309 (between corepkg-2.0.x and corepkg-2.1).
...
class _pkg_str(str):
    ...
    def __init__(
        ...
        from corepkg.repository.config import _gen_valid_repo
...
def cpv_getkey(mycpv: Union[_pkg_str, str], eapi: Any = None) -> Optional[str]:
    ...
    warnings.warn(
        "corepkg.versions.cpv_getkey() " + f"called with invalid cpv: '{mycpv}'",
        DeprecationWarning,
        stacklevel=2,
    )
