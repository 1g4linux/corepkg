...
import corepkg
from corepkg import eapi_is_valid_for_runtime, unsupported_eapi_message, _eapi_is_deprecated
...
def getmaskingstatus(mycpv, settings=None, portdb=None, myrepo=None):
    if settings is None:
        settings = config(clone=corepkg.settings)
    if portdb is None:
        portdb = corepkg.portdb
...
def _getmaskingstatus(mycpv, settings, portdb, myrepo=None):
    ...
    if not eapi_is_valid_for_runtime(eapi):
        return [_MaskReason("EAPI", unsupported_eapi_message(eapi))]
