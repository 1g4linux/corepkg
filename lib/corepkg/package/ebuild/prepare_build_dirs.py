...
import corepkg
from corepkg import os, shutil, _encodings, _unicode_encode, _unicode_decode
from corepkg.data import corepkg_gid, corepkg_uid, secpass
...
def prepare_build_dirs(myroot=None, settings=None, cleanup=False):
    ...
    try:
        for mydir in mydirs:
            ...
            try:
                apply_secpass_permissions(
                    mydir, gid=corepkg_gid, uid=corepkg_uid, mode=0o700, mask=0
                )
        ...
        for dir_key in ("HOME", "PKG_LOGDIR", "T"):
            ...
            apply_secpass_permissions(
                mysettings[dir_key], uid=corepkg_uid, gid=corepkg_gid
            )
        # The setgid bit prevents a lockfile group permission race for bug #468990.
        ipc_kwargs = {}
        if corepkg.data.secpass >= 1:
            ipc_kwargs["gid"] = corepkg_gid
    ...
    if mysettings.get("EBUILD_PHASE") not in ("info", "fetch", "pretend"):
        ...
        if "home-dir-template-copy" in settings.features:
            corepkg_username = mysettings.get("PORTAGE_USERNAME", "corepkg")
            home_template_dir = pwd.getpwnam(corepkg_username).pw_dir
        ...
        if myfeature in mysettings.features:
            ...
            try:
                ...
                for mydir in mydirs:
                    ...
                    if droppriv:
                        st = os.stat(mydir)
                        if st.st_gid != corepkg_gid or not dirmode == (
                        ...
                        if not droppriv_fix:
                            ...
                            for filename in os.listdir(mydir):
                                try:
                                    ...
                                    continue
                                if subdir_st.st_gid != corepkg_gid or (
                    ...
                    if modified or kwargs["always_recurse"] or droppriv_fix:
                        ...
                        if not apply_recursive_permissions(
                            ...
                            raise OperationNotPermitted(
                                _(
                                    "Failed to apply recursive permissions for the corepkg group."
                                )
                            )
...
def _prepare_workdir(mysettings):
    ...
    if corepkg.data.secpass >= 2:
        permissions["uid"] = corepkg_uid
    if corepkg.data.secpass >= 1:
        permissions["gid"] = corepkg_gid
...
def _ensure_log_subdirs(logdir, subdir):
    ...
    # If logdir is writable by the corepkg group but its uid
    # is not corepkg_uid, then set the uid to corepkg_uid if
    # we have privileges to do so, for compatibility with our
    # default logrotate config (see bug 378451). With the
    # "su corepkg corepkg" directive and logrotate-3.8.0,
    # logrotate's chown call during the compression phase will
    # only succeed if the log file's uid is corepkg_uid.
    if grp_mode and gid == corepkg_gid and corepkg.data.secpass >= 2:
        uid = corepkg_uid
        if st.st_uid != corepkg_uid:
...
def _prepare_fake_distdir(settings, alist):
    ...
    corepkg.util.ensure_dirs(edpath, gid=corepkg_gid, mode=0o755)
