...
from corepkg import bsd_chflags, load_mod, os, selinux, _unicode_decode
...
from corepkg.env.loaders import KeyValuePairFileLoader
from corepkg.exception import InvalidDependString, CorepkgException
from corepkg.localization import _
from corepkg.output import colorize
from corepkg.process import fakeroot_capable, sandbox_capable
from corepkg.repository.config import (
...
from corepkg.util.install_mask import _raise_exc
from corepkg.util.path import first_existing
from corepkg.util._path import exists_raise_eaccess, isdir_raise_eaccess
from corepkg.versions import catpkgsplit, catsplit, cpv_getkey, _pkg_str

from corepkg.package.ebuild._config import special_env_vars
from corepkg.package.ebuild._config.env_var_validation import validate_cmd_var
from corepkg.package.ebuild._config.features_set import features_set
from corepkg.package.ebuild._config.KeywordsManager import KeywordsManager
from corepkg.package.ebuild._config.LicenseManager import LicenseManager
from corepkg.package.ebuild._config.UseManager import UseManager
from corepkg.package.ebuild._config.LocationsManager import LocationsManager
from corepkg.package.ebuild._config.MaskManager import MaskManager
from corepkg.package.ebuild._config.VirtualsManager import VirtualsManager
from corepkg.package.ebuild._config.helper import (
...
def autouse(myvartree, use_cache=1, mysettings=None):
    warnings.warn("corepkg.autouse() is deprecated", DeprecationWarning, stacklevel=2)
    return ""
...
class config:
    """
    This class encompasses the main corepkg configuration.  Data is pulled from
    ...
    _module_aliases = {
        "cache.metadata_overlay.database": "corepkg.cache.flat_hash.mtime_md5_database",
        "corepkg.cache.metadata_overlay.database": "corepkg.cache.flat_hash.mtime_md5_database",
    }
    ...
    def __init__(
        ...
        @type mycpv: String
        @param config_profile_path: Configurable path to the profile (usually PROFILE_PATH from corepkg.const)
        @type config_profile_path: String
        @param config_incrementals: List of incremental variables
                (defaults to corepkg.const.INCREMENTALS)
        ...
        @type sysroot: String
        @param eprefix: set the EPREFIX variable (default is corepkg.const.EPREFIX)
        @type eprefix: String
        @param local_config: Enables loading of local config (/etc/corepkg); used most by repoman to
        ...
        @param repositories: Configuration of repositories.
                Defaults to corepkg.repository.config.load_repository_config().
        @type repositories: Instance of corepkg.repository.config.RepoConfigLoader class.
        """

        # This is important when config is reloaded after emerge --sync.
        _eapi_cache.clear()

        # When initializing the global corepkg.settings instance, avoid
        # raising exceptions whenever possible since exceptions thrown
        # from 'import corepkg' or 'import corepkg.exceptions' statements
        # can practically render the api unusable for api consumers.
        tolerant = hasattr(corepkg, "_initializing_globals")
        ...
        if clone:
            ...
            self.modules["default"] = {
                "portdbapi.auxdbmodule": "corepkg.cache.flat_hash.mtime_md5_database",
            }
            ...
            portdir_overlay = ""
            portdir_sync = None
            main_repo_sync_uri = None
            private_overlay_sync_uri = None
            for confs in [make_globals, make_conf, self.configdict["env"]]:
                ...
                if v is not None:
                    portdir_sync = v
                v = confs.get("PORTAGE_MAIN_REPO_SYNC_URI")
                if v is not None:
                    main_repo_sync_uri = v
                v = confs.get("PORTAGE_PRIVATE_OVERLAY_SYNC_URI")
                if v is not None:
                    private_overlay_sync_uri = v
            ...
            if main_repo_sync_uri:
                self["PORTAGE_MAIN_REPO_SYNC_URI"] = main_repo_sync_uri
            if private_overlay_sync_uri:
                self["PORTAGE_PRIVATE_OVERLAY_SYNC_URI"] = private_overlay_sync_uri
            ...
            if portdir_overlay:
                for ov in portdir_overlay:
                    ...
                    if isdir_raise_eaccess(ov) or corepkg._sync_mode:
                ...
                for x in profiles_complex:
                    ...
                    mygcfg_dlists.append(
                        getconfig(
                            ...
                            tolerant=tolerant,
                            expand=expand_map,
                            recursive=x.corepkg1_directories,
                        )
                    )
            ...
            # Blacklist vars that could interfere with corepkg internals.
            ...
            # The prefix of the running corepkg instance is used in the
            # ebuild environment to implement the --host-root option for
            # best_version and has_version.
            self["PORTAGE_OVERRIDE_EPREFIX"] = corepkg.const.EPREFIX
            self.backup_changes("PORTAGE_OVERRIDE_EPREFIX")

            self._ppropertiesdict = corepkg.dep.ExtendedAtomDict(dict)
            self._paccept_restrict = corepkg.dep.ExtendedAtomDict(dict)
            self._penvdict = corepkg.dep.ExtendedAtomDict(dict)
            self._pbashrcdict = {}
            self._pbashrc = ()
            ...
            for repo in self.repositories.repos_with_profiles():
                d = (
                    getconfig(
                        ...
                        recursive=repo.corepkg1_profiles,
                    )
                    or {}
                )
                if d:
            ...
            if local_config:
                ...
                for profile in profiles_complex:
                    if not "profile-bashrcs" in profile.profile_formats:
                        continue
                    self._pbashrcdict[profile] = corepkg.dep.ExtendedAtomDict(dict)
                ...
                if os.path.exists(provpath):
                    if _get_eapi_attrs(x.eapi).allows_package_provided:
                        pkgprovidedlines.append(
                            grabfile(provpath, recursive=x.corepkg1_directories)
                        )
                    else:
                        # TODO: bail out?
                        writemsg(
            ...
            if has_invalid_data:
                writemsg(
                    _("See corepkg(5) for correct package.provided usage.\n"),
                    noiselevel=-1,
                )
            self.pprovideddict = {}
            ...
            try:
                ...
                if corepkg.data._unprivileged_mode(eroot_or_parent, eroot_st):
                    unprivileged = True
                    ...
                    if "PORTAGE_USERNAME" not in self:
                        self["PORTAGE_USERNAME"] = "corepkg"
                        self.backup_changes("PORTAGE_USERNAME")

                    if "PORTAGE_GRPNAME" not in self:
                        self["PORTAGE_GRPNAME"] = "corepkg"
                        self.backup_changes("PORTAGE_GRPNAME")
            ...
            if self.depcachedir is None:
                self.depcachedir = os.path.join(
                    os.sep, corepkg.const.EPREFIX, DEPCACHE_PATH.lstrip(os.sep)
                )
            ...
            corepkg.output._init(config_root=self["PORTAGE_CONFIGROOT"])
            corepkg.data._init(self)
    ...
    @property
    def mygcfg(self):
        warnings.warn("corepkg.config.mygcfg is deprecated", stacklevel=3)
        return {}
    ...
    def _init_dirs(self):
        """
        Create a few directories that are critical to corepkg operation
        """
        from corepkg.data import corepkg_gid
        ...
        dir_mode_map = {
            ...
            PRIVATE_PATH: (corepkg_gid, 0o2750, 0o2, False),
            CACHE_PATH: (corepkg_gid, 0o755, 0o2, False),
        }
    ...
    def soname_provided(self):
        from corepkg.dep.soname.SonameAtom import SonameAtom
    ...
    def validate(self):
        ...
        (This code was previously in the global scope of corepkg.py)"""
        ...
        if profile_broken and not corepkg._sync_mode:
        ...
        if os.path.exists(abs_user_virtuals):
            writemsg("\n!!! /etc/corepkg/virtuals is deprecated in favor of\n")
            writemsg("!!! /etc/corepkg/profile/virtuals. Please move it to\n")
            ...
            try:
                ...
                try:
                    compression_binary = shlex.split(
                        corepkg.util.varexpand(compression["compress"], mydict=self)
                    )[0]
                    ...
                    if corepkg.process.find_binary(compression_binary) is None:
    ...
    def setcpv(self, mycpv, use_cache=None, mydb=None):
        ...
        if eapi_attrs.iuse_effective:
            corepkg_iuse = set(self._iuse_effective)
            corepkg_iuse.update(explicit_iuse)
            if built_use is not None:
                ...
                corepkg_iuse.update(built_use)
            self.configdict["pkg"]["IUSE_EFFECTIVE"] = " ".join(sorted(corepkg_iuse))

            self.configdict["env"]["BASH_FUNC____in_corepkg_iuse%%"] = (
                "() { "
                ...
                "}"
            ) % " ".join(f'["{x}"]=1' for x in corepkg_iuse)
        else:
            corepkg_iuse = self._get_implicit_iuse()
            corepkg_iuse.update(explicit_iuse)

            # The _get_implicit_iuse() returns a regular expression
            # so we can't use the (faster) map.  Fall back to
            # implementing ___in_corepkg_iuse() the older/slower way.

            # PORTAGE_IUSE is not always needed so it's lazily evaluated.
            self.configdict["env"].addLazySingleton(
                "PORTAGE_IUSE", _lazy_iuse_regex, corepkg_iuse
            )
            self.configdict["env"][
                "BASH_FUNC____in_corepkg_iuse%%"
            ] = "() { [[ $1 =~ ${PORTAGE_IUSE} ]]; }"
    ...
    def virts_p(self):
        warnings.warn(
            "corepkg config.virts_p attribute "
    ...
    def virtuals(self):
        warnings.warn(
            "corepkg config.virtuals attribute "
    ...
    def getvirtuals(self):
        from corepkg.dbapi.vartree import vartree
            ...
            if corepkg._internal_caller:
                ...
                try:
                    ...
                    with open("/dev/tty", "w") as f:
                        ...
                        f.write(
                            "=" * 8
                            + " Traceback for invalid call to corepkg.package.ebuild.config.config.__getitem__ "
                            + "=" * 8
                            + "\n"
                        )
                        f.writelines(stack)
                ...
                warnings.warn(
                    ...
                    % (
                        key,
                        "corepkg.package.ebuild.config.config.__getitem__",
                        "corepkg.package.ebuild.config.config.get",
                    ),
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    def _getitem(self, mykey):
        ...
        if mykey in self._constant_keys:
            ...
            # corepkg plans to update itself.
            if mykey == "PORTAGE_BIN_PATH":
                return corepkg._bin_path
            if mykey == "PORTAGE_PYM_PATH":
                return corepkg._pym_path
                ...
                if value:
                    try:
                        need_pym_path = not os.path.samefile(
                            value[0], corepkg._pym_path
                        )
                    except OSError:
                        pass
                if need_pym_path:
                    value.insert(0, corepkg._pym_path)
                return ":".join(value)

            if mykey == "PORTAGE_GID":
                return f"{corepkg_gid}"
    ...
    def environ(self):
        ...
        from corepkg.package.ebuild.doebuild import _phase_func_map
        ...
        for x, myvalue in self.iteritems():
            if (
                ...
                or x.startswith("PORTAGE_FETCH_HTTP_HEADER_")
                or x.startswith("RESUMECOMMAND_")
            ):
                continue
            if not isinstance(myvalue, str):
                writemsg(
