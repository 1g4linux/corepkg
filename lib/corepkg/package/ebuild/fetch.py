...
from corepkg.const import BASH_BINARY, CUSTOM_MIRRORS_FILE, GLOBAL_CONFIG_PATH
...
from corepkg.locks import lockfile, unlockfile
from corepkg.output import colorize, EOutput
...
from corepkg.process import spawn
...
_uri_command_var_component_re = re.compile(r"[^A-Za-z0-9]")


def _command_var_component(value):
    return _uri_command_var_component_re.sub("_", value).upper()


def _iter_uri_fetchcommand_vars(base_var, protocol, uri):
    """
    Yield fetch command variable names in descending order of specificity.

    Order:
      1. {base}_{protocol}_{host}_{port}
      2. {base}_{protocol}_{host}
      3. {base}_{protocol}
      4. {base}
    """
    protocol_component = _command_var_component(protocol) if protocol else ""
    if protocol_component:
        parsed_uri = urlparse(uri)
        host = parsed_uri.hostname
        if host:
            host_component = _command_var_component(host)
            try:
            ...
            if port is not None:
                yield (
                    f"{base_var}_{protocol_component}_{host_component}"
                    f"_{port}"
                )
            yield f"{base_var}_{protocol_component}_{host_component}"
        yield f"{base_var}_{protocol_component}"
    yield base_var


def _get_uri_fetchcommand(settings, base_var, protocol, uri):
    """
    Resolve a fetch command setting according to URI specificity.

    @return: tuple of (setting_name, setting_value)
    """
    for setting_name in _iter_uri_fetchcommand_vars(base_var, protocol, uri):
        setting_value = settings.get(setting_name)
        if setting_value is not None:
            return setting_name, setting_value
    return base_var, None
...
def _inject_http_header(fetch_args, header_value):
    ...
    Supports wget/wget2 (--header) and curl (-H), unless a header option is
    already present.
    """
    if not header_value or not fetch_args:
        return fetch_args

    fetcher = os.path.basename(fetch_args[0])

    if fetcher in ("wget", "wget2"):
        if any(
            arg == "--header" or arg.startswith("--header=")
            for arg in fetch_args[1:]
        ):
            return fetch_args
        return [fetch_args[0], "--header", header_value] + fetch_args[1:]

    if fetcher == "curl":
        if any(
            arg in ("-H", "--header")
            or arg.startswith("--header=")
            or arg.startswith("-H")
            for arg in fetch_args[1:]
        ):
            return fetch_args
        return [fetch_args[0], "-H", header_value] + fetch_args[1:]

    return fetch_args


def _get_oneg4_gitlab_header(uri, settings=None):
    """
    Build a GitLab Authorization header from ONEG4_GITLAB_TOKEN, if applicable.
    """
...
def _want_userfetch(settings):
    ...
    @param settings: corepkg config
    @type settings: corepkg.package.ebuild.config.config
    @return: True if desirable, False otherwise
    """
    return (
        "userfetch" in settings.features
        and corepkg.data.secpass >= 2
        and os.getuid() == 0
    )
...
def _drop_privs_userfetch(settings):
    """
    Drop privileges for userfetch, and update corepkg.data.secpass
...
def _spawn_fetch(settings, args, **kwargs):
    ...
    # wget pollute stderr (if corepkg detects a problem then it
...
async def _ensure_distdir(settings, distdir):
    ...
    @type distdir: str
    @raise CorepkgException: corepkg.exception wrapper exception
    """
    ...
    if "FAKED_MODE" in settings:
        # When inside fakeroot, directories with corepkg's gid appear
        # to have root's gid. Therefore, use root's gid instead of
        # corepkg's gid to avoid spurious permissions adjustments
        # when inside fakeroot.
        dir_gid = 0

    userfetch = corepkg.data.secpass >= 2 and "userfetch" in settings.features
    userpriv = corepkg.data.secpass >= 2 and "userpriv" in settings.features
    write_test_file = os.path.join(distdir, ".__corepkg_test_write__")
    ...
    if ensure_dirs(distdir, gid=dir_gid, mode=dirmode, mask=modemask):
        ...
        if not apply_recursive_permissions(
            ...
            raise OperationNotPermitted(
                _("Failed to apply recursive permissions for the corepkg group.")
            )
    ...
    def get_filenames(self, distdir):
        ...
        for dirpath, dirnames, filenames in os.walk(distdir, onerror=_raise_exc):
            for filename in filenames:
                try:
                    yield corepkg._unicode_decode(filename, errors="strict")
...
class FilenameHashLayout:
    ...
    def get_filenames(self, distdir):
        ...
        pattern += "*"
        for x in glob.iglob(
            corepkg._unicode_encode(os.path.join(distdir, pattern), errors="strict")
        ):
            try:
                yield corepkg._unicode_decode(x, errors="strict").rsplit("/", 1)[1]
            except UnicodeDecodeError:
                # Ignore it. Distfiles names must have valid UTF8 encoding.
                pass
...
class MirrorLayoutConfig:
    ...
    def read_from_file(self, f):
        from corepkg.util.configparser import (
            SafeConfigParser,
            read_configs,
            ConfigParserError,
        )
...
async def async_mirror_url(mirror_url, filename, mysettings, cache_path=None):
    ...
    from corepkg.util.configparser import ConfigParserError
...
def fetch(
    ...
    @type myuris: dict
    @param mysettings: Corepkg config instance.
    @type mysettings: corepkg.config
    ...
    @param use_locks: Enable locks. This parameter is ineffective if
            FEATURES=distlocks is disabled in the corepkg config!
    @type use_locks: bool
...
async def async_fetch(
    ...
    userfetch = corepkg.data.secpass >= 2 and "userfetch" in features
        ...
        if use_locks:
            ...
            writemsg(
                _(
                    ...
                    "FEATURES in /etc/corepkg/make.conf\n"
                ),
                noiselevel=-1,
            )
    # 			use_locks = 0
    ...
    for myfile in filedict:
        ...
        try:
                        ...
                        if userfetch:
                            writemsg(
                                _(
                                    ...
                                    " in /etc/corepkg/make.conf in order to fetch with\n"
            ...
            while uri_list:
                ...
                protocol = loc[0 : loc.find("://")] if "://" in loc else ""

                global_config_path = GLOBAL_CONFIG_PATH
                if corepkg.const.EPREFIX:
                    global_config_path = os.path.join(
                        corepkg.const.EPREFIX, GLOBAL_CONFIG_PATH.lstrip(os.sep)
                    )

                missing_file_param = False
                fetchcommand_var, fetchcommand = _get_uri_fetchcommand(
                    mysettings, "FETCHCOMMAND", protocol, loc
                )
                ...
                resumecommand_var, resumecommand = _get_uri_fetchcommand(
                    mysettings, "RESUMECOMMAND", protocol, loc
                )
                ...
                if fetched != 2 and has_space:
                    ...
                    if protocol in ("http", "https"):
                        _fetch_var, header_value = _get_uri_fetchcommand(
                            mysettings, "PORTAGE_FETCH_HTTP_HEADER", protocol, loc
                        )
                        if header_value:
                            header_value = varexpand(
                                header_value, mydict=mysettings
                            ).strip()
                        else:
                            header_value = _get_oneg4_gitlab_header(loc, mysettings)
                        if header_value:
                            myfetch = _inject_http_header(myfetch, header_value)
                    ...
                    try:
                        ...
                        try:
                            apply_secpass_permissions(
                                download_path, gid=corepkg_gid, mode=0o664, mask=0o2
                            )
                        except FileNotFound:
                            pass
                        except CorepkgException as e:
