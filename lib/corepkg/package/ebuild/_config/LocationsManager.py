...
from corepkg.eapi import eapi_allows_directories_on_profile_level_and_repository_level
from corepkg.exception import DirectoryNotFound, InvalidLocation, ParseError
...
from corepkg.repository.config import (
    parse_layout_conf,
    _corepkg1_profiles_allow_directories,
    _profile_node,
)
...
_allow_parent_colon = frozenset(["corepkg-2"])
...
class LocationsManager:
    def __init__(
        ...
        if self.eprefix is None:
            self.eprefix = corepkg.const.EPREFIX
        elif self.eprefix:
        ...
        if self.config_root is None:
            self.config_root = corepkg.const.EPREFIX + os.sep
        ...
        self.broot = corepkg.const.EPREFIX
    ...
    def _addProfile(self, currentPath, repositories, known_repos, previous_repos):
        ...
        try:
            ...
            if not eapi_is_valid_for_runtime(eapi):
                raise ParseError(
                    _("Profile contains %s: '%s'")
                    % (
                        unsupported_eapi_message(eapi),
        ...
        if compat_mode:
            ...
            if offenders:
                warnings.warn(
                    _(
                        "\nThe selected profile is implicitly using the 'corepkg-1' format:\n"
