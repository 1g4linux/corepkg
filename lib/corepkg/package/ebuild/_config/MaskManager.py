...
from corepkg.dep import ExtendedAtomDict, match_from_list
from corepkg.localization import _
from corepkg.repository.config import allow_profile_repo_deps
from corepkg.util import append_repo, grabfile_package, stack_lists, writemsg
...
class MaskManager:
    def __init__(
        ...
        def grab_pmask(loc, repo_config):
            if loc not in pmask_cache:
                ...
                pmask_cache[loc] = grabfile_package(
                    path,
                    recursive=repo_config.corepkg1_profiles,
                ...
                if repo_config.corepkg1_profiles_compat and os.path.isdir(path):
                    warnings.warn(
                        _(
                            "Repository '%(repo_name)s' is implicitly using "
                            "'corepkg-1' profile format in its profiles/package.mask, but "
        ...
        # Read /etc/corepkg/package.mask. Don't stack it to allow the user to
