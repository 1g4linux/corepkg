...
class SpawnNofetchWithoutBuilddir(CompositeTask):
    ...
    def _start(self):
        ...
        corepkg_tmpdir = settings.get("PORTAGE_TMPDIR")
        if not corepkg_tmpdir or not os.access(corepkg_tmpdir, os.W_OK):
            corepkg_tmpdir = None
        private_tmpdir = self._private_tmpdir = tempfile.mkdtemp(dir=corepkg_tmpdir)
