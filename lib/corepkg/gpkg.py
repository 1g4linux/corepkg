...
import corepkg
from corepkg import checksum
from corepkg import os
from corepkg import shutil
from corepkg import normalize_path
...
from corepkg.util._urlopen import urlopen
...
from corepkg.util.cpuinfo import makeopts_to_job_count
from corepkg.process import find_binary
from corepkg.const import MANIFEST2_HASH_DEFAULTS, HASHING_BLOCKSIZE
    ...
    def _cmd_read_thread(self):
        ...
        while True:
            try:
                ...
                if not self.killed:
                    # Do not raise error if killed by Corepkg
...
class checksum_helper:
    ...
    def show_gpg_error(self, operation, gpg_error_lines):
        ...
        corepkg_trust_helper_msg = [
            # fmt: off
            "\t" + "Possible fix:",
            "\t" + f"Try running '{corepkg_trust_helper}', i.e., $PORTAGE_TRUST_HELPER, as root",
            # fmt: on
        ]
        ...
        if _match_list(r"^\[GNUPG:\] NO_PUBKEY", gpg_error_lines):
            ...
            error_summaries.extend(corepkg_trust_helper_msg)
...
class gpkg:
    ...
    def update_metadata(self, metadata, new_basename=None, force=False):
        ...
        with (
            tempfile.NamedTemporaryFile(
                ...
                prefix=f"{os.path.basename(self.gpkg_file)}.{corepkg.getpid()}",
                delete=False,
            ) as tmp_gpkg_file,
            tarfile.TarFile(
    ...
    def _get_binary_cmd(self, compression, mode):
        """
        Get command list from corepkg and try to match compressor
        """
