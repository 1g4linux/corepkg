...
lchown = corepkg._unicode_func_wrapper(lchown)


def _target_eprefix():
    """
    Calculate the target EPREFIX, which may be different from
    corepkg.const.EPREFIX due to cross-prefix support. The result
    is equivalent to corepkg.settings["EPREFIX"], but the calculation
    is done without the expense of instantiating corepkg.settings.
    @rtype: str
    @return: the target EPREFIX
    """
    eprefix = os.environ.get("EPREFIX", corepkg.const.EPREFIX)
    if eprefix:
        eprefix = corepkg.util.normalize_path(eprefix)
    return eprefix
...
def _target_root():
    ...
    corepkg.settings["ROOT"], but the calculation
    is done without the expense of instantiating corepkg.settings.
    @rtype: str
    ...
    if not root:
        ...
        root = os.sep
    root = corepkg.util.normalize_path(root)
    return f"{root.rstrip(os.sep)}{os.sep}"


def corepkg_group_warning():
    from corepkg.output import colorize
    from corepkg.util import writemsg
    ...
    mylines = (
        ...
        "allowed in the corepkg group.  Untrusted users or processes",
        "can potentially exploit the corepkg group for attacks such as",
...
# Corepkg has 3 security levels that depend on the uid and gid of the main
# process and are assigned according to the following table:
#
# Privileges  secpass  uid    gid
# normal      0        any    any
# group       1        any    corepkg_gid
# super       2        0      any
#
# If the "wheel" group does not exist then wheelgid falls back to 0.
# If the "corepkg" group does not exist then corepkg_uid falls back to wheelgid.
...
# The corepkg_uid and corepkg_gid global constants, and others that
...
def _get_global(k):
    ...
    if k == "secpass":
        unprivileged = False
        if hasattr(corepkg, "settings"):
            unprivileged = "unprivileged" in corepkg.settings.features
        else:
        ...
        if uid == 0 or unprivileged:
            v = 2
        elif _get_global("corepkg_gid") in os.getgroups():
            v = 1

    elif k in ("corepkg_gid", "corepkg_uid"):
        # Discover the uid and gid of the corepkg user/group
        keyerror = False
        try:
            corepkg_uid = pwd.getpwnam(_get_global("_corepkg_username")).pw_uid
        except KeyError:
            keyerror = True
            corepkg_uid = 0

        try:
            corepkg_gid = grp.getgrnam(_get_global("_corepkg_grpname")).gr_gid
        except KeyError:
            grpname = _get_global("_corepkg_grpname")
            if grpname.isdigit():
                corepkg_gid = int(grpname)
            else:
                keyerror = True
                corepkg_gid = 0
        ...
        if keyerror and not (
            _get_global("_corepkg_username") == "root"
            and _get_global("_corepkg_grpname") == "root"
        ):
            writemsg(
                colorize("BAD", _("corepkg: 'corepkg' user or group missing.")) + "\n",
                noiselevel=-1,
            )
            writemsg(
                _(
            ...
            writemsg(
                colorize(
                    "GOOD",
                    "         corepkg:x:250:250:corepkg:/var/tmp/corepkg:/bin/false",
                )
                + "\n",
                noiselevel=-1,
            )
            writemsg(
                colorize("GOOD", "         corepkg::250:corepkg") + "\n", noiselevel=-1
            )
            corepkg_group_warning()

        globals()["corepkg_gid"] = corepkg_gid
        _initialized_globals.add("corepkg_gid")
        globals()["corepkg_uid"] = corepkg_uid
        _initialized_globals.add("corepkg_uid")
        ...
        v = [_get_global("corepkg_gid")]
        if secpass >= 2:
            # Get a list of group IDs for the corepkg user. Do not use
            # grp.getgrall() since it is known to trigger spurious
            # SIGPIPE problems with nss_ldap.
            encoding = corepkg._encodings["content"]
            cmd = (
                corepkg._unicode_encode(x, encoding=encoding, errors="strict")
                for x in ("id", "-G", _corepkg_username)
            )
            ...
            if os.WIFEXITED(status) and os.WEXITSTATUS(status) == os.EX_OK:
                ...
                unicode_decode = corepkg._unicode_decode(
    ...
    # Avoid instantiating corepkg.settings when the desired
    # variable is set in os.environ.
    elif k in ("_corepkg_grpname", "_corepkg_username"):
        v = "corepkg"
        env_key = "PORTAGE_USERNAME"
        if k == "_corepkg_grpname":
            env_key = "PORTAGE_GRPNAME"

        if env_key in os.environ:
            v = os.environ[env_key]
        elif hasattr(corepkg, "settings"):
            v = corepkg.settings.get(env_key, v)
        else:
            # The config class has equivalent code, but we also need to
            # do it here if _disable_legacy_globals() has been called.
            eroot_or_parent = first_existing(
...
class _GlobalProxy(corepkg.proxy.objectproxy.ObjectProxy):
    __slots__ = ("_name",)

    def __init__(self, name):
        corepkg.proxy.objectproxy.ObjectProxy.__init__(self)
...
def _init(settings):
    ...
    """
    if (
        "_corepkg_grpname" not in _initialized_globals
        and "_corepkg_username" not in _initialized_globals
    ):
        # Prevents "TypeError: expected string" errors
        # from grp.getgrnam() with PyPy
        native_string = platform.python_implementation() == "PyPy"

        v = settings.get("PORTAGE_GRPNAME", "corepkg")
        if native_string:
            v = corepkg._native_string(v)
        globals()["_corepkg_grpname"] = v
        _initialized_globals.add("_corepkg_grpname")

        v = settings.get("PORTAGE_USERNAME", "corepkg")
        if native_string:
            v = corepkg._native_string(v)
        globals()["_corepkg_username"] = v
        _initialized_globals.add("_corepkg_username")

    if "secpass" not in _initialized_globals:
        v = 0
        if uid == 0 or "unprivileged" in settings.features:
            v = 2
        elif corepkg_gid in os.getgroups():
            v = 1
        globals()["secpass"] = v
