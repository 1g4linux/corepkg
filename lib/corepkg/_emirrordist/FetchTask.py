...
import corepkg
from corepkg import _encodings, _unicode_encode
from corepkg import os
from corepkg.util import ensure_dirs
from corepkg.util._async.FileCopier import FileCopier
from corepkg.util._async.FileDigester import FileDigester
from corepkg.util._async.PipeLogger import PipeLogger
from corepkg.util._async.PopenProcess import PopenProcess
from _emerge.CompositeTask import CompositeTask

logger = logging.getLogger(__name__)

default_hash_name = corepkg.const.MANIFEST2_HASH_DEFAULT
...
class FetchTask(CompositeTask):
    ...
    def _fetch_uri(self, uri):
        ...
        with tempfile.NamedTemporaryFile(
            dir=distdir, prefix=f"._emirrordist_fetch_.{corepkg.getpid()}"
        ) as safe_temp:
        ...
        args = [corepkg.util.varexpand(x, mydict=variables) for x in args]
    ...
    def _select_hash(self):
        ...
        for hash_name in self.digests:
            if (
                hash_name != "size"
                and hash_name in corepkg.checksum.get_valid_checksum_keys()
            ):
                return hash_name
    ...
    def _hardlink_atomic(self, src, dest, dir_info, symlink=False):
        ...
        with tempfile.NamedTemporaryFile(
            dir=head, prefix=f".{tail}._mirrordist_hardlink_.{corepkg.getpid()}"
        ) as safe_temp:
