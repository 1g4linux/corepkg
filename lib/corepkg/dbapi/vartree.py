...
from corepkg import abssymlink, _movefile, bsd_chflags

# This is a special version of the os module, wrapped for unicode support.
from corepkg import os
from corepkg import shutil
from corepkg import _encodings
from corepkg import _os_merge
from corepkg import _selinux_merge
...
class vardbapi(dbapi):
    ...
    def __init__(
        ...
        available packages.
        """
        from corepkg.util._dyn_libs.PreservedLibsRegistry import PreservedLibsRegistry
        from corepkg.util._dyn_libs.LinkageMapELF import LinkageMapELF as LinkageMap
        ...
        self.blockers = None
        if settings is None:
            settings = corepkg.settings
        self.settings = settings

        if _unused_param is not DeprecationWarning:
            warnings.warn(
                "The first parameter of the "
                "corepkg.dbapi.vartree.vardbapi"
        ...
        if vartree is None:
            vartree = corepkg.db[settings["EROOT"]]["vartree"]
    ...
    def lock(self):
        ...
        from corepkg.locks import lockdir
    ...
    def unlock(self):
        ...
        """
        from corepkg.locks import unlockdir
    ...
    def _fs_lock(self):
        ...
        processes.
        """
        from corepkg.locks import lockfile
    ...
    def _fs_unlock(self):
        ...
        """
        from corepkg.locks import unlockfile
    ...
    def _bump_mtime(self, cpv):
        ...
        """
        from corepkg.util import ensure_dirs
        from corepkg.versions import catsplit
    ...
    def cpv_counter(self, mycpv):
        ...
        try:
            ...
            pass
        writemsg_level(
            _("corepkg: COUNTER for %s was corrupted; " "resetting to value of 0\n")
            % (mycpv,),
            level=logging.ERROR,
            noiselevel=-1,
        )
    ...
    def cpv_inject(self, mycpv):
        ...
        from corepkg.util import ensure_dirs, write_atomic
    ...
    def move_ent(self, mylist, repo_match=None):
        from corepkg.dep import isjustname, isvalidatom
        from corepkg.versions import catsplit
        from corepkg.util import ensure_dirs, write_atomic
    ...
    def cp_list(self, mycp, use_cache=1):
        from corepkg.versions import catsplit, pkgsplit, _pkg_str
    ...
    def cpv_all(self, use_cache=1):
        """
        Set use_cache=0 to bypass the corepkg.cachedir() cache in cases
    ...
    def _iter_cpv_all(self, use_cache=True, sort=False):
        from corepkg.versions import _pkg_str

        returnme = []
        basepath = os.path.join(self._eroot, VDB_PATH) + os.path.sep

        if use_cache:
            from corepkg import listdir
        else:
    ...
    def cp_all(self, use_cache=1, sort=False):
        from corepkg.versions import catpkgsplit
    ...
    def _clear_pkg_cache(self, pkg_dblink):
        from corepkg.util.listdir import dircache
    ...
    def match(self, origdep, use_cache=1):
        ...
        from corepkg.dep import dep_getkey
        from corepkg.versions import catsplit
    ...
    def flush_cache(self):
        ...
        from corepkg.util import ensure_dirs, atomic_ofstream, apply_secpass_permissions
    ...
    def _aux_cache_init(self):
        from corepkg.util import writemsg
    ...
    def aux_get(self, mycpv, wants, myrepo=None):
        ...
        """
        from corepkg.eapi import _get_eapi_attrs
        from corepkg.versions import _get_slot_re
    ...
    def _aux_env_search(self, cpv, variables):
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                raise
            raise corepkg.exception.CommandNotFound(args[0])
    ...
    async def unpack_metadata(self, pkg, dest_dir, loop=None):
        ...
        @type pkg: _pkg_str or corepkg.config
        @param dest_dir: destination directory
        @type dest_dir: str
        """
        loop = asyncio._wrap_loop(loop)
        if not isinstance(pkg, corepkg.config):
            cpv = pkg
        else:
            cpv = pkg.mycpv
    ...
    async def unpack_contents(
        ...
        if binpkg_format == "xpak":
            ...
            if proc.returncode != os.EX_OK:
                raise CorepkgException(f"command failed: {tar_cmd}")
            ...
            corepkg.gpkg.gpkg(settings, cpv, gpkg_tmp).decompress(dest_dir)
            os.remove(gpkg_tmp)
        else:
        ...
        if excluded_config_files:
            ...
            for line in log_lines:
                corepkg.elog.messages.ewarn(line, phase="install", key=cpv, out=out)
            scheduler.output(
                out.getvalue(),
    ...
    def counter_tick_core(self, incrementing: int = 1) -> int:
        ...
        """
        from corepkg.util import write_atomic
    ...
    def removeFromContents(self, pkg, paths, relative_paths=True):
        ...
        from corepkg.util import normalize_path, writemsg_level
    ...
    def writeContentsToContentsFile(self, pkg, new_contents, new_needed=None):
        ...
        """
        from corepkg.util import atomic_ofstream
        from corepkg.util._dyn_libs.LinkageMapELF import LinkageMapELF as LinkageMap
...
class vartree:
    ...
    def dep_bestmatch(self, mydep, use_cache=1):
        from corepkg.versions import best
        from corepkg.dbapi.dep_expand import dep_expand
...
class dblink:
    ...
    def __init__(
        ...
        @param settings: Typically corepkg.settings
        ...
        if vartree is None:
            vartree = corepkg.db[self._eroot]["vartree"]
        self.vartree = vartree
    ...
    def _get_protect_obj(self):
        from corepkg.util import ConfigProtect
    ...
    def _acquire_slot_locks(self, db):
        ...
        slot_atoms.append(corepkg.dep.Atom(f"{self.mycpv.cp}:{slot}"))
    ...
    def delete(self):
        ...
        if not self.dbdir.startswith(self.dbroot):
            writemsg(
                _("corepkg.dblink.delete(): invalid dbdir: %s\n") % self.dbdir,
                noiselevel=-1,
            )
            return
    ...
    def quickpkg(
        ...
        """
        import tarfile
        from corepkg.checksum import _perform_md5_merge as perform_md5
        from corepkg.util import ConfigProtect
        ...
        if binpkg_format == "xpak":
            ...
            gpkg_file = corepkg.gpkg.gpkg(settings, cpv, output_file)
    ...
    def unmerge(
        ...
        """
        from corepkg.package.ebuild.doebuild import (
            doebuild_environment,
            prepare_build_dirs,
        )
        from corepkg.util import write_atomic
        from corepkg.util._async.SchedulerInterface import SchedulerInterface
        from corepkg.util.env_update import env_update
        from corepkg.versions import catsplit, cpv_getkey

        if trimworld is not None:
            warnings.warn(
                "The trimworld parameter of the "
                + "corepkg.dbapi.vartree.dblink.unmerge()"
    ...
    def _unmerge_pkgfiles(self, pkgfiles, others_in_slot):
        ...
        """
        from corepkg.checksum import _perform_md5_merge as perform_md5
        from corepkg.util import grabdict, normalize_path, writedict
        from corepkg.versions import catsplit, cpv_getkey
                ...
                if os is _os_merge:
                    try:
                        ...
                        try:
                            ...
                            os = corepkg.os
                            perf_md5 = corepkg.checksum.perform_md5
    ...
    def isowner(self, filename, destroot=None):
        ...
        if destroot is not None and destroot != self._eroot:
            warnings.warn(
                ...
                + "corepkg.dbapi.vartree.dblink.isowner()"
    ...
    def _match_contents(self, filename, destroot=None):
        ...
        if destroot is not None and destroot != self._eroot:
            warnings.warn(
                "The second parameter of the "
                + "corepkg.dbapi.vartree.dblink._match_contents()"
                + " is now unused. Instead "
    ...
    def _collision_protect(self, srcroot, destroot, mypkglist, file_list, symlink_list):
        ...
        from corepkg.util import normalize_path
        from corepkg.util.path import iter_parents
        ...
        # packdebug tarballs are orphaned for management outside of Corepkg,
    ...
    def _elog(self, funcname, phase, lines):
        func = getattr(corepkg.elog.messages, funcname)
        if self._scheduler is None:
            for l in lines:
    ...
    def _elog_process(self, phasefilter=None):
        from corepkg.elog import elog_process, collect_ebuild_messages, collect_messages
        from corepkg.elog import _merge_logentries
    ...
    def treewalk(
        ...
        calls get_ro_checker to retrieve a function for checking whether Corepkg
        ...
        """
        from corepkg.dep import match_from_list, _slot_separator, _repo_separator
        from corepkg.package.ebuild.doebuild import (
            doebuild_environment,
        ...
        from corepkg.util.env_update import env_update
        from corepkg.util.install_mask import InstallMask, install_mask_dir
        from corepkg.util.writeable_check import get_ro_checker
        from corepkg.package.ebuild.prepare_build_dirs import prepare_build_dirs
        from corepkg.versions import (
            _pkg_str,
            _unknown_repo,
            catsplit,
            cpv_getkey,
        ...
        try:
            ...
            for cur_cpv in slot_matches:
                ...
                # like elog.
                settings_clone = corepkg.config(clone=self.settings)
                ...
                msg.extend(
                    wrap(
                        _(
                            ...
                            "/etc/corepkg/make.conf if you do not want to "
        ...
        self.settings["REPLACING_VERSIONS"] = " ".join(
            [corepkg.versions.cpv_getversion(other.mycpv) for other in others_in_slot]
        )
        ...
        if collisions:
            ...
            if self.settings.get("PORTAGE_QUIET") != "1":
                msg += _(
                    ...
                    " `corepkgq owners / <filename>` to identify the"
                    " installed package that owns a file. If corepkgq"
        ...
        # If corepkg is reinstalling itself, remove the old
        ...
        for dblnk in list(others_in_slot):
            ...
            dblnk.settings["REPLACED_BY_VERSION"] = corepkg.versions.cpv_getversion(
                self.mycpv
            )
    ...
    def _merge_contents(self, srcroot, destroot, cfgfiledict):
        from corepkg.util import atomic_ofstream, writedict
    ...
    def mergeme(
        ...
        from hashlib import md5
        from corepkg.checksum import _perform_md5_merge as perform_md5
        from corepkg.eapi import eapi_rewrites_symlinks
        from corepkg.util import normalize_path
        from corepkg.util.movefile import movefile
        from corepkg.versions import pkgsplit
                ...
                if mydmode is not None:
                    ...
                    if stat.S_ISDIR(mydmode) or (
                        ...
                        try:
                            ...
                            # corepkg.util.ensure_dirs() for cases
                            # like bug #187518.
    ...
    def _protect(
        ...
        from corepkg.util import new_protect_filename
    ...
    def _post_merge_sync(self):
        ...
        """
        import subprocess
        from corepkg.dbapi._SyncfsProcess import SyncfsProcess
    ...
    def merge(
        ...
        """
        from corepkg.package.ebuild.doebuild import doebuild_environment
        from corepkg.util._async.SchedulerInterface import SchedulerInterface
        ...
        if retval == os.EX_OK and self._postinst_failure:
            retval = corepkg.const.RETURNCODE_POSTINST_FAILURE
    ...
    def _quickpkg_dblink(self, backup_dblink, background, logfile):
        ...
        try:
            ...
            if not pythonpath or not os.path.samefile(pythonpath[0], corepkg._pym_path):
                pythonpath.insert(0, corepkg._pym_path)
    ...
    def _needs_move(self, mysrc, mydest, mymode, mydmode):
        ...
        """
        from corepkg.util import writemsg
        from corepkg.util.movefile import _cmpxattr
...
def merge(
    ...
    """
    from corepkg.dbapi._MergeProcess import MergeProcess
    from corepkg.util import writemsg
...
def tar_contents(contents, root, tar, protect=None, onProgress=None, xattrs=False):
    import tarfile
    from corepkg.util import normalize_path
    from corepkg.util._xattr import xattr
