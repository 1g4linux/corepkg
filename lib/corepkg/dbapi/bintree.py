...
from corepkg.cache.mappings import slot_dict_class
...
from corepkg.dep import Atom, use_reduce, paren_enclose
from corepkg.exception import (
    ...
    ParseError,
    CorepkgException,
    CorepkgPackageException,
    SignatureException,
)
from corepkg._sets.base import WildcardPackageSet
from corepkg.localization import _
from corepkg.output import colorize
from corepkg.package.ebuild.profile_iuse import iter_iuse_vars
from corepkg.sync.revision_history import get_repo_revision_history
...
class bindbapi(fakedbapi):
    ...
    def aux_get(self, mycpv, wants, myrepo=None):
        ...
        # consumers (including corepkgq, which passes in a cpv from
        ...
        if not self.bintree._remotepkgs or not self.bintree.isremote(mycpv):
            ...
            if binpkg_format == "xpak":
                metadata_bytes = corepkg.xpak.tbz2(binpkg_path).get_data()
                decode_metadata_name = False
            elif binpkg_format == "gpkg":
                metadata_bytes = corepkg.gpkg.gpkg(
        ...
        if not mydata.setdefault("EAPI", "8"):
            mydata["EAPI"] = "8"
    ...
    def aux_update(self, cpv, values):
        ...
        binpkg_format = get_binpkg_format(binpkg_path)
        if binpkg_format == "xpak":
            mytbz2 = corepkg.xpak.tbz2(binpkg_path)
            mydata = mytbz2.get_data()
            encoding_key = True
        elif binpkg_format == "gpkg":
            mybinpkg = corepkg.gpkg.gpkg(self.settings, cpv_str, binpkg_path)
            try:
        ...
        for k, v in list(mydata.items()):
            if not v:
                del mydata[k]
        if binpkg_format == "xpak":
            mytbz2.recompose_mem(corepkg.xpak.xpak_mem(mydata))
        elif binpkg_format == "gpkg":
        ...
        if add_pkg is not None:
            ...
            if binpkg_format == "xpak":
                await loop.run_in_executor(
                    ...
                    corepkg.xpak.tbz2(binpkg_file).unpackinfo,
                    dest_dir,
                )
            elif binpkg_format == "gpkg":
                gpkg_args = {}
        ...
        if pkg_path is not None:
            ...
            if binpkg_format == "xpak":
                ...
                if extractor.returncode != os.EX_OK:
                    raise CorepkgException(f"Error extracting '{pkg_path}'")
            elif binpkg_format == "gpkg":
                gpkg_args = {}
                repoconfig = self.bintree.get_local_repo(cpv)
                if repoconfig:
                ...
                raise corepkg.exception.InvalidBinaryPackageFormat(pkg_path)
        else:
    ...
    def getfetchsizes(self, pkg):
        ...
        if not self.bintree.download_required(pkg):
            ...
            try:
                ...
                raise corepkg.exception.MissingSignature("SIZE")
            except ValueError:
                raise corepkg.exception.InvalidSignature(f"SIZE: {metadata['SIZE']}")
            else:
...
class binarytree:
    ...
    def __init__(
        ...
        if _unused is not DeprecationWarning:
            warnings.warn(
                ...
                "corepkg.dbapi.bintree.binarytree"
    ...
    def move_ent(self, mylist, repo_match=None):
        ...
        from corepkg.update import update_dbentries
        from corepkg.util import writemsg
        from corepkg.versions import _pkg_str, catsplit
        ...
        for mycpv in origmatches:
            ...
            if binpkg_format == "xpak":
                ...
                mybinpkg = corepkg.gpkg.gpkg(self.settings, mycpv, binpkg_path)
                try:
                    mydata = mybinpkg.get_metadata()
                    signature_exist = mybinpkg.signature_exist
                except SignatureException:
            ...
            try:
                ...
                if binpkg_format == "xpak":
                    mytbz2 = corepkg.xpak.tbz2(update_path)
                    mytbz2.recompose_mem(corepkg.xpak.xpak_mem(mydata))
                elif binpkg_format == "gpkg":
                    mybinpkg = corepkg.gpkg.gpkg(self.settings, mycpv, update_path)
    ...
    def prevent_collision(self, cpv):
        warnings.warn(
            "The "
            "corepkg.dbapi.bintree.binarytree.prevent_collision "
    ...
    def _file_permissions(self, path):
        try:
            ...
            try:
                corepkg.util.apply_permissions(
    ...
    def _populate_local(self, reindex=True, invalid_errors=True):
        from corepkg.util import writemsg
        from corepkg.versions import _pkg_str, catpkgsplit, catsplit
        ...
        if True:
            ...
            for mydir, file_names in dir_files.items():
                ...
                for myfile in file_names:
                    ...
                    except (CorepkgPackageException, SignatureException) as e:
                        if invalid_errors:
                            writemsg(
                    ...
                    if not self.dbapi._category_re.match(mycat):
                        ...
                        writemsg(
                            _(
                                ...
                                " listed in %setc/corepkg/categories\n"
                            )
    ...
    def _run_trust_helper(self):
        ...
        corepkg_trust_helper = self.settings.get("PORTAGE_TRUST_HELPER", "")
        if corepkg_trust_helper == "":
            return
        try:
            ret = subprocess.run(corepkg_trust_helper)
        except FileNotFoundError:
            writemsg(
                _(
                    "\n!!! Corepkg trust helper %s for binary packages not found\n!!! Continuing, but did you install app-corepkg/getuto?\n"
                )
                % corepkg_trust_helper,
                noiselevel=-1,
            )
            return
    ...
    def _populate_remote(
        ...
        if need_trust_helper:
            if not pretend and self.dbapi.writable and corepkg.data.secpass >= 2:
    ...
    def _populate_remote_repo(
        ...
        from corepkg.util import atomic_ofstream, writemsg
        from corepkg.util.time import unix_to_iso_time
        from corepkg.util._urlopen import (
            urlopen as _urlopen,
            ...
            for remote_pkgindex_file in ("Packages.gz", "Packages"):
                ...
                if (
                    ...
                    try:
                        ...
                        raise ParseError(
                            f"Invalid Corepkg BINHOST value '{url.lstrip()}'"
                        )
                ...
                if f is None:
                    ...
                    if repo.fetchcommand is None and parsed_url.scheme == "ssh":
                        ...
                        success = corepkg.getbinpkg.file_get(
        ...
        if pkgindex is rmt_idx and changed:
            ...
            try:
                ...
                pkgindex.write(f)
                f.close()
            except (OSError, CorepkgException):
    ...
    def remove(self, cpv: corepkg.versions._pkg_str) -> None:
        """
        ...
        @type cpv: corepkg.versions._pkg_str
        @rtype: None
        @return: None
    ...
    def _read_metadata(self, filename, st, keys=None, binpkg_format=None):
        ...
        if binpkg_format == "xpak":
            binpkg_metadata = corepkg.xpak.tbz2(filename).get_data()
        elif binpkg_format == "gpkg":
            binpkg_metadata = corepkg.gpkg.gpkg(
    ...
    def _pkgindex_entry(self, cpv):
        from corepkg.checksum import perform_multiple_checksums
    ...
    def _new_pkgindex(self):
        return corepkg.getbinpkg.PackageIndex(
    ...
    def dep_bestmatch(self, mydep):
        from corepkg.dbapi.dep_expand import dep_expand
        from corepkg.dep import dep_getkey, match_from_list
        from corepkg.util import writemsg
    ...
    def gettbz2(self, pkgname):
        ...
        success = corepkg.getbinpkg.file_get(url, mydest, fcmd=fcmd)
        if not success:
            try:
                os.unlink(self.getname(pkgname))
            except OSError:
                pass
            raise corepkg.exception.FileNotFound(mydest)
        self.inject(pkgname)
    ...
    def digestCheck(self, pkg):
        ...
        if not ok:
            ...
            if not ok:
                raise corepkg.exception.DigestException((pkg_path,) + tuple(reason))
