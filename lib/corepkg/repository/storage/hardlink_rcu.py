...
from corepkg.repository.storage.interface import (
...
class HardlinkRcuRepoStorage(RepoStorageInterface):
    ...
    async def init_update(self):
        ...
        corepkg.util.ensure_dirs(self._storage_location)

        if self._latest_canonical is not None:
            corepkg.util.ensure_dirs(update_location)
            corepkg.util.apply_stat_permissions(
    ...
    async def commit_update(self):
        ...
        try:
            ...
            snapshots = []
            corepkg.util.ensure_dirs(self._snapshots_dir)
            corepkg.util.apply_stat_permissions(
