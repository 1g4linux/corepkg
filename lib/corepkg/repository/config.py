...
from corepkg import eclass_cache, os
from corepkg._sets.base import WildcardPackageSet
from corepkg.checksum import get_valid_checksum_keys
from corepkg.dep import Atom
from corepkg.const import PORTAGE_BASE_PATH, REPO_NAME_LOC, USER_CONFIG_PATH
...
from corepkg.util.configparser import SafeConfigParser, ConfigParserError, read_configs
from corepkg.util._path import isdir_raise_eaccess
...
_auto_private_overlay_name = "private-overlay"
_auto_private_overlay_relpath = os.path.join(
    "var", "db", "repos", _auto_private_overlay_name
)
...
class RepoConfig:
    ...
    def __init__(self, name, repo_opts, local_config=True):
        ...
        if location is not None and location.strip():
            if os.path.isdir(location) or corepkg._sync_mode:
        ...
        if self.location is not None:
            ...
            if missing:
                ...
                # sync operations to work when the repo
                # is empty (bug #484950).
                if name is not None:
                    self.name = name
                if corepkg._sync_mode:
                    missing = False
        elif name == "DEFAULT":
            missing = False
        ...
        if volatile is not None:
            ...
            try:
                ...
                # we're under /var/db/repos instead.
                    ...
                    self.volatile = True
                # If the owner of the repository isn't root or Corepkg, it's
                # an indication the user may expect to be able to safely make
                # changes in the directory, so default to volatile.
                elif Path(self.location).owner() not in ("root", "corepkg"):
                    self.volatile = True
                else:
                    self.volatile = False
            except Exception:
        ...
        self.corepkg1_profiles = True
        self.corepkg1_profiles_compat = False
    ...
    def iter_pregenerated_caches(self, auxdbkeys, readonly=True, force=False):
        ...
        if not formats:
            if not force:
                return
            # The default egencache format was 'pms' prior to corepkg-2.1.11.32
            # (corepkg versions prior to corepkg-2.1.11.14 will NOT
        ...
        for fmt in formats:
            name = None
            if fmt == "pms":
                from corepkg.cache.metadata import database

                name = "metadata/cache"
            elif fmt == "md5-dict":
                from corepkg.cache.flat_hash import md5_database as database
    ...
    def __repr__(self):
        return (
            "<corepkg.repository.config.RepoConfig(name={!r}, location={!r})>".format(
                self.name,
...
class RepoConfigLoader:
    ...
    def _add_repositories(
        ...
        if overlays:
            ...
            base_priority = 0
            for ov in overlays:
                # Ignore missing directory for the configured main repo so
                # first sync can create it.
    ...
    def __init__(self, paths, settings):
        ...
        default_portdir = os.path.join(
            os.sep, settings["EPREFIX"].lstrip(os.sep), "var/db/repos/bp"
        )
        for repo_location in ("var/db/repos/bp", "var/db/repos/gentoo", "usr/corepkg"):
            candidate = os.path.join(
                os.sep, settings["EPREFIX"].lstrip(os.sep), repo_location
            )
            if candidate in repo_locations:
                default_portdir = candidate
                break
            ...
            if private_overlay is None:
                private_overlay = RepoConfig(
                    auto_private_overlay_name,
                    {
                        ...
                        "sync-type": "git",
                        "sync-uri": private_overlay_sync_uri,
                        "auto-sync": "yes",
                        "priority": "-9999",
                    },
                ...
                if private_overlay.location is None:
                    private_overlay.location = private_overlay_location
                    private_overlay.user_location = private_overlay_location
                if private_overlay.priority is None:
                    private_overlay.priority = -9999
                if private_overlay.auto_sync is None:
            ...
            if repo.location is None:
                ...
                if not corepkg._sync_mode:
                    if not isdir_raise_eaccess(repo.location):
                        ...
                        # Ignore missing directory for configured main repo so
                        # first sync can create it.
                        allow_missing_main_repo = (
                            configured_main_repo is not None
                            and (
                                repo_name == configured_main_repo
                                or repo.name == configured_main_repo
                                or (
                                    repo.aliases is not None
                                    and configured_main_repo in repo.aliases
                                )
                            )
                        )
                        allow_missing_auto_private_overlay = (
                            auto_private_overlay_name is not None
                            and repo_name == auto_private_overlay_name
                        )
                        if not (
                            allow_missing_main_repo or allow_missing_auto_private_overlay
                        ):
                            del prepos[repo_name]
                            continue
        ...
        if main_repo is None or main_repo not in prepos:
            ...
            if main_repo is not None:
                ...
                if portdir and not corepkg._sync_mode:
                    writemsg(
        ...
        if auto_private_overlay_name is not None:
            private_overlay = prepos.get(auto_private_overlay_name)
            if (
                private_overlay is not None
                and main_repo is not None
                and main_repo in prepos
                and private_overlay.name != prepos[main_repo].name
            ):
                private_overlay.masters = (main_repo,)

        main_repo_sync_uri = settings.get("PORTAGE_MAIN_REPO_SYNC_URI", "").strip()
        if main_repo_sync_uri:
            if main_repo is not None and main_repo in prepos:
                prepos[main_repo].sync_uri = main_repo_sync_uri
            elif not corepkg._sync_mode:
                writemsg(
                    _(
                        "!!! PORTAGE_MAIN_REPO_SYNC_URI is set but main-repo is not configured.\n"
                    ),
                    noiselevel=-1,
                )
        ...
        if portdir_sync and main_repo is not None:
            writemsg(
                _(
                    ...
                    " in /etc/corepkg/repos.conf/bp.conf\n"
                ),
                noiselevel=-1,
            )
        ...
        for repo_name, repo in prepos.items():
            ...
            if (
                ...
                and not corepkg._sync_mode
            ):
                # TODO: Delete masters code in lib/corepkg/tests/resolver/ResolverPlayground.py when deleting this warning.
                writemsg_level(
                    "!!! %s\n"
        ...
        for name, r in self.prepos.items():
            if name != "DEFAULT":
                if r.location is None:
                    ...
                    if not isdir_raise_eaccess(r.location) and not corepkg._sync_mode:
...
def parse_layout_conf(repo_location, repo_name=None):
    ...
    if not cache_formats:
        ...
        # This behavior was deployed in corepkg-2.1.11.14, so that the
        # default egencache format could eventually be changed to md5-dict
        # in corepkg-2.1.11.32. WARNING: Versions prior to corepkg-2.1.11.14
        ...
        warnings.warn(
            (
                _(
                    ...
                    "'manifest-hashes'. Corepkg will default it to match "
            ...
            warnings.warn(
                (
                    _(
                        ...
                        "this corepkg version. You will have to upgrade "
                        "corepkg if you want to generate valid manifests for "
            ...
            warnings.warn(
                (
                    _(
                        ...
                        "'%(layout_filename)s; please upgrade corepkg."
                    )
                    % dict(
                        repo_name=repo_name or "unspecified",
                        layout_filename=layout_filename,
    ...
    try:
        ...
        elif not corepkg.eapi_is_valid_for_runtime(eapi):
            warnings.warn(
                (
                    _(
                        "Repository named '%(repo_name)s' has "
                    ...
                    % dict(
                        repo_name=repo_name or "unspecified",
                        error=corepkg.unsupported_eapi_message(eapi),
                        layout_filename=layout_filename,
                    )
                ),
                SyntaxWarning,
