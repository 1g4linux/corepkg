...
from corepkg.progress import ProgressBar

# from corepkg.emaint.defaults import DEFAULT_OPTIONS
from corepkg.util import writemsg, writemsg_level
from corepkg.output import create_color_func

good = create_color_func("GOOD")
bad = create_color_func("BAD")
warn = create_color_func("WARN")
from corepkg.package.ebuild.doebuild import _check_temp_dir
from corepkg.metadata import action_metadata
from corepkg.util.hooks import get_hooks_from_dir
from corepkg.util._async.AsyncFunction import AsyncFunction
...
class TaskHandler:
    ...
    def __init__(self, show_progress_bar=True, verbose=True, callback=None):
        ...
        self.progress_bar = ProgressBar(
            self.isatty, title="Corepkg-Sync", max_desc_length=27
        )
...
class SyncManager:
    ...
    def module_controller(self):
        ...
        return corepkg.sync.module_controller
    ...
    def __getattr__(self, name):
        if name == "async":
            warnings.warn(
                "corepkg.sync.controller.SyncManager.async "
    ...
    def perform_post_sync_hook(self, reponame, dosyncuri="", repolocation=""):
        ...
        for filepath in _hooks:
            ...
            if reponame:
                ...
                retval = corepkg.process.spawn([filepath], env=self.settings.environ())
            if retval != os.EX_OK:
                writemsg_level(
    ...
    def pre_sync(self, repo):
        ...
        if st is None:
            ...
            corepkg.util.ensure_dirs(repo.location, **perms)
            st = os.stat(repo.location)
