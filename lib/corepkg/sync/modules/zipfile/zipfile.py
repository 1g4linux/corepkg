...
import corepkg
from corepkg.util import writemsg_level, writemsg_stdout
from corepkg.sync.syncbase import SyncBase
...
class ZipFile(SyncBase):
    ...
    def __init__(self):
        SyncBase.__init__(self, "emerge", corepkg.const.PORTAGE_PACKAGE_ATOM)

    def retrieve_head(self, **kwargs):
        """Get information about the checksum of the unpacked archive"""
        if kwargs:
            self._kwargs(kwargs)
        info = corepkg.grabdict(os.path.join(self.repo.location, ".info"))
        if "etag" in info:
