...
import corepkg
from corepkg.repository.storage.interface import RepoStorageException
from corepkg.util import writemsg_level
from corepkg.util._eventloop.global_event_loop import global_event_loop
from corepkg.util.backoff import RandomExponentialBackoff
from corepkg.util.futures._sync_decorator import _sync_methods
from corepkg.util.futures.retry import retry
...
class SyncBase:
    ...
    def __init__(self, bin_command, bin_pkg):
        ...
        if bin_command:
            self.bin_command = corepkg.process.find_binary(bin_command)
    ...
    def _select_storage_module(self):
        ...
        if self.repo.sync_rcu:
            mod_name = "corepkg.repository.storage.hardlink_rcu.HardlinkRcuRepoStorage"
        elif self.repo.sync_allow_hardlinks:
            mod_name = "corepkg.repository.storage.hardlink_quarantine.HardlinkQuarantineRepoStorage"
        else:
            mod_name = "corepkg.repository.storage.inplace.InplaceRepoStorage"
        return mod_name
    ...
    def repo_storage(self):
        ...
        if self._repo_storage is None:
            storage_cls = corepkg.load_mod(self._select_storage_module())
