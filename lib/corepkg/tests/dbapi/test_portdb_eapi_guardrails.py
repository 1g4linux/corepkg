...
class PortdbEapiGuardrailsTestCase(TestCase):
    ...
    def test_fetch_map_normalizes_aux_get_eapi(self):
        ebuilds = {
            "dev-libs/A-1": {
                "EAPI": "8",
                "SRC_URI": "https://example.invalid/distfiles/A-1.tar.xz",
        ...
        try:
            portdb = playground.trees[playground.eroot]["porttree"].dbapi
            original_async_aux_get = portdb.async_aux_get

            async def fake_async_aux_get(*args, **kwargs):
                return (" 8 ", "https://example.invalid/distfiles/A-1.tar.xz")

            portdb.async_aux_get = fake_async_aux_get
            try:
                fetch_map = portdb.getFetchMap("dev-libs/A-1")
            finally:
                portdb.async_aux_get = original_async_aux_get
