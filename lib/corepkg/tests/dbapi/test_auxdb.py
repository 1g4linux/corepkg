...
class AuxdbTestCase(TestCase):
    def test_anydbm(self):
        try:
            from corepkg.cache.anydbm import database
        except ImportError:
            self.skipTest("dbm import failed")
        self._test_mod("corepkg.cache.anydbm.database", multiproc=False, picklable=True)

    def test_flat_hash_md5(self):
        self._test_mod("corepkg.cache.flat_hash.md5_database")

    def test_volatile(self):
        self._test_mod("corepkg.cache.volatile.database", multiproc=False)
    ...
    def test_sqite(self):
        ...
        self._test_mod("corepkg.cache.sqlite.database", picklable=True)
