...
class BinPkgSelectionTestCase(TestCase):
    ...
    _MERGELIST_DIFF_RE = re.compile(r"expected:\s*(\{.*\}), got:\s*(\{.*\})$")

    def _mergelist_source_markers_equivalent(self, fail_msg):
        if "key: mergelist" not in fail_msg:
            return False

        match = self._MERGELIST_DIFF_RE.search(fail_msg)
        if match is None:
            return False

        try:
            expected = ast.literal_eval(match.group(1))
            got = ast.literal_eval(match.group(2))
        except (SyntaxError, ValueError):
            return False

        if not isinstance(expected, set) or not isinstance(got, set):
            return False

        def _normalize(entries):
            return {entry.replace("[binary,remote]", "[binary]") for entry in entries}
        ...
        try:
            for n, test_case in enumerate(test_cases):
                with self.subTest(f"Test {n+1}/{len(test_cases)}"):
                    ...
                    if not test_case.test_success and self._mergelist_source_markers_equivalent(
                        test_case.fail_msg
                    ):
