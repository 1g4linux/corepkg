...
from corepkg import os, _encodings
...
class ProfileDefaultEAPITestCase(TestCase):
    def testProfileFormatsUnsupportedDefaultEapiWarning(self):
        repo_configs = {
            "test_repo": {
                "layout.conf": (
                    "profile-formats = profile-default-eapi",
                    "profile_eapi_when_unspecified = 9",
                ),
            }
        }
        ...
        self.assertTrue(
            any(
                "unsupported EAPI: '9' (expected EAPI='8')" in str(w.message)
    ...
    def _testProfileDefaultEAPI(self, repo_configs=None):
