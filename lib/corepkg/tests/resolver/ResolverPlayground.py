...
from corepkg.dep import Atom, _repo_separator
from corepkg.dbapi.bintree import binarytree
from corepkg._sets import load_default_config
from corepkg._sets.base import InternalPackageSet
from corepkg.tests import cnf_path
from corepkg.util import ensure_dirs, normalize_path
from corepkg.versions import catsplit
from corepkg.exception import InvalidBinaryPackageFormat
...
"""
    ...
    def __init__(
        ...
        if True:
            ...
            ensure_dirs(eubin)
            os.symlink(corepkg._python_interpreter, os.path.join(eubin, "python"))
            for x in self.corepkg_bin:
                os.symlink(os.path.join(PORTAGE_BIN_PATH, x), os.path.join(eubin, x))

            eusbin = os.path.join(self.eprefix, "usr", "sbin")
            ensure_dirs(eusbin)
            for x in self.corepkg_sbin:
            ...
            for path in orig_path.split(":"):
                if path and not fnmatch.fnmatch(path, "*/corepkg/*/ebuild-helpers*"):
            ...
            try:
                ...
                for x in essential_binaries:
                    path = find_binary(x)
                    if path is None:
                        raise corepkg.exception.CommandNotFound(x)
        ...
        self._orig_eprefix = corepkg.const.EPREFIX
        corepkg.const.EPREFIX = self.eprefix.rstrip(os.sep)
        ...
        self.distdir = os.path.join(self.eroot, "var", "corepkg", "distfiles")
    ...
    def _create_ebuilds(self, ebuilds):
        for cpv in ebuilds:
            ...
            unknown_keys = set(metadata).difference(corepkg.dbapi.dbapi._known_keys)
            if unknown_keys:
                raise ValueError(
    ...
    def _create_ebuild_manifests(self, ebuilds):
        for repo_name in self._repositories:
            ...
            egencache_cmd = [
                ...
                f"--jobs={corepkg.util.cpuinfo.get_cpu_count()}",
            ]
    ...
    def _create_binpkgs(self, repo_dir, binpkgs):
        ...
        for cpv, metadata in items:
            ...
            if binpkg_format == "xpak":
                t = corepkg.xpak.tbz2(binpkg_path)
                t.recompose_mem(corepkg.xpak.xpak_mem(metadata))
            elif binpkg_format == "gpkg":
                t = corepkg.gpkg.gpkg(self.settings, a.cpv, binpkg_path)
    ...
    def _create_installed(self, installed):
        for cpv in installed:
            ...
            with open(ebuild_path, "w") as f:
                f.write(f"EAPI=\"{metadata.pop('EAPI', '8')}\"\n")
                for k, v in metadata.items():
                    f.write(f'{k}="{v}"\n')
    ...
    def _create_profile(
        ...
        make_conf = {
            "ACCEPT_KEYWORDS": "x86",
            "BINPKG_GPG_SIGNING_BASE_COMMAND": f"flock {gpg_test_path}/corepkg-binpkg-gpg.lock /usr/bin/gpg --sign --armor --yes --pinentry-mode loopback --passphrase GentooTest [PORTAGE_CONFIG]",
            ...
            "PKGDIR": self.pkgdir,
            "PORTAGE_INST_GID": str(corepkg.data.corepkg_gid),
            "PORTAGE_INST_UID": str(corepkg.data.corepkg_uid),
        ...
        if not corepkg.process.sandbox_capable or os.environ.get("SANDBOX_ON") == "1":
        ...
        # Create /usr/share/corepkg/config/make.globals
        ...
        # Create /usr/share/corepkg/config/sets/corepkg.conf
        default_sets_conf_dir = os.path.join(
            self.eroot, "usr/share/corepkg/config/sets"
        )
        ...
        provided_sets_corepkg_conf = os.path.join(str(cnf_path), "sets", "corepkg.conf")
        os.symlink(
            provided_sets_corepkg_conf,
            os.path.join(default_sets_conf_dir, "corepkg.conf"),
        )
    ...
    def _create_world(self, world, world_sets):
        # Create /var/lib/corepkg/world
        var_lib_corepkg = os.path.join(self.eroot, "var", "lib", "corepkg")
        os.makedirs(var_lib_corepkg)

        world_file = os.path.join(var_lib_corepkg, "world")
        world_set_file = os.path.join(var_lib_corepkg, "world_sets")
    ...
    def run(self, atoms, options={}, action=None):
        ...
        global_noiselimit = corepkg.util.noiselimit
        ...
        try:
            ...
            corepkg.util.noiselimit = global_noiselimit
    ...
    def cleanup(self):
        ...
        if hasattr(self, "_orig_eprefix"):
            corepkg.const.EPREFIX = self._orig_eprefix
