...
def corepkg_func(func, args, exit_status=0):
...
def create_corepkg_wrapper(f):
    def derived_func(*args):
        newargs = list(args)
        newargs.insert(0, f)
        return corepkg_func(*newargs)
...
for f in os.listdir(os.path.join(bindir, "ebuild-helpers")):
    if (
        ...
        globals()[f] = create_corepkg_wrapper(os.path.join(bindir, "ebuild-helpers", f))
