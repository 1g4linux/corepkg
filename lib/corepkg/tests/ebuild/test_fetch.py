...
import corepkg
from corepkg import shutil, os
from corepkg.checksum import checksum_str
from corepkg.const import BASH_BINARY, MANIFEST2_HASH_DEFAULTS, PORTAGE_PYM_PATH
from corepkg.tests import TestCase
from corepkg.tests.resolver.ResolverPlayground import ResolverPlayground
from corepkg.tests.util.test_socks5 import AsyncHTTPServer
...
from corepkg.package.ebuild.fetch import (
    ContentHashLayout,
    DistfileName,
    _get_uri_fetchcommand,
    _get_oneg4_gitlab_header,
    _inject_http_header,
    _iter_uri_fetchcommand_vars,
    _download_suffix,
    fetch,
    FilenameHashLayout,
    FlatLayout,
...
from corepkg._emirrordist.Config import Config as EmirrordistConfig
from _emerge.EbuildFetcher import EbuildFetcher
from _emerge.Package import Package


class EbuildFetchTestCase(TestCase):
    def test_oneg4_gitlab_header(self):
        ...
        try:
            ...
            self.assertIsNone(
                _get_oneg4_gitlab_header("ftp://gitlab.example.com/group/proj")
            )

            # Restrict token use to a specific owner path.
    ...
    def test_inject_http_header(self):
        header = "Authorization: Bearer token123"

        self.assertEqual(
            _inject_http_header(
                ["/usr/bin/wget", "-O", "/tmp/out", "https://example.com/src.tar.xz"],
                header,
            ),
            [
                "/usr/bin/wget",
                ...
                "-O",
                "/tmp/out",
                "https://example.com/src.tar.xz",
            ],
        )

        self.assertEqual(
            _inject_http_header(
                ["curl", "-fL", "-o", "/tmp/out", "https://example.com/src.tar.xz"],
                header,
            ),
            [
                "curl",
        ...
        # Existing header options should be preserved as-is.
        wget_args = [
            "wget",
        ...
        self.assertEqual(_inject_http_header(wget_args, header), wget_args)
        ...
        self.assertEqual(_inject_http_header(curl_args, header), curl_args)

        # Unsupported fetchers are unchanged.
        bash_args = ["bash", "-c", "wget \"$1\"", "bash", "https://example.com/src.tar.xz"]
        self.assertEqual(_inject_http_header(bash_args, header), bash_args)

    def test_uri_fetchcommand_lookup(self):
        uri = "https://gitlab.example.com:8443/group/proj/-/archive/abc123/proj.tar.gz"
        self.assertEqual(
            tuple(_iter_uri_fetchcommand_vars("FETCHCOMMAND", "https", uri)),
            (
                "FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM_8443",
                "FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM",
                "FETCHCOMMAND_HTTPS",
                "FETCHCOMMAND",
            ),
        )
        ...
        settings = {
            ...
            "FETCHCOMMAND_HTTPS": "protocol",
            "FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM": "host",
            "FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM_8443": "host_port",
        }
        self.assertEqual(
            _get_uri_fetchcommand(settings, "FETCHCOMMAND", "https", uri),
            ("FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM_8443", "host_port"),
        )

        del settings["FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM_8443"]
        self.assertEqual(
            _get_uri_fetchcommand(settings, "FETCHCOMMAND", "https", uri),
            ("FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM", "host"),
        )

        del settings["FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM"]
        self.assertEqual(
            _get_uri_fetchcommand(settings, "FETCHCOMMAND", "https", uri),
            ("FETCHCOMMAND_HTTPS", "protocol"),
        )

        del settings["FETCHCOMMAND_HTTPS"]
        self.assertEqual(
            _get_uri_fetchcommand(settings, "FETCHCOMMAND", "https", uri),
            ("FETCHCOMMAND", "default"),
        )
        ...
        self.assertEqual(
            tuple(_iter_uri_fetchcommand_vars("FETCHCOMMAND", "", "/tmp/foo.tar.gz")),
            ("FETCHCOMMAND",),
        )

        # Invalid URI ports should not raise and should simply skip
        # host+port-specific lookup.
        bad_port_uri = "https://gitlab.example.com:bad/group/proj.tar.gz"
        self.assertEqual(
            tuple(_iter_uri_fetchcommand_vars("FETCHCOMMAND", "https", bad_port_uri)),
            (
                "FETCHCOMMAND_HTTPS_GITLAB_EXAMPLE_COM",
                "FETCHCOMMAND_HTTPS",
                "FETCHCOMMAND",
    ...
    def _pre_exec_interrupt_patch(pw):
        corepkg.package.ebuild.fetch._async_spawn_fetch_pre_wait = functools.partial(
            EbuildFetchTestCase._fetch_pre_wait,
            pw,
        )
        corepkg.package.ebuild.fetch._async_spawn_fetch_post_terminate = (
            functools.partial(
    ...
    def _testEbuildFetch(
        ...
        fetch_bin = corepkg.process.find_binary(fetchcommand[0])
        ...
        resume_bin = corepkg.process.find_binary(resumecommand[0])
