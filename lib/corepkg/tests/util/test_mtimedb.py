...
import corepkg
from corepkg.data import corepkg_gid, uid
from corepkg.util.mtimedb import MtimeDB, _MTIMEDBKEYS
...
class MtimeDBTestCase(TestCase):
    ...
    def test_instances_created_with_only_expected_keys(self):
        ...
        for contents in all_fixtures:
            with patch("corepkg.util.mtimedb.open", mock_open(read_data=contents)):
                mtimedb = MtimeDB("/path/to/mtimedb")
            self.assertLessEqual(set(mtimedb.keys()), _MTIMEDBKEYS)

    def test_default_values(self):
        with patch("corepkg.util.mtimedb.open", mock_open(read_data=_EMPTY_FILE)):
    ...
    def test_instances_keep_a_deepcopy_of_clean_data(self):
        with patch(
            "corepkg.util.mtimedb.open", mock_open(read_data=_ONE_RESUME_LIST_JSON)
        ):
            mtimedb = MtimeDB("/some/path/mtimedb")
    ...
    @patch("corepkg.util.mtimedb.MtimeDB._MtimeDB__write_to_disk")
    ...
    @patch("corepkg.util.mtimedb.apply_secpass_permissions")
    @patch("corepkg.util.mtimedb.atomic_ofstream")
    def test_write_to_disk(self, matomic_ofstream, mapply_perms):
        with patch(
            ...
            mtimedb = MtimeDB("/some/path/mtimedb")
        d = {"z": "zome", "a": "AAA"}
        encoding = corepkg._encodings["repo.content"]
        ...
        self.assertEqual(d["version"], str(corepkg.VERSION))
        ...
        mapply_perms.assert_called_once_with(
            mtimedb.filename, uid=uid, gid=corepkg_gid, mode=0o644
        )
