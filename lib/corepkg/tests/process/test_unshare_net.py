...
class UnshareNetTestCase(TestCase):
    def setUp(self):
        ...
        """
        super().setUp()
        self.ABILITY_TO_UNSHARE = corepkg.process._unshare_validate(CLONE_NEWNET)

    @pytest.mark.skipif(
        corepkg.process.find_binary("ping") is None, reason="ping not found"
    )
    ...
    def testUnshareNet(self):
        ...
        env = os.environ.copy()
        env["IPV6"] = "1" if corepkg.process.has_ipv6() else ""
        self.assertEqual(
