...
from corepkg.data import corepkg_gid, corepkg_uid, secpass, uid, userpriv_groups
from corepkg.dbapi.porttree import _parse_uri_map
from corepkg.dep import (
    Atom,
    check_required_use,
...
from corepkg.dep.libc import find_libc_deps
from corepkg.eapi import (
    eapi_exports_KV,
...
from corepkg.elog import elog_process
from corepkg.elog.messages import eerror, eqawarn
from corepkg.exception import (
    DigestException,
    FileNotFound,
    IncorrectParameter,
    InvalidData,
...
from corepkg.output import colormap
from corepkg.package.ebuild.prepare_build_dirs import prepare_build_dirs
from corepkg.process import find_binary
from corepkg.util import (
...
from corepkg.util.cpuinfo import get_cpu_count, makeopts_to_job_count
from corepkg.util.lafilefixer import rewrite_lafile
...
def _spawn_phase(
    ...
    **kwargs,
    ...
    from corepkg.util._async.SchedulerInterface import SchedulerInterface
...
def _doebuild_path(settings, eapi=None):
    ...
    # when corepkg is reinstalling itself.
    corepkg_bin_path = [settings["PORTAGE_BIN_PATH"]]
    if corepkg_bin_path[0] != corepkg.const.PORTAGE_BIN_PATH:
        # Add a fallback path for restarting failed builds (bug 547086)
        corepkg_bin_path.append(corepkg.const.PORTAGE_BIN_PATH)

    path = [x for x in settings.get("__PORTAGE_TEST_PATH_OVERRIDE", "").split(":") if x]

    if "xattr" in settings.features:
        for x in corepkg_bin_path:
...
def doebuild_environment(
    ...
    """
    from corepkg.util.ExtractKernelVersion import ExtractKernelVersion
    ...
    if mydo == "depend":
        ...
        # call it here (corepkg code always calls setcpv properly,
    ...
    # Set requested Python interpreter for Corepkg helpers.
    mysettings["PORTAGE_PYTHON"] = corepkg._python_interpreter
    ...
    # $PORTAGE_TMPDIR/corepkg, since it's common for /tmp and /var/tmp
    # to be mounted with the "noexec" option (see bug #346899).
    mysettings["BUILD_PREFIX"] = mysettings["PORTAGE_TMPDIR"] + "/corepkg"
    ...
    if "COLUMNS" not in mysettings:
        ...
        if columns is None:
            rows, columns = corepkg.output.get_term_size()
            if columns < 1:
    ...
    if mydo != "depend":
        ...
        if binpkg_format not in corepkg.const.SUPPORTED_GENTOO_BINPKG_FORMATS:
            writemsg(
...
def doebuild(
    ...
    returnpid=False,
    returnproc=False,
) -> Union[int, corepkg.process.MultiprocessingProcess, list[int]]:
    """
    ...
    @type _unused: String
    @param settings: Corepkg Configuration
    @type settings: instance of corepkg.config
    ...
    from corepkg.package.ebuild.digestcheck import digestcheck
    from corepkg.package.ebuild.digestgen import digestgen
    from corepkg.package.ebuild.prepare_build_dirs import _prepare_fake_distdir
    from corepkg.package.ebuild._spawn_nofetch import spawn_nofetch
    ...
    if _unused is not DeprecationWarning:
        warnings.warn(
            ...
            "corepkg.doebuild() is deprecated. Instead "
    ...
    if mydbapi is None:
        mydbapi = corepkg.db[myroot][tree].dbapi

    if vartree is None and mydo in ("merge", "qmerge", "unmerge"):
        vartree = corepkg.db[myroot]["vartree"]
    ...
    if (returnproc or returnpid) and mydo != "depend":
        ...
        warnings.warn(
            "The corepkg.doebuild() returnpid parameter is deprecated and replaced by returnproc",
            UserWarning,
            stacklevel=1,
        )
    ...
    if (
        ...
        and not corepkg._doebuild_manifest_exempt_depend
        and not (
    ...
    try:
        if mydo in ("digest", "manifest", "help"):
            ...
            corepkg._doebuild_manifest_exempt_depend += 1
        ...
        # user isn't in the corepkg group.
        ...
        if eapi_exports_replace_vars(mysettings["EAPI"]) and (
            ...
            if not vartree:
                ...
                cpv_slot = f"{cpv.cp}{corepkg.dep._slot_separator}{cpv.slot}"
                mysettings["REPLACING_VERSIONS"] = " ".join(
                    {
                        corepkg.versions.cpv_getversion(match)
        ...
        if regular_actionmap_phase:
            ...
            if mydo == "package":
                ...
                if hasattr(corepkg, "db"):
                    bintree = corepkg.db[mysettings["EROOT"]]["bintree"]
                    ...
                    with tempfile.NamedTemporaryFile(
                        ...
                        suffix=".tbz2." + str(corepkg.getpid()),
                    ...
                    corepkg.util.ensure_dirs(parent_dir)
...
def _fetch_subprocess(fetchme, mysettings, listonly, dist_digests, fetchonly):
    from corepkg.package.ebuild.fetch import (
...
def _check_temp_dir(settings):
    ...
    # for those people.
    checkdir = first_existing(os.path.join(settings["PORTAGE_TMPDIR"], "corepkg"))
            ...
            if not os.access(fd.name, os.X_OK):
                writemsg(
                    _(
                        ...
                        "Please make sure that corepkg can execute files in this directory.\n"
                    )
                    % checkdir,
                    noiselevel=-1,
                )
...
def _spawn_actionmap(settings):
    ...
    if not corepkg.process.sandbox_capable:
        nosandbox = True
    ...
    ebuild_sh_binary = os.path.join(
        corepkg_bin_path, os.path.basename(EBUILD_SH_BINARY)
    )
    misc_sh_binary = os.path.join(corepkg_bin_path, os.path.basename(MISC_SH_BINARY))
...
def _validate_deps(mysettings, myroot, mydo, mydbapi):
    ...
    if msgs:
        corepkg.util.writemsg_level(
            _("Error(s) in metadata for '%s':\n") % (mysettings.mycpv,),
            level=logging.ERROR,
            noiselevel=-1,
        )
        for x in msgs:
            corepkg.util.writemsg_level(x, level=logging.ERROR, noiselevel=-1)
...
def spawn(
    ...
    **keywords,
    ...
    """
    Spawn a subprocess with extra corepkg-specific options.
    Options include:
    ...
    Reduced Privileges: Drops privileges such that the process runs as corepkg:corepkg
    instead of as root.

    Notes: os.system cannot be used because it messes with signal handling.  Instead we
    use the corepkg.process spawn* family of functions.

    This function waits for the process to terminate.

    @param mystring: Command to run
    @type mystring: String
    @param mysettings: Either a Dict of Key,Value pairs or an instance of corepkg.config
    @type mysettings: Dictionary or config instance
    @param debug: Ignored
    @type debug: Boolean
    @param free: Enable sandboxing for this process
    @type free: Boolean
    @param droppriv: Drop to corepkg:corepkg when running this command
    ...
    """
    from corepkg.package.ebuild.config import check_config_instance
    from corepkg.util._async.SchedulerInterface import SchedulerInterface
    ...
    fakeroot = fakeroot and uid != 0 and corepkg.process.fakeroot_capable
    corepkg_build_uid = os.getuid()
    corepkg_build_gid = os.getgid()
    logname = None
    if uid == 0 and corepkg_uid and corepkg_gid and hasattr(os, "setgroups"):
        if droppriv:
            logname = corepkg.data._corepkg_username
            keywords.update(
                {
                    "uid": corepkg_uid,
                    "gid": corepkg_gid,
            ...
            if stdout_fd is not None:
                try:
                    ...
                    if subprocess_tty != parent_tty:
                        _os.chown(subprocess_tty, int(corepkg_uid), int(corepkg_gid))
        ...
        if (
            ...
            corepkg_build_uid = int(corepkg_uid)
            corepkg_build_gid = int(corepkg_gid)

    if "PORTAGE_BUILD_USER" not in mysettings:
        user = None
        try:
            user = pwd.getpwuid(corepkg_build_uid).pw_name
        except KeyError:
            if corepkg_build_uid == 0:
                user = "root"
            elif corepkg_build_uid == corepkg_uid:
                user = corepkg.data._corepkg_username
        if user is not None:
    ...
    if "PORTAGE_BUILD_GROUP" not in mysettings:
        group = None
        try:
            group = grp.getgrgid(corepkg_build_gid).gr_name
        except KeyError:
            if corepkg_build_gid == 0:
                group = "root"
            elif corepkg_build_gid == corepkg_gid:
                group = corepkg.data._corepkg_grpname
    ...
    if not free and not (fakeroot or corepkg.process.sandbox_capable):
        free = True
    ...
    if free or "SANDBOX_ACTIVE" in os.environ:
        ...
        spawn_func = corepkg.process.spawn_bash
    elif fakeroot:
        keywords["opt_name"] += " fakeroot"
        keywords["fakeroot_state"] = os.path.join(mysettings["T"], "fakeroot.state")
        spawn_func = corepkg.process.spawn_fakeroot
    else:
        keywords["opt_name"] += " sandbox"
        spawn_func = corepkg.process.spawn_sandbox
    ...
    if unexported_env_vars:
        ...
        if is_pms_ebuild_phase or is_ebuild_phase_with_t:  # case A
            ...
            ebuild_extra_source_path = os.path.join(
                t, f".corepkg-ebuild-extra-source-{phase}"
            )
        else:  # case B and C
            global _emerge_tmpdir
            if _emerge_tmpdir is None:
                _emerge_tmpdir = tempfile.mkdtemp(
                    prefix=f"corepkg-tmpdir-{corepkg.getpid()}-"
                )
                os.chmod(_emerge_tmpdir, 0o1775)
                os.chown(_emerge_tmpdir, -1, int(corepkg_build_gid))
                corepkg.process.atexit_register(shutil.rmtree, _emerge_tmpdir)
            ebuild_extra_source_fd, ebuild_extra_source_path = tempfile.mkstemp(
                prefix=f"corepkg-ebuild-extra-source-{phase}-",
                dir=_emerge_tmpdir,
            )
            try:
...
def _check_build_log(mysettings, out=None):
    ...
    #
    # Configuration:
    #  Automake:                   ${SHELL} /var/tmp/corepkg/dev-libs/yaz-3.0.47/work/yaz-3.0.47/config/missing --run automake-1.10
    ...
    try:
        ...
        _eerror(
            [
                f"corepkg encountered a zlib error: '{e}'",
...
def _post_src_install_write_metadata(settings):
    ...
    """
    from corepkg.dep._slot_operator import evaluate_slot_operator_equal_deps
...
def _post_src_install_uid_fix(mysettings, out):
    """
    Files in $D with user and group bits that match the "corepkg"
    ...
    necessary.
    """
    from corepkg.util._desktop_entry import validate_desktop_entry
    ...
    desktop_file_validate = (
        corepkg.process.find_binary("desktop-file-validate") is not None
    )
        ...
        for parent, dirs, files in os.walk(destdir):
            ...
            for fname in chain(dirs, files):
                ...
                if fixlafiles and fname.endswith(".la") and os.path.isfile(fpath):
                    ...
                    except corepkg.exception.InvalidData as e:
                        needs_update = False
                ...
                if stat.S_ISREG(mystat.st_mode) and mystat.st_ino not in counted_inodes:
                    ...
                    size += mystat.st_size
                if mystat.st_uid != corepkg_uid and mystat.st_gid != corepkg_gid:
                    continue
                myuid = -1
                mygid = -1
                if mystat.st_uid == corepkg_uid:
                    myuid = inst_uid
                if mystat.st_gid == corepkg_gid:
                    mygid = inst_gid
                apply_secpass_permissions(
...
def _post_src_install_soname_symlinks(mysettings, out):
    ...
    operation.
    """
    from corepkg.dep.soname.multilib_category import compute_multilib_category
    from corepkg.util._dyn_libs.NeededEntry import NeededEntry
    from corepkg.util._dyn_libs.soname_deps import SonameDepsProcessor
    from corepkg.util.elf.header import ELFHeader
    ...
    libpaths = set(corepkg.util.getlibpaths(mysettings["ROOT"], env=mysettings))
    libpath_inodes = set()
    ...
    needed_file = corepkg.util.atomic_ofstream(
...
def _prepare_self_update(settings):
    """
    Call this when corepkg is updating itself, in order to create
    ...
    if corepkg._bin_path != corepkg.const.PORTAGE_BIN_PATH:
        return

    # Make the temp directory inside $PORTAGE_TMPDIR/corepkg, since
    # it's common for /tmp and /var/tmp to be mounted with the
    # "noexec" option (see bug #346899).
    build_prefix = os.path.join(settings["PORTAGE_TMPDIR"], "corepkg")
    corepkg.util.ensure_dirs(build_prefix)
    base_path_tmp = tempfile.mkdtemp("", "._corepkg_reinstall_.", build_prefix)
    corepkg.process.atexit_register(shutil.rmtree, base_path_tmp)

    orig_bin_path = corepkg._bin_path
    corepkg._bin_path = os.path.join(base_path_tmp, "bin")
    shutil.copytree(orig_bin_path, corepkg._bin_path, symlinks=True)

    orig_pym_path = corepkg._pym_path
    corepkg._pym_path = os.path.join(base_path_tmp, "lib")
    os.mkdir(corepkg._pym_path)
    for pmod in PORTAGE_PYM_PACKAGES:
        shutil.copytree(
            os.path.join(orig_pym_path, pmod),
            os.path.join(corepkg._pym_path, pmod),
            symlinks=True,
        )

    for dir_path in (base_path_tmp, corepkg._bin_path, corepkg._pym_path):
    ...
    sys.path.insert(0, corepkg._pym_path)
...
def _handle_self_update(settings, vardb):
    ...
    if settings["ROOT"] != "/":
        return False
    if not corepkg.dep.match_from_list(
        corepkg.const.PORTAGE_PACKAGE_ATOM, [settings.mycpv]
    ):
        return False
