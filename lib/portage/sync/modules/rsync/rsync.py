...
from corepkg.const import VCS_DIRS, TIMESTAMP_FORMAT, RSYNC_PACKAGE_ATOM
from corepkg.output import create_color_func, yellow, blue, bold
from corepkg.process import has_ipv6
from corepkg.sync.getaddrinfo_validate import getaddrinfo_validate
from corepkg.sync.syncbase import NewBase
from corepkg.util import writemsg, writemsg_level, writemsg_stdout
...
class RsyncSync(NewBase):
    ...
    def update(self):
        ...
        quiet = "--quiet" in opts
        out = corepkg.output.EOutput(quiet=quiet)
        ...
        try:
            ...
            content = corepkg.util.grabfile(self.servertimestampfile)
            timestamp = 0
            if content:
                try:
    ...
    def _process_exitcode(self, exitcode, syncuri, out, maxretries):
        if exitcode == 0:
            ...
            if exitcode == 1:
                ...
                msg.append(
                    "trying, or use an alternate sync backend (for example git or zipfile)"
                )
                msg.append(
                    "if rsync is blocked by firewall or other restrictions. This should be a"
                )
                msg.append(
    ...
    def _validate_rsync_opts(self, rsync_opts, syncuri):
        ...
        corepkg.writemsg("Using PORTAGE_RSYNC_OPTS instead of hardcoded defaults\n", 1)
        ...
        if syncuri.rstrip("/").endswith(".gentoo.org/gentoo-corepkg"):
    ...
    def _do_rsync(self, syncuri, timestamp, opts):
        ...
        # user. We assume that ${PORTAGE_TMPDIR}/corepkg will satisfy this
        # requirement, since that's not necessarily true for the
        # default directory used by the tempfile module.
        if self.usersync_uid is not None:
            tmpdir = os.path.join(self.settings["PORTAGE_TMPDIR"], "corepkg")
            ensure_dirs_kwargs = {}
            if corepkg.secpass >= 1:
                ensure_dirs_kwargs["gid"] = corepkg.corepkg_gid
        ...
        try:
            ...
            try:
                ...
                content = corepkg.grabfile(tmpservertimestampfile)
            finally:
                try:
        ...
        if exitcode == os.EX_OK:
            if (servertimestamp != 0) and (servertimestamp == timestamp):
                ...
                exitcode = None
                try:
                    exitcode = corepkg.process.spawn(command, **self.spawn_kwargs)
                finally:
                    if exitcode is None:
                        # interrupted
