...
from corepkg.repository.config import RepoConfig
...
def get_repo_revision_history(
    ...
    for repo in repos or ():
        ...
        if repo.sync_type:
            try:
                sync_mod = corepkg.sync.module_controller.get_class(repo.sync_type)
...
def _maybe_update_revisions(eroot, items):
    repo_revisions_file = os.path.join(eroot, corepkg.const.REPO_REVISIONS)
    ...
    try:
        ...
        if modified and os.access(
            ...
            f = corepkg.util.atomic_ofstream(repo_revisions_file)
