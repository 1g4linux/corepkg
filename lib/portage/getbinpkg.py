# getbinpkg.py -- Corepkg binary-package helper functions
...
def make_metadata_dict(data):
    warnings.warn(
        "corepkg.getbinpkg.make_metadata_dict() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    metadata = (
        ...
        for k_bytes in corepkg.xpak.getindex_mem(myid)
    )
    mydict = {
        k: _unicode_decode(
            corepkg.xpak.getitem(data, k_bytes),
...
class ParseLinks(html_parser_HTMLParser):
    ...
    def __init__(self):
        warnings.warn(
            "corepkg.getbinpkg.ParseLinks is deprecated",
            DeprecationWarning,
            stacklevel=2,
        )
...
def create_conn(baseurl, conn=None):
    ...
    warnings.warn(
        "corepkg.getbinpkg.create_conn() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
...
def make_ftp_request(conn, address, rest=None, dest=None):
    ...
    warnings.warn(
        "corepkg.getbinpkg.make_ftp_request() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
...
def make_http_request(conn, address, _params={}, headers={}, dest=None):
    ...
    warnings.warn(
        "corepkg.getbinpkg.make_http_request() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
...
def match_in_array(array, prefix="", suffix="", match_both=1, allow_overlap=0):
    warnings.warn(
        "corepkg.getbinpkg.match_in_array() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
...
def dir_get_list(baseurl, conn=None):
    ...
    warnings.warn(
        "corepkg.getbinpkg.dir_get_list() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    if protocol in ["http", "https"]:
        ...
        if page:
            ...
            import corepkg.exception

            raise corepkg.exception.CorepkgException(
...
def file_get_metadata(baseurl, conn=None, chunk_size=3000):
    ...
    warnings.warn(
        "corepkg.getbinpkg.file_get_metadata() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    if data:
        xpaksize = corepkg.xpak.decodeint(data[-8:-4])
        ...
        myid = corepkg.xpak.xsplit_mem(xpak_data)
        if not myid:
            myid = None, None
        del xpak_data
    else:
...
def file_get(
    ...
    if not fcmd:
        warnings.warn(
            "Use of corepkg.getbinpkg.file_get() without the fcmd "
    ...
    if "DISTDIR" not in variables:
        if dest is None:
            raise corepkg.exception.MissingParameter(
    ...
    from corepkg.util import varexpand
...
def file_get_lib(baseurl, dest, conn=None):
    ...
    warnings.warn(
        "corepkg.getbinpkg.file_get_lib() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
...
def dir_get_metadata(
    ...
    warnings.warn(
        "corepkg.getbinpkg.dir_get_metadata() is deprecated",
        DeprecationWarning,
        stacklevel=2,
    )
...
class PackageIndex:
    ...
    def write(self, pkgfile):
        ...
        for metadata in sorted(self.packages, key=corepkg.util.cmp_sort_key(_cmp_cpv)):
