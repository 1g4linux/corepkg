...
"""deps.py -- Corepkg dependency resolution functions"""
...
from corepkg.eapi import _eapi_attrs, _get_eapi_attrs
from corepkg.exception import InvalidAtom, InvalidData, InvalidDependString
...
def _get_atom_re(eapi_attrs: corepkg.eapi._eapi_attrs) -> re.Pattern:
    global _atom_re
...
def cpvequal(cpv1, cpv2):
    ...
    Example Usage:
            ...
            >>> cpvequal("sys-apps/corepkg-2.1","sys-apps/corepkg-2.1")
            >>> True

    @param cpv1: CategoryPackageVersion (no operators) Example: "sys-apps/corepkg-2.1"
    @type cpv1: String
    @param cpv2: CategoryPackageVersion (no operators) Example: "sys-apps/corepkg-2.1"
    @type cpv2: String
    @rtype: Boolean
    @return:
            1.  True if cpv1 = cpv2
            2.  False Otherwise
            3.  Throws CorepkgException if cpv1 or cpv2 is not a CPV
    """
...
def paren_reduce(mystr, _deprecation_warn=True):
    ...
    """
    if corepkg._internal_caller and _deprecation_warn:
        warnings.warn(
            _("%s is deprecated and will be removed without replacement.")
            % ("corepkg.dep.paren_reduce",),
            DeprecationWarning,
            stacklevel=2,
        )
    mysplit = mystr.split()
...
def _use_reduce_cached(
    ...
    if opconvert and flat:
        raise ValueError(
            "corepkg.dep.use_reduce: 'opconvert' and 'flat' are mutually exclusive"
        )

    if matchall and matchnone:
        raise ValueError(
            "corepkg.dep.use_reduce: 'matchall' and 'matchnone' are mutually exclusive"
        )
...
def dep_opconvert(deplist):
    ...
    if corepkg._internal_caller:
        warnings.warn(
            ...
            % ("corepkg.dep.dep_opconvert", "corepkg.dep.use_reduce"),
            DeprecationWarning,
            stacklevel=2,
        )
...
class _use_dep:
    ...
    def __repr__(self):
        return f"corepkg.dep._use_dep({repr(self.tokens)})"
...
class ExtendedAtomDict(corepkg.cache.mappings.MutableMapping):
    """
