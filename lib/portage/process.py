# corepkg.py -- core Corepkg functionality
...
from corepkg.const import BASH_BINARY, SANDBOX_BINARY, FAKEROOT_BINARY
from corepkg.exception import CommandNotFound
from corepkg.proxy.objectproxy import ObjectProxy
from corepkg.util._ctypes import load_libc, LoadLibrary, ctypes

try:
    from corepkg.util.netlink import RtNetlink
except ImportError:
...
elif os.path.isdir(f"/proc/{corepkg.getpid()}/fd"):
    # In order for this function to work in forked subprocesses,
    # os.getpid() must be called from inside the function.
    def get_open_fds():
        return (
            int(fd) for fd in os.listdir(f"/proc/{corepkg.getpid()}/fd") if fd.isdigit()
        )
...
def atexit_register(func, *args, **kargs):
    ...
    what is registered.  For example, when corepkg restarts itself via
    ...
    if iscoroutinefunction(func):
        ...
        _exithandlers.append((func, args, kargs, corepkg.getpid()))
...
def run_exitfuncs():
    ...
    example)."""
    from corepkg.util import dump_traceback
    ...
    while _exithandlers:
        ...
        try:
            ...
            dump_traceback("Error in corepkg.process.run_exitfuncs", noiselevel=0)
            exc_info = sys.exc_info()
...
def spawn(
    ...
    # Force instantiation of corepkg.data.userpriv_groups before the
    ...
    if returnpid:
        warnings.warn(
            "The corepkg.process.spawn returnpid parameter is deprecated and replaced by returnproc",
            UserWarning,
            stacklevel=1,
        )
        return mypids
...
def _exec(
    ...
    if not opt_name:
        if binary is corepkg._python_interpreter:
    ...
    if unshare_pid:
        ...
        binary, myargs = (
            ...
            [
                corepkg._python_interpreter,
                os.path.join(corepkg._bin_path, "pid-ns-init"),
        ...
        # init process.
        binary, myargs = corepkg._python_interpreter, [
            corepkg._python_interpreter,
            os.path.join(corepkg._bin_path, "pid-ns-init"),
            str(main_child_pid),
        ]
...
def _setup_pipes(fd_pipes, close_fds=True, inheritable=None):
    ...
    file descriptors, see corepkg.locks._close_fds().
...
def _start_fork(
    ...
    try:
        ...
        # Don't used corepkg.getpid() here, in case there is a race
