...
from corepkg.cache import cache_errors, flat_hash
import corepkg.eclass_cache
from corepkg.cache.template import reconstruct_eclasses
from corepkg.cache.mappings import ProtectedDict
...
class database(flat_hash.database):
    ...
    def _parse_data(self, data, cpv):
        ...
        if "_eclasses_" not in d:
            if "INHERITED" in d:
                if self.ec is None:
                    self.ec = corepkg.eclass_cache.cache(self.location[:-15])
    ...
    def _setitem(self, cpv, values):
        ...
        tempfile_kwargs = dict(
            prefix=_unicode_encode(
                f".update.{corepkg.getpid()}.{cpv[s + 1:]}",
