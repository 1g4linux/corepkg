...
class MoveHandler:
    ...
    def _grab_global_updates(self):
        from corepkg.update import grab_updates, parse_updates
    ...
    def check(self, **kwargs):
        ...
        for i, cpv in enumerate(cpv_all):
            ...
            if not updates:
                continue
            metadata_updates = corepkg.update_dbentries(updates, metadata, parent=pkg)
            if metadata_updates:
...
class MoveInstalled(MoveHandler):
    ...
    def __init__(self):
        ...
        MoveHandler.__init__(
            self, corepkg.db[eroot]["vartree"], corepkg.db[eroot]["porttree"]
        )
...
class MoveBinary(MoveHandler):
    ...
    def __init__(self):
        eroot = corepkg.settings["EROOT"]
        MoveHandler.__init__(
            self, corepkg.db[eroot]["bintree"], corepkg.db[eroot]["porttree"]
        )
