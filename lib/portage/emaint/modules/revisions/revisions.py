...
class PurgeRevisions:
    ...
    def __init__(self, settings=None):
        ...
        @param settings: optional corepkg.config instance to get EROOT from.
        """
        self._settings = settings

    @property
    def settings(self):
        return self._settings or corepkg.settings

    def purgeallrepos(self, **kwargs):
        """Purge revisions for all repos"""
        repo_revisions_file = os.path.join(
            self.settings["EROOT"], corepkg.const.REPO_REVISIONS
        )
        msgs = []
        try:
            ...
            try:
                repo_revisions_lock = corepkg.locks.lockfile(repo_revisions_file)
                ...
                if repo_revisions_lock is not None:
                    corepkg.locks.unlockfile(repo_revisions_lock)
