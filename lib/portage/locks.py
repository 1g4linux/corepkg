...
import corepkg
from corepkg import os, _encodings, _unicode_decode
...
def lockfile(mypath, wantnewlockfile=0, unlinkfile=0, waiting_msg=None, flags=0):
    ...
    directory as the file: '.' + basename + '.corepkg_lockfile'.
    """
    lock = None
    while lock is None:
...
def _lockfile_iteration(
    ...
    corepkg_gid = int(corepkg.data.corepkg_gid)
    ...
    if hasattr(mypath, "fileno"):
        warnings.warn(
            "corepkg.locks.lockfile() support for "
    ...
    if isinstance(mypath, int):
        warnings.warn(
            "corepkg.locks.lockfile() support for integer file "
        ...
        lockfilename = os.path.join(base, "." + tail + ".corepkg_lockfile")
            ...
            if not preexisting:
                try:
                    if (
                        ...
                        and os.stat(lockfilename).st_gid != corepkg_gid
                    ):
                        os.chown(lockfilename, -1, corepkg_gid)
                except OSError as e:
                    if e.errno in (errno.ENOENT, errno.ESTALE):
                        os.close(myfd)
                        return None
                    writemsg(
                        "%s: chown('%s', -1, %d)\n" % (e, lockfilename, corepkg_gid),
                        noiselevel=-1,
                    )
                    writemsg(
...
def hardlock_name(path):
    ...
    return os.path.join(
        base,
        ".%s.hardlock-%s-%s"
        % (tail, corepkg._decode_argv([os.uname()[1]])[0], corepkg.getpid()),
    )
...
def hardlink_lockfile(
    ...
    if max_wait is not DeprecationWarning:
        warnings.warn(
            ...
            "corepkg.locks.hardlink_lockfile() is now unused. Use "
        ...
        try:
            ...
            try:
                ...
                if not preexisting:
                    ...
                    if corepkg.data.secpass >= 1 and myfd_st.st_gid != corepkg_gid:
                        os.fchown(myfd, -1, corepkg_gid)
            except OSError as e:
                if e.errno not in (errno.ENOENT, errno.ESTALE):
                    writemsg(
                        "%s: fchown('%s', -1, %d)\n" % (e, lockfilename, corepkg_gid),
...
def hardlock_cleanup(path, remove_all_locks=False):
    myhost = corepkg._decode_argv([os.uname()[1]])[0]
