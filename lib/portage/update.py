...
from corepkg.const import USER_CONFIG_PATH, VCS_DIRS
from corepkg.eapi import _get_eapi_attrs
from corepkg.exception import DirectoryNotFound, InvalidAtom, CorepkgException
...
def update_dbentry(update_cmd, mycontent, eapi=None, parent=None):
    from corepkg.dep import Atom, isvalidatom, match_from_list
...
def fixdbentries(update_iter, dbdir, eapi=None, parent=None):
    ...
    warnings.warn(
        "corepkg.update.fixdbentries() is deprecated", DeprecationWarning, stacklevel=2
    )
...
def update_config_files(
    ...
    """Perform global updates on /etc/corepkg/package.*, /etc/corepkg/profile/package.*,
    /etc/corepkg/profile/packages and /etc/corepkg/sets.
