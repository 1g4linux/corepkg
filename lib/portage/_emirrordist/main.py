...
from corepkg.package.ebuild.fetch import ContentHashLayout
from corepkg.util import normalize_path, _recursive_file_list
...
common_options = (
    ...
    {
        "longopt": "--config-root",
        "help": "location of corepkg config files",
        "metavar": "DIR",
    },
    {
...
def parse_args(args):
    ...
    actions.add_argument(
        "--version", action="store_true", help="display corepkg version and exit"
    )
    actions.add_argument(
        "--mirror",
        action="store_true",
...
def emirrordist_main(args):
    ...
    if options.version:
        sys.stdout.write(f"Corepkg {corepkg.VERSION}\n")
        return os.EX_OK
    ...
    settings = corepkg.config(config_root=config_root, local_config=False, env=env)
    ...
    portdb = corepkg.portdbapi(mysettings=settings)

    # Limit ebuilds to the specified repo.
    portdb.porttrees = [repo_path]

    corepkg.util.initialize_logger()
