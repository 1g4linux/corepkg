...
from corepkg import load_mod
from corepkg import _unicode_decode
from corepkg import _unicode_encode
from corepkg import _encodings
from corepkg.const import USER_CONFIG_PATH, GLOBAL_CONFIG_PATH
from corepkg.const import VCS_DIRS
from corepkg.const import _ENABLE_SET_CONFIG
from corepkg.exception import PackageSetNotFound
...
class SetConfig:
    ...
    def _create_default_config(self):
        """
        Create a default hardcoded set configuration for a corepkg version
        that does not support set configuration files. This is only used
        in the current branch of corepkg if _ENABLE_SET_CONFIG is False.
        Even if it's not used in this branch, keep it here in order to
        minimize the diff between branches.

                [world]
                class = corepkg.sets.base.DummyPackageSet
                packages = @selected @system

                [selected]
                class = corepkg.sets.files.WorldSelectedSet

                [system]
                class = corepkg.sets.profiles.PackagesSystemSet
        ...
        parser.set("world", "class", "corepkg.sets.base.DummyPackageSet")
        parser.set("world", "packages", "@profile @selected @system")

        parser.remove_section("profile")
        parser.add_section("profile")
        parser.set(
            "profile", "class", "corepkg.sets.ProfilePackageSet.ProfilePackageSet"
        )

        parser.remove_section("selected")
        parser.add_section("selected")
        parser.set("selected", "class", "corepkg.sets.files.WorldSelectedSet")

        parser.remove_section("selected-packages")
        parser.add_section("selected-packages")
        parser.set(
            "selected-packages", "class", "corepkg.sets.files.WorldSelectedPackagesSet"
        )

        parser.remove_section("selected-sets")
        parser.add_section("selected-sets")
        parser.set("selected-sets", "class", "corepkg.sets.files.WorldSelectedSetsSet")

        parser.remove_section("system")
        parser.add_section("system")
        parser.set("system", "class", "corepkg.sets.profiles.PackagesSystemSet")

        parser.remove_section("security")
        parser.add_section("security")
        parser.set("security", "class", "corepkg.sets.security.NewAffectedSet")

        parser.remove_section("usersets")
        parser.add_section("usersets")
        parser.set("usersets", "class", "corepkg.sets.files.StaticFileSet")
        parser.set("usersets", "multiset", "true")
        parser.set("usersets", "directory", "%(PORTAGE_CONFIGROOT)setc/corepkg/sets")
        parser.set("usersets", "world-candidate", "true")

        parser.remove_section("live-rebuild")
        parser.add_section("live-rebuild")
        parser.set("live-rebuild", "class", "corepkg.sets.dbapi.VariableSet")
        ...
        parser.set("deprecated-live-rebuild", "class", "corepkg.sets.dbapi.VariableSet")
        parser.set("deprecated-live-rebuild", "variable", "INHERITED")
        parser.set(
            "deprecated-live-rebuild",
            "includes",
            " ".join(sorted(corepkg.const.LIVE_ECLASSES)),
        )

        parser.remove_section("module-rebuild")
        parser.add_section("module-rebuild")
        parser.set("module-rebuild", "class", "corepkg.sets.dbapi.OwnerSet")
        ...
        parser.set(
            "preserved-rebuild",
            "class",
            "corepkg.sets.libs.PreservedLibraryConsumerSet",
        )

        parser.remove_section("x11-module-rebuild")
        parser.add_section("x11-module-rebuild")
        parser.set("x11-module-rebuild", "class", "corepkg.sets.dbapi.OwnerSet")
    ...
    def _parse(self, update=False):
        ...
        for sname in parser.sections():
            ...
            if not parser.has_option(sname, "class"):
                classname = "corepkg._sets.files.StaticFileSet"
            else:
                classname = parser.get(sname, "class")

            if classname.startswith("corepkg.sets."):
            ...
            try:
                ...
                try:
                    setclass = load_mod("corepkg._sets." + classname)
                except (ImportError, AttributeError):
                    self.errors.append(
