...
if installation.TYPE == installation.TYPES.MODULE:
    GLOBAL_CONFIG_PATH = "/share/corepkg/config"
else:
    GLOBAL_CONFIG_PATH = "/usr/share/corepkg/config"

# these variables are not used with target_root or config_root
# NOTE: Use realpath(__file__) so that python module symlinks in site-packages
# are followed back to the real location of the whole corepkg installation.
if installation.TYPE == installation.TYPES.SYSTEM:
    ...
    PORTAGE_BASE_PATH = os.path.join(
        os.path.realpath(__import__("sys").prefix), "lib/corepkg"
    )
else:
    PORTAGE_BASE_PATH = os.path.join(
...
PORTAGE_PACKAGE_ATOM = "app-core/corepkg"
...
RSYNC_PACKAGE_ATOM = "app-net/rsync"
...
# List of known live eclasses. Keep it in sync with cnf/sets/corepkg.conf
...
# Top-level names of Python packages installed by Corepkg.
PORTAGE_PYM_PACKAGES = ("_emerge", "corepkg")
