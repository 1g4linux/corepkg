...
from corepkg import _native_string, _unicode_encode
...
def setexec(ctx="\n"):
    ...
    try:
        ...
        msg = _(
            ...
            + "Is your current SELinux context allowed to run Corepkg?"
        )
        if selinux.security_getenforce() == 1:
            raise OSError(msg)
        else:
            corepkg.writemsg(f"!!! {msg}\n", noiselevel=-1)
    ...
    *after* the fork (thereby avoiding any interference with concurrent
    ...
    def __call__(self, *args, **kwargs):
        if self._con is not None:
            ...
            kwargs["pre_exec"] = (
                corepkg.process._chain_pre_exec_fns(pre_exec, kwargs["pre_exec"])
