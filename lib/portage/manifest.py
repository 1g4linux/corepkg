...
from corepkg.const import MANIFEST2_HASH_DEFAULTS, MANIFEST2_IDENTIFIERS
...
class FileNotInManifestException(CorepkgException):
    pass
...
class Manifest:
    ...
    def __init__(
        ...
        from corepkg.repository.config import _find_invalid_path_char

        if manifest1_compat is not DeprecationWarning:
            warnings.warn(
                "The manifest1_compat parameter of the "
                "corepkg.manifest.Manifest constructor is deprecated.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def _is_cpv(self, cat, pn, filename):
        ...
        cpv = f"{cat}/{pf}"
        if not ps:
            raise CorepkgPackageException(_(f"Invalid package name: '{cpv}'"))
        if ps[0] != pn:
            raise CorepkgPackageException(
    ...
    def _catsplit(self, pkg_key):
        ...
        This is compatible with corepkg.catsplit()"""
