...
COMPAT_DISTDIR = "usr/corepkg/distfiles"
COMPAT_PKGDIR = "usr/corepkg/packages"
COMPAT_RPMDIR = "usr/corepkg/rpm"
COMPAT_MAIN_REPO = "usr/corepkg"


def main():
    """
    If the current installation is still configured to use any of the
    legacy default /usr/corepkg locations, then patch make.globals and
    ...
    config = corepkg.settings

    compat_distdir = os.path.join(corepkg.const.EPREFIX or "/", COMPAT_DISTDIR)
    try:
        do_distdir = os.path.samefile(config["DISTDIR"], compat_distdir)
    except OSError:
        do_distdir = False

    compat_pkgdir = os.path.join(corepkg.const.EPREFIX or "/", COMPAT_PKGDIR)
    try:
        do_pkgdir = os.path.samefile(config["PKGDIR"], compat_pkgdir)
    except OSError:
        do_pkgdir = False

    compat_rpmdir = os.path.join(corepkg.const.EPREFIX or "/", COMPAT_RPMDIR)
    try:
        do_rpmdir = os.path.samefile(config["RPMDIR"], compat_rpmdir)
    except OSError:
        do_rpmdir = False

    compat_main_repo = os.path.join(corepkg.const.EPREFIX or "/", COMPAT_MAIN_REPO)
    try:
