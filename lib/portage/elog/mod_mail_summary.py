...
def process(mysettings, key, logentries, fulltext):
    ...
    ) % {"pkg": key, "pid": corepkg.getpid(), "time": time_str}
    ...
    # have come from /etc/corepkg/package.env, since
...
def _finalize(mysettings, items):
    ...
    ) % {"pid": corepkg.getpid(), "host": socket.getfqdn()}
    for key in items:
        mybody += f"- {key}\n"

    mymessage = corepkg.mail.create_message(
