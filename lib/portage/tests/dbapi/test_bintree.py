...
from corepkg.const import BINREPOS_CONF_FILE
...
class BinarytreeTestCase(TestCase):
    ...
    @patch("corepkg.dbapi.bintree.binarytree._populate_local")
    ...
    @patch("corepkg.dbapi.bintree.binarytree._populate_additional")
    @patch("corepkg.dbapi.bintree.binarytree._populate_local")
    ...
    @patch("corepkg.dbapi.bintree.BinRepoConfigLoader")
    @patch("corepkg.dbapi.bintree.binarytree._populate_remote")
    @patch("corepkg.dbapi.bintree.binarytree._populate_local")
    ...
    def test_populate_with_getbinpkgs_and_not_BinRepoConfigLoader(
        ...
        refresh = "something"
        settings = MagicMock()
        corepkg_root = "/some/path"
        settings.__getitem__.return_value = corepkg_root
        pBinRepoConfigLoader.return_value = None
        conf_file = os.path.join(corepkg_root, BINREPOS_CONF_FILE)
        bt = binarytree(pkgdir=os.getenv("TMPDIR", "/tmp"), settings=settings)
        out = io.StringIO()
    ...
    @patch("corepkg.data.secpass", 2)
    @patch("corepkg.dbapi.bintree.BinRepoConfigLoader")
    @patch("corepkg.dbapi.bintree.binarytree._run_trust_helper")
