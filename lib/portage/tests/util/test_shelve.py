# Copyright 2020-2023 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

import argparse
import os
import shutil
import tempfile
import time

from corepkg.tests import TestCase
from corepkg.util.shelve import dump, open_shelve, restore


class ShelveUtilsTestCase(TestCase):
    TEST_DATA = (
        # distfiles_db
        {
            "corepkg-2.3.89.tar.bz2": "sys-apps/corepkg-2.3.89",
            "corepkg-2.3.99.tar.bz2": "sys-apps/corepkg-2.3.99",
        },
        # deletion_db
        {
            "corepkg-2.3.89.tar.bz2": time.time(),
            "corepkg-2.3.99.tar.bz2": time.time(),
        },
        # recycle_db
        {
            "corepkg-2.3.89.tar.bz2": (0, time.time()),
            "corepkg-2.3.99.tar.bz2": (0, time.time()),
        },
    )

    def test_dump_restore(self):
        for data in self.TEST_DATA:
            tmpdir = tempfile.mkdtemp()
            try:
                dump_args = argparse.Namespace(
                    src=os.path.join(tmpdir, "shelve_file"),
                    dest=os.path.join(tmpdir, "pickle_file"),
                )
                db = open_shelve(dump_args.src, flag="c")
                for k, v in data.items():
                    db[k] = v
                db.close()
                dump(dump_args)

                restore_args = argparse.Namespace(
                    dest=dump_args.src,
                    src=dump_args.dest,
                )
                restore(restore_args)

                db = open_shelve(restore_args.dest, flag="r")
                for k, v in data.items():
                    self.assertEqual(db[k], v)
                db.close()
            finally:
                shutil.rmtree(tmpdir)
