...
import corepkg
from corepkg.tests import TestCase, get_pythonpath
from corepkg.util import socks5
...
class _socket_file_wrapper(corepkg.proxy.objectproxy.ObjectProxy):
    """
    ...
    async def _test_socks5_proxy(self):
        ...
        try:
            ...
            try:
                ...
                await corepkg.process.run_coroutine_exitfuncs()
...
class Socks5ServerAtExitTestCase(TestCase):
    ...
    terminated by corepkg.process.run_exitfuncs(), using a subprocess
    for isolation.
