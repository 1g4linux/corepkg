...
from corepkg.util.futures._asyncio.streams import _writer
...
class AsyncFunctionTestCase(TestCase):
    ...
    async def _testAsyncFunctionStdin(self, loop):
        ...
        stdin_backup = os.dup(corepkg._get_stdin().fileno())
        os.dup2(pr.fileno(), corepkg._get_stdin().fileno())
        pr.close()
        try:
            ...
            os.dup2(stdin_backup, corepkg._get_stdin().fileno())
    ...
    def _test_getpid_fork(preexec_fn=None):
        """
        Verify that corepkg.getpid() cache is updated in a forked child process.
        """
        if preexec_fn is not None:
            preexec_fn()
        loop = asyncio._wrap_loop()
        proc = AsyncFunction(scheduler=loop, target=corepkg.getpid)
        proc.start()
        proc.wait()
    ...
    def test_spawn_getpid(self):
        """
        Test corepkg.getpid() with multiprocessing spawn start method.
        """
        loop = asyncio._wrap_loop()
        proc = AsyncFunction(
            scheduler=loop,
    ...
    def test_getpid_double_fork(self):
        """
        Verify that corepkg.getpid() cache is updated correctly after
        two forks.
        """
        loop = asyncio._wrap_loop()
