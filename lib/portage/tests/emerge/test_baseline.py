...
"""This module defines a baseline for corepkg's functionality.

Multiple corepkg commands are executed in a sequence in a playground
(see the ``baseline_command`` fixture in ``conftest.py``).

All the commands are triggered from the ``test_corepkg_baseline`` test.
...
functional or end-to-end test, ensuring some functionality of corepkg.
...
from corepkg.tests import cnf_etc_path
...
@pytest.mark.ft
def test_corepkg_baseline(async_loop, playground, binhost, baseline_command):
...
async def _async_test_baseline(playground, binhost, commands):
    ...
    env = {
        ...
        "PATH": path,
        "PKGDIR": pkgdir,
        "PORTAGE_INST_GID": str(os.getgid()),  # str(corepkg.data.corepkg_gid),
        "PORTAGE_INST_UID": str(os.getuid()),  # str(corepkg.data.corepkg_uid),
        "PORTAGE_PYTHON": corepkg._python_interpreter,
        "PORTAGE_REPOSITORIES": settings.repositories.config_string(),
        "PORTAGE_TMPDIR": corepkg_tmpdir,
        "PORTAGE_TRUST_HELPER": "getuto",
        "PORTAGE_LOGDIR": corepkg_tmpdir,
