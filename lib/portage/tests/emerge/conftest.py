...
from corepkg.tests import cnf_bindir, cnf_sbindir
from corepkg.process import find_binary
from corepkg.util import find_updated_config_files
import corepkg
...
class Ebuild(CorepkgCommand):
    name = "ebuild"
    command = (PORTAGE_PYTHON, "-b", "-Wd", os.path.join(str(cnf_bindir), name))


class Egencache(CorepkgCommand):
    name = "egencache"
    command = (
        PORTAGE_PYTHON,
        "-b",
...
class Emaint(CorepkgCommand):
    name = "emaint"
    command = (PORTAGE_PYTHON, "-b", "-Wd", os.path.join(str(cnf_sbindir), name))


class EtcUpdate(CorepkgCommand):
    name = "etc-update"
    command = (BASH_BINARY, os.path.join(str(cnf_sbindir), name))


class Fixpackages(CorepkgCommand):
    name = "fixpackages"
    command = (
        PORTAGE_PYTHON,
        "-b",
...
class Regenworld(CorepkgCommand):
    name = "regenworld"
    command = (
        PORTAGE_PYTHON,
        "-b",
...
def _generate_all_baseline_commands(playground, binhost):
    ...
    ``test_corepkg_baseline`` will use.
    ...
    test_commands["emerge with quickpkg direct"] = CorepkgCommandSequence(
        *quickpkg_direct_seq
    )

    test_commands["env-update"] = EnvUpdate()
    test_commands["corepkgq envvar"] = Corepkgq(
        "envvar",
        "-v",
        "CONFIG_PROTECT",
        "EROOT",
    ...
    test_commands["emerge --regen/--metadata"] = CorepkgCommandSequence(*regen_seq)
    ...
    test_commands["misc package operations"] = CorepkgCommandSequence(*abcd_seq)
    ...
    test_commands["misc operations with eprefix"] = CorepkgCommandSequence(
        *cross_prefix_seq
    )
    ...
    if fetch_bin is None:
        ...
        fetch_sequence = CorepkgCommandSequence(
...
def baseline_command(request, _generate_all_baseline_commands):
    ...
    functional test of corepkg. It uses another fixture, namely
