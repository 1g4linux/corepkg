...
import corepkg
from corepkg import os, shutil
from corepkg import _unicode_decode
from corepkg.const import PORTAGE_PYM_PATH, REPO_REVISIONS, TIMESTAMP_FORMAT
...
class SyncLocalTestCase(TestCase):
    ...
    def testSyncLocal(self):
        ...
        profile = {"eapi": ("8",), "package.use.stable.mask": ("dev-libs/A flag",)}

        ebuilds = {
            "dev-libs/A-0": {"EAPI": "8"},
            "sys-apps/corepkg-3.0": {"IUSE": "+python_targets_python3_8"},
        }

        installed = {
            "sys-apps/corepkg-2.3.99": {
                "EAPI": "8",
        ...
        for cmd in ("egencache", "emerge", "emaint"):
            for bindir in (self.bindir, self.sbindir):
                ...
                if os.path.exists(path):
                    cmds[cmd] = (corepkg._python_interpreter, "-b", "-Wd", path)
                    break
            else:
