...
import corepkg
import corepkg.util.formatter as formatter
...
from corepkg.const import COLOR_MAP_FILE
...
def _parse_color_map(config_root="/", onerror=None):
    """
    Parse /etc/corepkg/color.map and return a dict of error codes.
...
def xtermTitleReset():
    from corepkg.process import find_binary, spawn
...
def set_term_size(lines: int, columns: int, fd: int) -> Optional[Future]:
    ...
    try:
        ...
        writemsg(_("corepkg: stty: command not found\n"), noiselevel=-1)
    else:
...
class _LazyInitColorMap(corepkg.proxy.objectproxy.ObjectProxy):
    __slots__ = ("_attr",)
