...
__all__ = ["close_portdbapi_caches", "FetchlistDict", "corepkgtree", "portdbapi"]

import corepkg

from corepkg.cache import volatile
from corepkg.cache.cache_errors import CacheError
from corepkg.cache.mappings import Mapping
...
from corepkg.util.futures.iter_completed import iter_gather
...
def close_portdbapi_caches():
    ...
    try:
        corepkg._legacy_globals_constructed
    except AttributeError:
        pass
    else:
        if "db" in corepkg._legacy_globals_constructed:
            try:
                db = corepkg.db
            except AttributeError:
                pass
...
corepkg.process.atexit_register(close_portdbapi_caches)
...
class _dummy_list(list):
    def remove(self, item):
        # TODO: Trigger a DeprecationWarning here, after stable corepkg
...
class _better_cache:
    ...
    Better_cache.items data may look like this::
      ...
      { "sys-apps/corepkg" : [ repo1, repo2 ] }

    Without better_cache, Corepkg will get slower and slower (due to excessive IO) as more overlays are added.
...
class portdbapi(dbapi):
    """this tree will scan a corepkg directory located at root (passed to init)"""
    ...
    def porttree_root(self):
        warnings.warn(
            "corepkg.dbapi.porttree.portdbapi.porttree_root is deprecated in favor of corepkg.repository.config.RepoConfig.location "
            "(available as repositories[repo_name].location attribute of instances of corepkg.dbapi.porttree.portdbapi class)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def eclassdb(self):
        warnings.warn(
            "corepkg.dbapi.porttree.portdbapi.eclassdb is deprecated in favor of corepkg.repository.config.RepoConfig.eclass_db "
            "(available as repositories[repo_name].eclass_db attribute of instances of corepkg.dbapi.porttree.portdbapi class)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def __init__(self, _unused_param=DeprecationWarning, mysettings=None):
        ...
        # Support corepkg.data reload for unit tests.
        corepkg_gid = corepkg.data.corepkg_gid
        secpass = corepkg.data.secpass

        if mysettings:
            self.settings = mysettings
        else:
            self.settings = config(clone=corepkg.settings)

        if _unused_param is not DeprecationWarning:
            warnings.warn(
                "The first parameter of the "
                + "corepkg.dbapi.porttree.portdbapi"
                + " constructor is unused since corepkg-2.1.8. "
    ...
    def _set_porttrees(self, porttrees):
        ...
        self._porttrees_repos = corepkg.OrderedDict(
            (repo.name, repo)
            for repo in (
    ...
    def _init_cache_dirs(self):
        """Create /var/cache/edb/dep and adjust permissions for the corepkg
        group."""
        ...
        try:
            ensure_dirs(self.depcachedir, gid=corepkg_gid, mode=dirmode, mask=modemask)
    ...
    def aux_get(
        ...
        'return: ["0",">=sys-libs/bar-1.0","http://www.foo.com"] or raise CorepkgKeyError if error'
    ...
    async def async_aux_get(self, mycpv, mylist, mytree=None, myrepo=None, loop=None):
        ...
        try:
            ...
            # Missing slash. Can't find ebuild so raise CorepkgKeyError.
            raise CorepkgKeyError(mycpv)
        ...
        async with contextlib.AsyncExitStack() as stack:
            try:
                if (
                    ...
                    settings = corepkg.config(clone=self.settings)
    ...
    def _aux_get_return(
        ...
        if not eapi:
            eapi = "8"
            mydata["EAPI"] = eapi
        if eapi_is_valid_for_runtime(eapi):
    ...
    def async_fetch_map(self, mypkg, useflags=None, mytree=None, loop=None):
        ...
        def aux_get_done(aux_get_future):
            ...
            if aux_get_future.exception() is not None:
                if isinstance(aux_get_future.exception(), CorepkgKeyError):
                    # Convert this to an InvalidDependString exception since
                    # callers already handle it.
                    result.set_exception(
                        corepkg.exception.InvalidDependString(
            ...
            if effective_eapi != "8":
                ...
                result.set_exception(
                    corepkg.exception.InvalidDependString(
                        f"getFetchMap(): '{mypkg}' has "
                        f"{unsupported_eapi_message(eapi)}"
                    )
                )
                return
            ...
            try:
                result.set_result(
                    _parse_uri_map(
                        mypkg,
                        {"EAPI": effective_eapi, "SRC_URI": myuris},
                        use=useflags,
                    )
                )
    ...
    def getfetchsizes(self, mypkg, useflags=None, debug=0, myrepo=None):
        from corepkg.package.ebuild.fetch import _download_suffix
        from corepkg.util import writemsg
            ...
            if mystat is None:
                ...
                if ro_distdirs is not None:
                    for x in shlex.split(ro_distdirs):
                        try:
                            mystat = os.stat(
                                corepkg.package.ebuild.fetch.get_mirror_url(
    ...
    def fetch_check(
        ...
        for x in myfiles:
            if not mysums or x not in mysums:
                ...
                try:
                    ok, reason = corepkg.checksum.verify_all(
    ...
    def cp_list(self, mycp, use_cache=1, mytree=None):
        from corepkg.util import writemsg
        from corepkg.versions import pkgsplit, ver_regexp, _pkg_str
    ...
    def xmatch(
        ...
        @type origdep: corepkg.dep.Atom or str
        ...
        if level == "list-visible":
            ...
            warnings.warn(
                ...
                "corepkg.dbapi.porttree.portdbapi.xmatch "
    ...
    async def async_xmatch(self, level, origdep, loop=None):
        ...
        from corepkg.dep import match_from_list, _match_slot
...
class corepkgtree:
    def __init__(
        ...
        """
        Constructor for a CorepkgTree

        @param root: deprecated, defaults to settings['ROOT']
        @type root: String/Path
        @param virtual: UNUSED
        @type virtual: No Idea
        @param settings: Corepkg Configuration object (corepkg.settings)
        ...
        if root is not DeprecationWarning:
            warnings.warn(
                ...
                + "corepkg.dbapi.porttree.corepkgtree"
    ...
    def portroot(self):
        ...
        warnings.warn(
            ...
            "corepkg.dbapi.porttree.corepkgtree is deprecated. Use the "
    ...
    def getname(self, pkgname):
        ...
        warnings.warn(
            ...
            "corepkg.dbapi.porttree.corepkgtree is deprecated. "
...
class FetchlistDict(Mapping):
    ...
    to allow corepkg.manifest.Manifest to access fetch lists via a standard
    ...
    def has_key(self, pkg_key):
        ...
        warnings.warn(
            "corepkg.dbapi.porttree.FetchlistDict.has_key() is "
...
def _async_manifest_fetchlist(
    ...
    @param max_jobs: max number of futures to process concurrently (default
            is corepkg.util.cpuinfo.get_cpu_count())
    @type max_jobs: int
...
def _parse_uri_map(cpv, metadata, use=None):
    from corepkg.dep import use_reduce
    ...
    while myuris:
        ...
        if myuris and myuris[-1] == "->":
            ...
            if not distfile:
                raise corepkg.exception.InvalidDependString(
