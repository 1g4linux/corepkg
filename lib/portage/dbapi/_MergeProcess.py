...
import fcntl
import corepkg
from corepkg import os, _unicode_decode
from corepkg.package.ebuild._ipc.QueryCommand import QueryCommand
from corepkg.util._ctypes import load_libc
import corepkg.elog.messages
...
class MergeProcess(ForkProcess):
    ...
    def _start(self):
        # Corepkg should always call setcpv prior to this
        ...
        self.fd_pipes.setdefault(0, corepkg._get_stdin().fileno())
        ...
        if output:
            ...
            if len(lines) == 1:
                ...
                for line in lines:
                    ...
                    reporter = getattr(corepkg.elog.messages, funcname)
    ...
    def _spawn(self, args, fd_pipes, **kwargs):
        ...
        child_db = {}
        parent_db = corepkg.db if QueryCommand._db is None else QueryCommand._db
        ...
        corepkg.elog.messages.collect_messages(key=mylink.mycpv)
    ...
    def _target(
        ...
        if QueryCommand._db is None:
            ...
            QueryCommand._db = db
        corepkg.output.havecolor = not no_color(settings)
    ...
    def _proc_join_done(self, proc, future):
        ...
        if (
            ...
            and proc.exitcode == corepkg.const.RETURNCODE_POSTINST_FAILURE
        ):
