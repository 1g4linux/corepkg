...
from corepkg.cache.mappings import UserDict
...
def initialize_logger(level=logging.WARNING) -> None:
    """Sets up basic logging of corepkg activities
    Args:
...
def writemsg_level(msg: str, level: int = 0, noiselevel: int = 0) -> None:
    ...
    3. noiselevel - corepkg's logging level, passed directly to writemsg
    """
...
def stack_dictlist(original_dicts, incremental=0, incrementals=[], ignore_none=0):
    ...
    Example usage:
       >>> from corepkg.util import stack_dictlist
...
def grabdict(
    ...
    @param recursive: Recursively grab ( support for /etc/corepkg/package.keywords/* and friends )
    ...
    1.  Returns the lines in a file in a dictionary, for example:
            'sys-apps/corepkg x86 amd64 ppc'
            would return
            {"sys-apps/corepkg" : ['x86', 'amd64', 'ppc']}
    """
    newdict = {}
...
class _getconfig_shlex(shlex.shlex):
    def __init__(self, corepkg_tolerant=False, **kwargs):
        shlex.shlex.__init__(self, **kwargs)
        self.__corepkg_tolerant = corepkg_tolerant

    def allow_sourcing(self, var_expand_map):
        self.source = corepkg._native_string("source")
...
def getconfig(
    ...
    if content and content[-1] != corepkg._native_string("\n"):
        content += corepkg._native_string("\n")

    # Warn about dos-style line endings since that prevents
    # people from being able to source them with bash.
    if corepkg._native_string("\r") in content:
        writemsg(
            (
                "!!! "
                + _(
    ...
    try:
        ...
        lex = _getconfig_shlex(
            instream=content, infile=mycfg, posix=True, corepkg_tolerant=tolerant
        )
        lex.wordchars = corepkg._native_string(
            string.digits + string.ascii_letters + r"~!@#$%*_\:;?,./-+{}"
        )
        lex.quotes = corepkg._native_string("\"'")
        if allow_sourcing:
...
def unique_array(s):
    ...
    # blah.	 back to original corepkg.unique_array
    u = []
    for x in s:
        if x not in u:
            u.append(x)
    ...
    if (uid != -1 and uid != stat_cached.st_uid) or (
        ...
        try:
            if follow_links:
                ...
                corepkg.data.lchown(filename, uid, gid)
            modified = True
        except OSError as oe:
...
def apply_recursive_permissions(
    ...
    function; it will be called with one argument, a CorepkgException instance.
...
def apply_secpass_permissions(
    ...
    # Avoid accessing corepkg.data.secpass when possible, since
    # it triggers config loading (undesirable for chmod-lite).
    if (uid != -1 or gid != -1) and corepkg.data.secpass < 2:
    ...
    if last_pfile and newmd5:
        try:
            ...
            if stat.S_ISLNK(old_pfile_st.st_mode):
                ...
                try:
                    last_pfile_md5 = corepkg.checksum._perform_md5_merge(old_pfile)
                except FileNotFound:
