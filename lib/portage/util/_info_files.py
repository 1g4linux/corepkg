...
def chk_updated_info_files(root, infodirs, prev_mtimes):
    if os.path.exists("/usr/bin/install-info"):
        ...
        for z in infodirs:
            if z == "":
                continue
            inforoot = corepkg.util.normalize_path(root + z)
        ...
        if not regen_infodirs:
            corepkg.util.writemsg_stdout("\n")
            if corepkg.util.noiselimit >= 0:
            ...
            for inforoot in regen_infodirs:
                ...
                for x in file_list:
                    ...
                    try:
                        ...
                        myso = corepkg._unicode_decode(proc.communicate()[0]).rstrip(
                            "\n"
                        )
                        proc.wait()
            ...
            if badcount:
                ...
                corepkg.util.writemsg_level(errmsg, level=logging.ERROR, noiselevel=-1)
            else:
                if icount > 0 and corepkg.util.noiselimit >= 0:
