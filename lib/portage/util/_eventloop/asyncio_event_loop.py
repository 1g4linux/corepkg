...
class AsyncioEventLoop(_AbstractEventLoop):
    ...
    event loop and is minimally compatible with _CorepkgEventLoop.
    """
    ...
    def _close(self):
        """
        Before closing the main loop, run corepkg.process.run_exitfuncs()
        ...
        if not (self._closing or self.is_closed()):
            ...
            if self._coroutine_exithandlers:
                self.run_until_complete(corepkg.process.run_coroutine_exitfuncs())
            self._loop.close()
            self._closing = False
    ...
    def _internal_caller_exception_handler(loop, context):
        ...
        implementation is only used when corepkg._internal_caller is True.
        """
        ...
        if "exception" in context:
            ...
            os.kill(corepkg.getpid(), signal.SIGTERM)
    ...
    def _asyncio_child_watcher(self):
        """
        Corepkg internals use this as a layer of indirection for
        ...
        if self._child_watcher is None:
            ...
            if PidfdChildWatcher is not None and hasattr(os, "pidfd_open"):
                try:
                    fd = os.pidfd_open(corepkg.getpid())
                except Exception:
                    pass
                else:
                    os.close(fd)
    ...
    def _asyncio_wrapper(self):
        """
        Corepkg internals use this as a layer of indirection in cases
    ...
    def _run_until_complete(self, future):
        ...
        implementation is only used when corepkg._internal_caller is True.
        """
        if not corepkg._internal_caller:
        ...
        if self._wakeup_fd != -1:
            ...
            # set_wakeup_fd calls.
            os.kill(corepkg.getpid(), signal.SIGCHLD)
        try:
            return self._loop.run_until_complete(future)
        finally:
            try:
