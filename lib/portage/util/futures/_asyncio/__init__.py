...
import corepkg
from corepkg.process import atexit_register
from corepkg.util._eventloop.asyncio_event_loop import (
...
def get_event_loop_policy():
    ...
    """
    from corepkg.util.futures.unix_events import _CorepkgEventLoopPolicy

    global _lock, _policy
    with _lock:
        if _policy is None:
            _policy = _CorepkgEventLoopPolicy()
        return _policy
...
def set_event_loop_policy(policy):
    ...
    global _lock, _policy
    with _lock:
        _policy = policy or _CorepkgEventLoopPolicy()
...
def _safe_loop(create: Optional[bool] = True) -> Optional[_AsyncioEventLoop]:
    ...
    For corepkg internal callers or external API consumers calling from
    ...
    with _thread_weakrefs.lock:
        if _thread_weakrefs.pid != corepkg.getpid():
            _thread_weakrefs.pid = corepkg.getpid()
...
def _get_running_loop():
    ...
    corepkg's internal AsyncioEventLoop wrapper. If there is no running
    asyncio event loop but corepkg has a reference to another running
    loop in this thread, then use that instead.

    This behavior enables corepkg internals to use the real asyncio.run
