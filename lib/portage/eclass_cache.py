...
import errno
from corepkg.exception import FileNotFound, PermissionDenied
...
class hashed_path:
    ...
    def __repr__(self):
        return f"<corepkg.eclass_cache.hashed_path('{self.location}')>"
...
class cache:
    ...
    def __init__(self, porttree_root, overlays=None):
        if overlays is not None:
            warnings.warn(
                "overlays parameter of corepkg.eclass_cache.cache constructor is deprecated and no longer used",
                DeprecationWarning,
                stacklevel=2,
            )
