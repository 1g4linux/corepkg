...
from corepkg.util._dyn_libs.soname_deps_qa import (
    _get_all_provides,
    _get_unresolved_soname_deps,
)
from corepkg.package.ebuild.prepare_build_dirs import (
    _prepare_workdir,
    _prepare_fake_distdir,
    _prepare_fake_filesdir,
)
from corepkg.eapi import _get_eapi_attrs
from corepkg.util import writemsg, ensure_dirs
from corepkg.util._async.AsyncTaskFuture import AsyncTaskFuture
from corepkg.util._async.BuildLogger import BuildLogger
...
from corepkg.const import SUPPORTED_GENTOO_BINPKG_FORMATS

try:
    from corepkg.xml.metadata import MetaDataXML
...
async def _setup_locale(settings):
    from corepkg.util.locale import async_check_locale, split_LC_ALL
...
async def _setup_repo_revisions(settings):
    from corepkg.dbapi.porttree import portdbapi
    ...
    if (
        ...
        for repo_ref in referenced_repos.values():
            if repo_ref.sync_type:
                sync = corepkg.sync.module_controller.get_class(repo_ref.sync_type)()
                try:
...
class EbuildPhase(CompositeTask):
    ...
    def _start_lock(self):
        if (
            ...
            lock_path = os.path.join(eroot, corepkg.VDB_PATH + "-ebuild")
    ...
    def _fail_clean(self):
        self.returncode = None
        corepkg.elog.elog_process(self.settings.mycpv, self.settings)
        phase = "clean"
    ...
    async def _elog(self, elog_funcname, lines, background=None):
        from corepkg.elog import messages as elog_messages
        from corepkg.util.futures.unix_events import _set_nonblocking
        ...
        global_havecolor = corepkg.output.havecolor
        try:
            corepkg.output.havecolor = self.settings.get(
                "NOCOLOR", "false"
            ).lower() in ("no", "false")
            for line in lines:
                elog_func(line, phase=phase, key=self.settings.mycpv, out=out)
        finally:
            corepkg.output.havecolor = global_havecolor
        msg = out.getvalue()
        if msg:
            build_logger = None
            try:
...
class _PostPhaseCommands(CompositeTask):
    ...
    def _commands_exit(self, task):
        from corepkg.package.ebuild.doebuild import _post_src_install_soname_symlinks
