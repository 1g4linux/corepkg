...
class BlockerCache(corepkg.cache.mappings.MutableMapping):
    ...
    def __init__(self, myroot, vardb):
        ...
        self._cache_filename = os.path.join(
            vardb.settings["EROOT"], corepkg.CACHE_PATH, "vdb_blockers.pickle"
        )
        ...
        if cache_valid:
            ...
            for k, v in self._cache_data["blockers"].items():
                if not isinstance(k, str):
                    ...
                    continue
                try:
                    if corepkg.catpkgsplit(k) is None:
                        invalid_items.add(k)
                        continue
                except corepkg.exception.InvalidData:
                    invalid_items.add(k)
                    continue
                ...
                for atom in atoms:
                    if not isinstance(atom, str):
                        invalid_atom = True
                        break
                    if atom[:1] != "!" or not corepkg.isvalidatom(
    ...
    def flush(self):
        ...
        if len(self._modified) >= self._cache_threshold and secpass >= 2:
            try:
                with corepkg.util.atomic_ofstream(self._cache_filename, mode="wb") as f:
                    pickle.dump(self._cache_data, f, protocol=2)

                corepkg.util.apply_secpass_permissions(
                    self._cache_filename, gid=corepkg.corepkg_gid, mode=0o644
                )
            except OSError:
                pass
            self._modified.clear()
