...
class PackageUninstall(CompositeTask):
    ...
    subprocess, since the corepkg.locks module does not behave
    ...
    def _start(self):
        ...
        cat, pf = corepkg.catsplit(self.pkg.cpv)
    ...
    def _start_unmerge(self, lock_task):
        ...
        corepkg.prepare_build_dirs(settings=self.settings, cleanup=True)
    ...
    def _writemsg_level(self, msg, level=0, noiselevel=0):
        ...
        if log_path is None:
            if not (background and level < logging.WARNING):
                corepkg.util.writemsg_level(msg, level=level, noiselevel=noiselevel)
        else:
