...
from corepkg.exception import FileNotFound
from corepkg.util._async.AsyncTaskFuture import AsyncTaskFuture
from corepkg.util._async.FileCopier import FileCopier
...
class BinpkgFetcher(CompositeTask):
    ...
    def __init__(self, **kwargs):
        ...
        if not binpkg_path:
            raise FileNotFound(
                "PATH not found in the binpkg index, the binhost's corepkg is probably out of date."
            )
    ...
    async def _main(self) -> int:
        ...
        if not self.pretend:
            corepkg.util.ensure_dirs(os.path.dirname(self.pkg_path))
        ...
        try:
            if bintree._remote_has_index:
                ...
                if not rel_uri:
                    ...
                    # never happen in new corepkg versions.
...
class _BinpkgFetcherProcess(SpawnProcess):
    ...
    def _start(self):
        ...
        fetch_args = [
            corepkg.util.varexpand(x, mydict=fcmd_vars) for x in shlex.split(fcmd)
        ]
        ...
        fd_pipes.setdefault(0, corepkg._get_stdin().fileno())
    ...
    class AlreadyLocked(corepkg.exception.CorepkgException):
        pass
