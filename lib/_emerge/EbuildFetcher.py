...
from corepkg.checksum import _hash_filter
...
from corepkg.util._pty import _create_pty_or_pipe
...
class _EbuildFetcherProcess(ForkProcess):
    ...
    def _check_already_fetched(self, settings, uri_map):
        ...
        out = io.StringIO()
        eout = corepkg.output.EOutput()
        eout.quiet = settings.get("PORTAGE_QUIET") == "1"
        success = True
        try:
            sys.stdout = out
            sys.stderr = out
            if corepkg.output.havecolor:
                corepkg.output.havecolor = not self.background
            ...
            for filename in uri_map:
                ...
                if not ok:
                    success = False
                    break
        except corepkg.exception.FileNotFound:
    ...
    def _start(self):
        ...
        corepkg.doebuild_environment(ebuild_path, "fetch", settings=settings, db=portdb)
    ...
    def _target(settings, manifest, uri_map, fetchonly, pre_exec):
        ...
        corepkg.output.havecolor = settings.get("NOCOLOR") not in ("yes", "true")
    ...
    def _prefetch_size_ok(self, uri_map, settings, ebuild_path):
        ...
        sizes = {}
        for filename in uri_map:
            # Use stat rather than lstat since corepkg.fetch() creates
            # symlinks when PORTAGE_RO_DISTDIRS is used.
            try:
                st = os.stat(os.path.join(distdir, filename))
            except OSError:
