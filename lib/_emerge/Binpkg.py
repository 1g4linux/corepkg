...
from corepkg.eapi import eapi_exports_replace_vars
...
class Binpkg(CompositeTask):
    ...
    def _start(self):
        ...
        if eapi_exports_replace_vars(settings["EAPI"]):
            ...
            settings["REPLACING_VERSIONS"] = " ".join(
                {
                    corepkg.versions.cpv_getversion(x)
        ...
        # corepkg.locks functions are only designed
    ...
    def _start_fetcher(self, lock_task=None):
        if lock_task is not None:
            ...
            corepkg.prepare_build_dirs(self.settings["ROOT"], self.settings, 1)
    ...
    async def _unpack_metadata(self, loop=None):
        ...
        for mydir in (dir_path, self._image_dir, infloc):
            corepkg.util.ensure_dirs(
                mydir,
                uid=corepkg.data.corepkg_uid,
                gid=corepkg.data.corepkg_gid,
                mode=dir_mode,
            )
        ...
        if pkg_path is not None:
            ...
            if not md5sum:
                md5sum = corepkg.checksum.perform_md5(pkg_path)
            with open(
                _unicode_encode(
