...
class JobStatusDisplay:
    ...
    def sigwinch(self):
        if not self._isatty:
            return
        width = corepkg.output.get_term_size()[1]
    ...
    def _init_term(self):
        ...
        for k, capname in self._termcap_name_map.items():
            ...
            code = tigetstr(corepkg._native_string(capname))
            if code is None:
    ...
    def _display_status(self):
        ...
        style_file = corepkg.output.ConsoleStyleFile(color_output)
        style_file.write_listener = plain_output
        style_writer = corepkg.output.StyleWriter(file=style_file, maxcol=9999)
