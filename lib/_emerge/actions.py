...
from corepkg import _encodings, _unicode_decode
from corepkg.binrepo.config import BinRepoConfigLoader
...
from corepkg.dbapi._expand_new_virt import expand_new_virt
from corepkg.dbapi.IndexedPortdb import IndexedPortdb
from corepkg.dbapi.IndexedVardb import IndexedVardb
from corepkg.dep import Atom, _repo_separator, _slot_separator
...
from corepkg.package.ebuild.fetch import _hide_url_passwd
from corepkg._sets import load_default_config, SETPREFIX
...
from corepkg.util.SlotObject import SlotObject
from corepkg.util._async.run_main_scheduler import run_main_scheduler
...
from corepkg._global_updates import _global_updates
from corepkg.sync.old_tree_timestamp import old_tree_timestamp_warn
from corepkg.localization import _
from corepkg.metadata import action_metadata
from corepkg.emaint.main import print_results
from corepkg.gpg import GPG
...
def action_build(
    ...
    # It's best for config updates in /etc/corepkg to be processed
    # before we get here, so warn if they're not (bug #267103).
    chk_updated_cfg_files(settings["EROOT"], ["/etc/corepkg"])
    ...
    if quickpkg_direct:
        ...
        if quickpkg_root == emerge_config.running_config.settings["ROOT"]:
            ...
            quickpkg_settings = corepkg.config(
            ...
            quickpkg_vardb = corepkg.vartree(settings=quickpkg_settings).dbapi
    ...
    if resume:
        ...
        try:
        ...
        except (corepkg.exception.PackageNotFound, depgraph.UnsatisfiedResumeDep) as e:
            if isinstance(e, depgraph.UnsatisfiedResumeDep):
                mydepgraph = e.depgraph

            from corepkg.output import EOutput
            ...
            if isinstance(e, depgraph.UnsatisfiedResumeDep):
                ...
                for line in textwrap.wrap(msg, 72):
                    out.eerror(line)
            elif isinstance(e, corepkg.exception.PackageNotFound):
        ...
        if need_write_bindb or need_write_vardb:
            ...
            for eroot in eroots:
                ...
                if (
                    ...
                    corepkg.writemsg_stdout(">>> Unlocking GnuPG... ")
                    sys.stdout.flush()
        ...
        if mergecount == 0:
            ...
            if retval == os.EX_OK and not (buildpkgonly or fetchonly or pretend):
                if "yes" == settings.get("AUTOCLEAN"):
                    corepkg.writemsg_stdout(">>> Auto-cleaning packages...\n")
                    unmerge(
...
def action_config(settings, trees, myopts, myfiles):
    ...
    except corepkg.exception.AmbiguousPackageName as e:
    ...
    mysettings = corepkg.config(clone=settings)
    vardb = trees[mysettings["EROOT"]]["vartree"].dbapi
    debug = mysettings.get("PORTAGE_DEBUG") == "1"
    retval = corepkg.doebuild(
        ebuildpath,
        "config",
        settings=mysettings,
    ...
    corepkg.elog.elog_process(mysettings.mycpv, mysettings)
    if retval == os.EX_OK:
        corepkg.doebuild(
            ebuildpath,
            "clean",
...
def action_depclean(
    ...
    if (
        ...
        msg.append("app-corepkg/gentoolkit) in order to detect such breakage.\n")
        msg.append("\n")
    ...
    msg.append("package.provided (see corepkg(5)) will be removed by\n")
    ...
    if action == "depclean" and "--quiet" not in myopts and not myfiles:
        ...
        for x in msg:
            corepkg.writemsg_stdout(colorize("WARN", " * ") + x)
    ...
    for k in ("profile", "system", "selected"):
        ...
        except corepkg.exception.PackageSetNotFound:
...
def _calc_depclean(
    ...
    def show_parents(child_node):
        ...
        msg.append("\n")
        corepkg.writemsg_stdout("".join(msg), noiselevel=-1)
    ...
    if clean_set:
        ...
        for node in clean_set:
            ...
            for dep_type in Package._dep_keys:
                ...
                for atom in atoms:
                    if not isinstance(atom, corepkg.dep.Atom):
        ...
        for atom in atoms:
            if not atom.startswith(SETPREFIX):
                if atom.cp.startswith("null/"):
                    ...
                    null_cat, pn = corepkg.catsplit(atom.cp)
                    for world_atom in world_atoms:
                        cat, world_pn = corepkg.catsplit(world_atom.cp)
                        if pn == world_pn:
                            expanded_atoms.add(
                                Atom(
...
class _info_pkgs_ver:
    ...
    def __lt__(self, other):
        return corepkg.versions.vercmp(self.ver, other.ver) < 0
...
def action_info(settings, trees, myopts, myfiles):
    ...
    from corepkg.util._get_vm_info import get_vm_info
    ...
    for repo in repos:
        ...
        if repo.sync_type:
            sync = corepkg.sync.module_controller.get_class(repo.sync_type)()
            options = {"repo": repo}
            try:
    ...
    basename = os.path.basename(
        os.path.realpath(os.path.join(os.sep, corepkg.const.EPREFIX, "bin", "sh"))
    )
    try:
        ...
        except corepkg.exception.AmbiguousPackageName:
    ...
    myvars += corepkg.util.grabfile(settings["PORTDIR"] + "/profiles/info_pkgs")
    atoms = []
    for x in myvars:
        try:
            x = Atom(x)
    ...
    for orig_atom, x in myvars:
        ...
        for cpv in pkg_matches:
            matched_cp = corepkg.versions.cpv_getkey(cpv)
            ver = corepkg.versions.cpv_getversion(cpv)
    ...
    if "--verbose" in myopts:
        ...
        myvars.extend(
            corepkg.util.grabfile(settings["PORTDIR"] + "/profiles/info_vars")
        )
    ...
    if mypkgs:
        ...
        pkgsettings = corepkg.config(clone=settings)
        ...
        for mypkg in mypkgs:
            ...
            if pkg_type == "installed":
                ...
                if binpkg_format == "xpak":
                    ebuild_file_contents = corepkg.xpak.tbz2(binpkg_file).getfile(
                        ebuild_file_name
                    )
                elif binpkg_format == "gpkg":
                    ebuild_file_contents = corepkg.gpkg.gpkg(
...
def action_sync(
    ...
    from corepkg.emaint.modules.sync.sync import SyncRepos
    ...
    for x in files:
        if is_valid_package_atom(x, allow_repo=True) or (
            ...
            for cpv in vardb.cpv_all():
                if corepkg.match_from_list(ext_atom, [cpv]):
                    require_metadata = False
                    atom = corepkg.cpv_getkey(cpv)
                    if ext_atom.operator == "=*":
                        atom = "=" + atom + "-" + corepkg.versions.cpv_getversion(cpv)
                    if ext_atom.slot:
                    ...
                    if require_metadata:
                        try:
                            ...
                            continue
                        if not corepkg.match_from_list(atom, [cpv]):
                            continue
    ...
    if lookup_owners:
        ...
        if owners:
            for cpv in owners:
                ...
                valid_atoms.append(corepkg.dep.Atom(atom))
        else:
            writemsg_level(
...
def adjust_configs(myopts, trees):
    for myroot, mytrees in trees.items():
        ...
        if "--usepkgonly" in myopts and mytrees["bintree"]._propagate_config(
            ...
            mytrees["porttree"].dbapi.doebuild_settings = corepkg.config(
                clone=mysettings
            )
...
def adjust_config(myopts, settings):
    ...
    try:
        ...
        if PORTAGE_DEBUG not in (0, 1):
            ...
            corepkg.writemsg("!!! PORTAGE_DEBUG must be either 0 or 1\n", noiselevel=-1)
            PORTAGE_DEBUG = 0
    except ValueError as e:
        corepkg.writemsg(f"!!! {str(e)}\n", noiselevel=-1)
        corepkg.writemsg(
    ...
    if not corepkg.util.no_color(settings):
        corepkg.output.havecolor = 1
...
def get_libc_version(vardb: corepkg.dbapi.vartree.vardbapi) -> list[str]:
    libcver = []
    libclist = find_libc_deps(vardb, True)

    if libclist:
        for cpv in sorted(libclist):
            libc_split = corepkg.catpkgsplit(cpv)[1:]
            if libc_split[-1] == "r0":
...
def get_profile_version(
    portdir: str, profile: str, vardb: corepkg.dbapi.vartree.vardbapi
) -> str:
    profilever = None
    ...
    if profile:
        ...
        if profilever is None:
            try:
                for parent in corepkg.grabfile(os.path.join(profile, "parent")):
                    ...
                    if colon != -1:
                        ...
                        try:
                            ...
                            if profilever is not None:
                                break
            except corepkg.exception.CorepkgException:
                pass
...
def getcorepkgversion(
    portdir: str,
    profile: str,
    chost: str,
    vardb: corepkg.dbapi.vartree.vardbapi,
) -> str:
    pythonver = (
        "python"
    ...
    return f"Corepkg {corepkg.VERSION} ({pythonver}, {profilever}, {gccver}, {','.join(libcver)}, {unameout})"
...
def load_emerge_config(emerge_config=None, env=None, **kargs):
    ...
    for k, envvar in (
        ...
        if v is not None:
            kwargs[k] = v
    emerge_config.trees = corepkg.create_trees(trees=emerge_config.trees, **kwargs)
    ...
    emerge_config.target_config.mtimedb = corepkg.MtimeDB(
        os.path.join(target_eroot, corepkg.CACHE_PATH, "mtimedb")
    )
    ...
    if target_eroot != emerge_config.trees._running_eroot:
        emerge_config.running_config.mtimedb = corepkg.MtimeDB(
            os.path.join(
                emerge_config.trees._running_eroot, corepkg.CACHE_PATH, "mtimedb"
            )
        )
...
def getgccversion(chost=None):
    ...
    corepkg.writemsg(gcc_not_found_error, noiselevel=-1)
    return "[unavailable]"
...
def validate_ebuild_environment(trees):
    from corepkg.util.locale import check_locale
    ...
    for name, pid in pids:
        ...
        try:
            with open(os.devnull, "wb", 0) as dev_null:
                rval = corepkg.process.spawn(
                    cmd, env=os.environ, fd_pipes={1: dev_null.fileno()}
                )
        except corepkg.exception.CommandNotFound:
            if "PORTAGE_NICENESS" in settings:
                out = corepkg.output.EOutput()
                out.eerror(
...
def ionice(settings, pids):
    ...
    for name, pid in pids:
        ...
        try:
            rval = corepkg.process.spawn(cmd, env=os.environ)
        except corepkg.exception.CommandNotFound:
...
def missing_sets_warning(root_config, missing_sets):
    ...
    global_config_path = corepkg.const.GLOBAL_CONFIG_PATH
    if corepkg.const.EPREFIX:
        global_config_path = os.path.join(
            corepkg.const.EPREFIX, corepkg.const.GLOBAL_CONFIG_PATH.lstrip(os.sep)
        )
    msg.append(
        "        This usually means that "
        f"'{os.path.join(global_config_path, 'sets/corepkg.conf')}'"
    )
...
def repo_name_check(trees):
    ...
    # /usr/local/corepkg (see bug #248603).
    try:
        missing_repo_names.remove("/usr/local/corepkg")
    except KeyError:
        pass
    ...
    if ignored_repos:
        ...
        msg.extend(
            ...
            for x in textwrap.wrap(
                ...
                + "/etc/corepkg/make.conf if you would like to disable this warning."
            )
        )
        msg.append("\n")
        writemsg_level(
...
def run_action(emerge_config):
    ...
    from corepkg.news import count_unread_news, display_news_notifications
    ...
    for fmt in emerge_config.target_config.settings.get(
        ...
        if not fmt in corepkg.const.SUPPORTED_BINPKG_FORMATS:
    ...
    if "--quiet" not in emerge_config.opts:
        corepkg.deprecated_profile_check(settings=emerge_config.target_config.settings)
    ...
    if corepkg.data.secpass < 2:
        ...
        if "--pretend" not in emerge_config.opts and emerge_config.action not in (
            ...
            need_superuser = emerge_config.action in (
                ...
                fetchonly
                or (buildpkgonly and corepkg.data.secpass >= 1)
                or emerge_config.action in ("metadata", "regen", "sync")
            )
            if corepkg.data.secpass < 1 or need_superuser:
                if need_superuser:
                    access_desc = "superuser"
                else:
                    access_desc = "corepkg group"
                # Always show corepkg_group_warning() when only corepkg group
                # access is required but the user is not in the corepkg group.
                if "--ask" in emerge_config.opts:
                    writemsg_stdout(
                        f"This action requires {access_desc} access...\n",
                        noiselevel=-1,
                    )
                    if corepkg.data.secpass < 1 and not need_superuser:
                        corepkg.data.corepkg_group_warning()
    ...
    if not disable_emergelog:
        if emerge_log_dir:
            try:
                ...
                corepkg.util.ensure_dirs(emerge_log_dir)
            except corepkg.exception.CorepkgException as e:
                writemsg_level(
                ...
                corepkg.util.ensure_dirs(_emerge.emergelog._emerge_log_dir)
            else:
