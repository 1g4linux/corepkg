...
from corepkg.localization import localized_size
from corepkg.output import bold, darkgreen, green, red
from corepkg.util import writemsg_stdout
from corepkg.util.iterators.MultiIterGroupBy import MultiIterGroupBy
...
class search:
    ...
    def _xmatch(self, level, atom):
        ...
        if level == "match-all":
            ...
            result = list(x for x in matches if corepkg.cpv_getkey(x) == cp)
            ...
            for db in self._dbs:
                if hasattr(db, "xmatch"):
                    ...
                    if not cpv or corepkg.cpv_getkey(cpv) != cp:
                        continue
                    if not result or cpv == corepkg.best([cpv, result]):
                        result = cpv
                else:
                    db_keys = list(db._aux_cache_keys)
    ...
    def _iter_search(self):
        ...
        # after this is available in a stable version of corepkg).
    ...
    def getInstallationStatus(self, package):
        if not isinstance(package, corepkg.dep.Atom):
            package = corepkg.dep.Atom(package)
        ...
        if installed_package:
            try:
                ...
                installed_package = corepkg.best(installed_package)
    ...
    def getVersion(self, full_package, detail):
        if len(full_package) > 1:
            package_parts = corepkg.catpkgsplit(full_package)
