...
from corepkg.emaint.modules.logs.logs import CleanLogs
from corepkg.news import count_unread_news, display_news_notifications
from corepkg.output import colorize
from corepkg.util._dyn_libs.display_preserved_libs import display_preserved_libs
from corepkg.util._info_files import chk_updated_info_files
...
def post_emerge(myaction, myopts, myfiles, target_root, trees, mtimedb, retval):
    ...
    vdb_path = os.path.join(root_config.settings["EROOT"], corepkg.VDB_PATH)
    ...
    postemerge = os.path.join(
        settings["PORTAGE_CONFIGROOT"], corepkg.USER_CONFIG_PATH, "bin", "post_emerge"
    )
    if os.access(postemerge, os.X_OK):
        hook_retval = corepkg.process.spawn([postemerge], env=settings.environ())
