...
from corepkg import _unicode_decode, _unicode_encode, _encodings
...
from corepkg.dbapi import dbapi
from corepkg.dbapi.dep_expand import dep_expand
from corepkg.dbapi.DummyTree import DummyTree
from corepkg.dbapi.IndexedPortdb import IndexedPortdb
from corepkg.dbapi._similar_name_search import similar_name_search
...
from corepkg.dep.libc import find_libc_deps, strip_libc_deps
from corepkg.dep._slot_operator import ignore_built_slot_operator_deps, strip_slots
from corepkg.eapi import eapi_has_strong_blocks, eapi_has_required_use, _get_eapi_attrs
...
from corepkg.output import colorize, create_color_func, darkgreen, green

bad = create_color_func("BAD")
from corepkg.package.ebuild.getmaskingstatus import _getmaskingstatus, _MaskReason
from corepkg._sets import SETPREFIX
from corepkg._sets.base import InternalPackageSet, WildcardPackageSet
from corepkg.dep._slot_operator import evaluate_slot_operator_equal_deps
from corepkg.util import ConfigProtect, new_protect_filename
from corepkg.util import cmp_sort_key, writemsg, writemsg_stdout
from corepkg.util import ensure_dirs, normalize_path
from corepkg.util import writemsg_level, write_atomic
...
from corepkg.util.corepkg_lru_cache import show_lru_cache_info
from corepkg.versions import _pkg_str, catpkgsplit
from corepkg.binpkg import get_binpkg_format
...
class _frozen_depgraph_config:
    def __init__(self, settings, trees, myopts, params, spinner):
        ...
        for myroot in trees:
            ...
            self.pkgsettings[myroot] = corepkg.config(
...
class _dynamic_depgraph_config:
    ...
    used in calculating Corepkg dependencies. Each depgraph created by the depgraph.py code gets its own
    ...
    **self.digraph**
    ...
    ``corepkg.util.digraph``, a directed graph data structure. ``corepkg.util.digraph`` is used for a variety of
    purposes in the Corepkg codebase, but in this particular scenario as ``self.digraph``, it is used to create a
    dependency tree of Corepkg packages. So for ``self.digraph``, each *node* of the directed graph is a ``Package``,
    ...
    **digraph creation**
    ...
    def __init__(self, depgraph, myparams, allow_backtracking, backtrack_parameters):
        ...
        # to the graph.
        self._graph_trees = corepkg._trees_dict()
...
class depgraph:
    ...
    def _load_vdb(self):
        ...
        for myroot in self._frozen_config.trees:
            ...
            if not fake_vartree.dbapi:
                ...
                self._frozen_config.pkgsettings[myroot] = corepkg.config(
        ...
        for pkg in fake_vartree.dbapi:
            ...
            if metadata is not None:
                ...
                if config_pool:
                    ...
                    settings = corepkg.config(clone=portdb.settings)
        ...
        for replacement_parent in self._iter_similar_available(
            ...
            for atom in atoms:
                ...
                if not atom.package:
                    ...
                    if replacement_parent.built and corepkg.dep._match_slot(
                        atom, dep.child
                    ):
                        # We are searching for a replacement_parent
                        # atom that will pull in a different child,
    ...
    def _flatten_atoms(self, pkg, use):
        ...
        for dep_key in pkg._dep_keys:
            ...
            dep_string = corepkg.dep.use_reduce(
                dep_string,
                uselist=use,
    ...
    def _changed_deps(self, pkg):
        ...
        if ebuild is None:
            ...
            try:
                built_deps = []
                for k in depvars:
                    dep_struct = corepkg.dep.use_reduce(
                        pkg._raw_metadata[k],
    ...
    def _add_pkg(self, pkg, dep):
        ...
        if True:
            ...
            except corepkg.exception.InvalidDependString as e:
                if not pkg.installed:
        ...
        if not pkg.onlydeps:
            ...
            if not pkg.installed:
                ...
                try:
                    ...
                    settings.lock()
                except corepkg.exception.InvalidDependString:
                    if not pkg.installed:
                        # should have been masked before it was selected
                        raise
        ...
        for dep_root, dep_string, dep_priority in deps:
            ...
            try:
                if (
                    ...
                    test_deps = corepkg.dep.use_reduce(
                        dep_string,
    ...
    def _wrapped_add_pkg_dep_string(
        ...
        if debug:
            writemsg_level(f"\nParent:    {pkg}\n", noiselevel=-1, level=logging.DEBUG)
            dep_repr = corepkg.dep.paren_enclose(
    ...
    def _dep_expand(self, root_config, atom_without_category):
        ...
        """
        null_cp = corepkg.dep_getkey(
            insert_category_into_atom(atom_without_category, "null")
        )
        cat, atom_pn = corepkg.catsplit(null_cp)
        ...
        for x in myfiles:
            if x.endswith(SUPPORTED_XPAK_EXTENSIONS) or x.endswith(
                ...
                if binpkg_format == "xpak":
                    mytbz2 = corepkg.xpak.tbz2(x)
                    mykey = None
                    cat = mytbz2.getfile("CATEGORY")
                elif binpkg_format == "gpkg":
                    mygpkg = corepkg.gpkg.gpkg(root_config.settings, None, x)
                    mykey = None
                ...
                ebuild_path = corepkg.util.normalize_path(os.path.abspath(x))
                ...
                if not corepkg.isvalidatom(cp):
                    writemsg(error_msg, noiselevel=-1)
                    return 0, myfavorites
                cat = corepkg.catsplit(cp)[0]
                mykey = cat + "/" + os.path.basename(ebuild_path[:-7])
                if not corepkg.isvalidatom("=" + mykey):
                    writemsg(error_msg, noiselevel=-1)
                    return 0, myfavorites
                ...
                if ebuild_path:
                    ...
                    if mykey not in portdb.xmatch(
                        "match-visible", corepkg.cpv_getkey(mykey)
                    ):
                        writemsg(
                            colorize(
                                "BAD",
                        ...
                        writemsg(
                            colorize(
                                "BAD",
                                "*** /etc/corepkg/package.* to accomplish this. See corepkg(5) man\n",
                            ),
                            noiselevel=-1,
                        )
                        writemsg(
                ...
                if x.startswith(SETPREFIX):
                    s = x[len(SETPREFIX) :]
                    if s not in sets:
                        raise corepkg.exception.PackageSetNotFound(s)
                    if s in depgraph_sets.sets:
                        continue

                    try:
                        set_atoms = root_config.setconfig.getSetAtoms(s)
                    except corepkg.exception.PackageSetNotFound as e:
                ...
                if not is_valid_package_atom(x, allow_repo=True):
                    ...
                    corepkg.writemsg("!!! Please check ebuild(5) for full details.\n")
                    corepkg.writemsg(
                ...
                if expanded_atoms:
                    ...
                    cat, atom_pn = corepkg.catsplit(null_atom.cp)
        ...
        for arg in self._expand_set_args(args, add_to_digraph=True):
            ...
            for atom in sorted(arg.pset.getAtoms()):
                ...
                try:
                    ...
                    if pprovided and corepkg.match_from_list(atom, pprovided):
    ...
    def _greedy_slots(self, root_config, atom, blocker_lookahead=False):
        ...
        while slots:
            slot = slots.pop()
            slot_atom = corepkg.dep.Atom(f"{highest_pkg.cp}:{slot}")
    ...
    def _select_atoms_highest_available(
        ...
        if not isinstance(depstring, list):
            ...
            depstring = corepkg.dep.use_reduce(
                depstring,
                uselist=myuse,
                opconvert=True,
                token_class=Atom,
        ...
        if True:
            ...
            if not mycheck[0]:
                raise corepkg.exception.InvalidDependString(mycheck[1])
        if parent is None:
                ...
                for atom_node in atom_graph.child_nodes(node):
                    ...
                    for child_node in atom_graph.child_nodes(atom_node):
                        if child_node in traversed_nodes:
                            continue
                        if not corepkg.match_from_list(child_atom, [child_node]):
    ...
    def _show_unsatisfied_dep(
        ...
        for db, pkg_type, built, installed, db_keys in dbs:
            ...
            for cpv in cpv_list:
                for repo in repo_list:
                    ...
                    if metadata is not None and corepkg.eapi_is_valid_for_runtime(
                        metadata["EAPI"]
                    ):
                        if not repo:
        ...
        if show_req_use is not None:
            ...
            if have_eapi_mask:
                writemsg("\n", noiselevel=-1)
                msg = (
                    "The current version of corepkg supports "
                    f"EAPI '{corepkg.const.EAPI}'. You must upgrade to a newer version"
                    " of corepkg before EAPI masked packages can"
                    " be installed."
                )
        ...
        if cp_list:
            ...
            for cpv in cp_list:
                ...
                if match_from_list(atom_exp, [cpv]):
                    ...
                    except corepkg.exception.PackageNotFound:
                        pass
                    else:
    ...
    def _wrapped_select_pkg_highest_available_imp(
                    ...
                    if (
                        ...
                        for rejected in packages_with_invalid_use_config:
                            ...
                            if rejected > pkg:
                                ...
                                break
                            if corepkg.dep.cpvequal(rejected.cpv, pkg.cpv):
        ...
        if (
            ...
            and cp is not None
            and "virtual" == corepkg.catsplit(cp)[0]
        ):
            for pkg in matched_packages:
                if pkg.cp != cp:
                    continue
        ...
        if len(matched_packages) > 1:
            ...
            if visible_matches:
                bestmatch = corepkg.best(visible_matches)
            else:
                # all are masked, so ignore visibility
                bestmatch = corepkg.best([pkg.cpv for pkg in matched_packages])
            matched_packages = [
                pkg
                for pkg in matched_packages
                if corepkg.dep.cpvequal(pkg.cpv, bestmatch)
            ]
    ...
    def _pkg(
        ...
        if pkg is None:
            ...
            try:
                ...
                raise corepkg.exception.PackageNotFound(cpv)
            ...
            for myroot in self._frozen_config.trees:
                ...
                for pkg in vardb:
                    ...
                    if blocker_data:
                        ...
                        try:
                            success, atoms = corepkg.dep_check(
                                depstr,
                                final_db,
                                pkgsettings,
                                myuse=self._pkg_use_enabled(pkg),
                    ...
                    if blocker_atoms:
                        ...
                        except corepkg.exception.InvalidAtom as e:
                            depstr = " ".join(vardb.aux_get(pkg.cpv, dep_keys))
    ...
    def _implicit_libc_deps(self, mergelist, graph):
        ...
        for root in implicit_libc_roots:
            ...
            for atom in self._expand_virt_from_graph(
                root, corepkg.const.LIBC_PACKAGE_ATOM
            ):
                if atom.blocker:
                    continue
                for pkg in self._dynamic_config._package_tracker.match(root, atom):
    ...
    def _serialize_tasks(self):
        ...
        # sys-apps/corepkg needs special treatment if ROOT="/"
        running_root = self._frozen_config._running_root.root
        runtime_deps = InternalPackageSet(initial_atoms=[PORTAGE_PACKAGE_ATOM])
        running_corepkg = self._frozen_config.trees[running_root][
            "vartree"
        ].dbapi.match_pkgs(Atom(PORTAGE_PACKAGE_ATOM))
        replacement_corepkg = list(
            self._dynamic_config._package_tracker.match(
        ...
        if running_corepkg:
            running_corepkg = running_corepkg[0]
        else:
            running_corepkg = None

        if replacement_corepkg:
            replacement_corepkg = replacement_corepkg[0]
        else:
            replacement_corepkg = None

        if replacement_corepkg == running_corepkg:
            replacement_corepkg = None

        if running_corepkg is not None:
            try:
                corepkg_rdepend = self._select_atoms_highest_available(
                    running_root,
                    running_corepkg._metadata["RDEPEND"],
                    myuse=self._pkg_use_enabled(running_corepkg),
                    parent=running_corepkg,
                    strict=False,
                )
            except corepkg.exception.InvalidDependString as e:
                corepkg.writemsg(
                    "!!! Invalid RDEPEND in "
                    f"'{running_root}var/db/pkg/{running_corepkg.cpv}/RDEPEND': {e}\n",
                    noiselevel=-1,
                )
                del e
                corepkg_rdepend = {running_corepkg: []}
            for atoms in corepkg_rdepend.values():
        ...
        for root in implicit_libc_roots:
            ...
            if libc_pkgs:
                ...
                for atom in self._expand_virt_from_graph(
                    root, corepkg.const.OS_HEADERS_PACKAGE_ATOM
                ):
                    if atom.blocker:
                        continue
        ...
        def gather_deps(ignore_priority, mergeable_nodes, selected_nodes, node):
            ...
            if node not in mergeable_nodes:
                return False
            if node == replacement_corepkg and any(
                ...
                # Make sure that corepkg always has all of its
                # RDEPENDs installed first, but ignore uninstalls
                # (these occur when new corepkg blocks an older package version).
                return False
            ...
            if not selected_nodes and myblocker_uninstalls:
                ...
                for task in myblocker_uninstalls.leaf_nodes():
                    ...
                    if heuristic_overlap and running_root == task.root:
                        # Never uninstall sys-apps/corepkg or it's essential
    ...
    def _show_circular_deps(self, mygraph):
        ...
        prefix = colorize("BAD", " * ")
        corepkg.writemsg("\n", noiselevel=-1)
        corepkg.writemsg(prefix + "Error: circular dependencies:\n", noiselevel=-1)
        corepkg.writemsg("\n", noiselevel=-1)
        ...
        if handler.circular_dep_message is not None:
            corepkg.writemsg(handler.circular_dep_message, noiselevel=-1)
    ...
    def _display_autounmask(self, autounmask_continue=False):
            ...
            if pkg in self._dynamic_config.digraph:
                ...
                for reason in mreasons:
                    if reason.unmask_hint and reason.unmask_hint.key == "p_mask":
                        ...
                        comment, filename = corepkg.getmaskingreason(
                            pkg.cpv,
                            metadata=pkg._metadata,
        ...
        def find_config_file(abs_user_config, file_name):
            """
            Searches /etc/corepkg for an appropriate file to append changes to.
            ...
            corepkg.util.grablines(recursive=True).
        ...
        for root in roots:
            ...
            def _writemsg(reason, file):
                writemsg(
                    ...
                    f' (see "{file}" in the corepkg(5) man page for more details)\n',
                    noiselevel=-1,
                )
    ...
    def saveNomergeFavorites(self):
        ...
        for x in self._dynamic_config._set_nodes:
            ...
            try:
                ...
                writemsg(
                    f"!!! see '{os.path.join(x.root, corepkg.VDB_PATH, x.cpv, 'PROVIDE')}'\n\n",
                    noiselevel=-1,
                )
                del e
        all_added = []
    ...
    def _loadResumeCommand(self, resume_data, skip_masked=True, skip_missing=True):
        ...
        for x in mergelist:
            ...
            for atom in depgraph_sets.atoms.getAtoms():
                if atom.repo and corepkg.dep.match_from_list(atom, [pkg_key]):
                    repo = atom.repo
                    break
            ...
            if pkg is None:
                ...
                if skip_missing:
                    ...
                    continue
                raise corepkg.exception.PackageNotFound(pkg_key)
    ...
    def _load_favorites(self, favorites):
        ...
        for x in favorites:
            ...
            if x.startswith(SETPREFIX):
                ...
                except corepkg.exception.InvalidAtom:
                    continue
    ...
    class UnsatisfiedResumeDep(corepkg.exception.CorepkgException):
        """
        ...
        def __init__(self, depgraph, value):
            corepkg.exception.CorepkgException.__init__(self, value)
            self.depgraph = depgraph

    class _internal_exception(corepkg.exception.CorepkgException):
...
def ambiguous_package_name(arg, atoms, root_config, spinner, myopts):
    if "--quiet" in myopts:
        ...
        for cp in sorted({corepkg.dep_getkey(atom) for atom in atoms}):
    ...
    null_cp = corepkg.dep_getkey(insert_category_into_atom(arg, "null"))
    cat, atom_pn = corepkg.catsplit(null_cp)
    s.searchkey = atom_pn
    for cp in sorted({corepkg.dep_getkey(atom) for atom in atoms}):
        s.addCP(cp)
...
def _spinner_start(spinner, myopts):
    ...
    if show_spinner:
        corepkg.writemsg_stdout("Calculating dependencies  ")
...
def _spinner_stop(spinner, backtracked: int = -1, max_retries: int = -1):
    ...
    if spinner.update != spinner.update_basic:
        ...
        corepkg.writemsg_stdout("\b\b")

    corepkg.writemsg_stdout("... done!\n")
...
def backtrack_depgraph(
    settings: corepkg.package.ebuild.config.config,
    trees: corepkg._trees_dict,
...
def get_mask_info(
    ...
    if metadata is None:
        ...
        eapi = metadata["EAPI"]
        if not corepkg.eapi_is_valid_for_runtime(eapi):
            mreasons = [corepkg.unsupported_eapi_message(eapi)]
        else:
            pkg = Package(
                type_name=pkg_type,
                root_config=root_config,
...
def show_masked_packages(masked_packages):
    ...
    for root_config, pkgsettings, cpv, repo, metadata, mreasons in masked_packages:
        ...
        eapi_masked = metadata is not None and not corepkg.eapi_is_valid_for_runtime(
            metadata["EAPI"]
        )
        if eapi_masked:
            have_eapi_mask = True
...
def show_blocker_docs_link():
    ...
    writemsg(
        "https://wiki.gentoo.org/wiki/Handbook:X86/Working/Corepkg#Blocked_packages\n\n",
        noiselevel=-1,
    )
