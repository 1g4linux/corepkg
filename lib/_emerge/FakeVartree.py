...
from corepkg.dep._slot_operator import find_built_slot_operator_atoms
from corepkg.eapi import _get_eapi_attrs
from corepkg.exception import InvalidData, InvalidDependString
from corepkg.update import grab_updates, parse_updates, update_dbentries
...
class FakeVartree(vartree):
    ...
    def _aux_get_wrapper(self, cpv, wants, myrepo=None):
        ...
        except (KeyError, corepkg.exception.CorepkgException):
            live_metadata = None
    ...
    def _apply_dynamic_deps(self, pkg, live_metadata):
        try:
            ...
            if not (
                corepkg.eapi_is_valid_for_runtime(live_metadata["EAPI"])
                and corepkg.eapi_is_valid_for_runtime(pkg.eapi)
            ):
                raise _DynamicDepsNotApplicable()
...
def grab_global_updates(portdb):
    ...
    for repo_name in portdb.getRepositories():
        ...
        except corepkg.exception.DirectoryNotFound:
            rawupdates = []
        upd_commands = []
