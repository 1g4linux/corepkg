...
from corepkg.output import bold, colorize, darkgreen, green
from corepkg._sets import SETPREFIX
from corepkg._sets.base import EditablePackageSet
from corepkg.versions import cpv_sort_key, _pkg_str
...
def _unmerge_display(
    ...
    clean_delay=1,
    ordered=0,
    writemsg_level=corepkg.util.writemsg_level,
):
    """
    ...
    vdb_path = os.path.join(settings["EROOT"], corepkg.VDB_PATH)
    try:
        # At least the parent needs to exist for the lock file.
        corepkg.util.ensure_dirs(vdb_path)
    ...
    try:
        ...
        for x in candidate_catpkgs:
            ...
            except corepkg.exception.AmbiguousPackageName as errpkgs:
                print(
            ...
            if unmerge_action in ["rage-clean", "unmerge"]:
                ...
                for mypkg in mymatch[1:]:
                    ...
                    ) or mypkg == corepkg.best([mypkg, best_version]):
                        if myslot == best_slot:
                ...
                for mypkg in vartree.dbapi.cp_list(corepkg.cpv_getkey(mymatch[0])):
                    myslot = vartree.getslot(mypkg)
                    if myslot not in slotmap:
                        slotmap[myslot] = {}
                    slotmap[myslot][vartree.dbapi.cpv_counter(mypkg)] = mypkg
    ...
    for cp in range(len(pkgmap)):
        for cpv in pkgmap[cp]["selected"].copy():
            ...
            if unmerge_action != "clean" and root_config.root == "/":
                skip_pkg = False
                if corepkg.match_from_list(corepkg.const.PORTAGE_PACKAGE_ATOM, [pkg]):
                    msg = (
                        f"Not unmerging package {pkg.cpv} "
                        "since there is no valid reason for Corepkg to "
                        f"{unmerge_action} itself."
                    )
                    skip_pkg = True
                elif vartree.dbapi._dblink(cpv).isowner(corepkg._python_interpreter):
                    msg = (
                        f"Not unmerging package {pkg.cpv} since there is no valid "
                        f"reason for Corepkg to {unmerge_action} currently used Python "
                        "interpreter."
                    )
                    skip_pkg = True
                if skip_pkg:
            ...
            for s in installed_sets:
                ...
                if s not in sets:
                    ...
                    out.eerror(
                        f"Unknown set '@{s}' in "
                        f"{root_config.settings['EROOT']}{corepkg.const.WORLD_SETS_FILE}"
                    )
                    continue
    ...
    if not ordered:
        ...
        for d in pkgmap:
            ...
            if not selected:
                continue
            cp = corepkg.cpv_getkey(next(iter(selected)))
