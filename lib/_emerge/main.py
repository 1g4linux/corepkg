...
from corepkg.repository.config import _find_bad_atoms
from corepkg.sync import _SUBMODULE_PATH_MAP
...
def parse_opts(tmpcmdline, silent=False):
    ...
    argument_options = {
        ...
        "--config-root": {
            "help": "specify the location for corepkg configuration files",
            "action": "store",
        },
    ...
    if myoptions.jobs is not None:
        ...
        if jobs == 0:
            from corepkg.util.cpuinfo import get_cpu_count
...
def emerge_main(args: Optional[list[str]] = None):
    ...
    from corepkg.output import xtermTitleReset
    from corepkg.util import writemsg_level

    if args is None:
        args = sys.argv[1:]

    args = corepkg._decode_argv(args)
    ...
    if "--debug" in myopts:
        ...
        corepkg.util.initialize_logger(logging.DEBUG)
    else:
    ...
    with open(os.devnull, "r+b") as dev_null:
        ...
        exit_code = corepkg.process.spawn_bash(
