...
def show_invalid_depstring_notice(parent_node, error_msg):
    ...
    msg = []
    if p_status == "nomerge":
        category, pf = corepkg.catsplit(p_key)
        pkg_location = os.path.join(
            parent_node.root_config.settings["EROOT"], corepkg.VDB_PATH, category, pf
        )
        msg.append("Corepkg is unable to process the dependencies of the ")
