...
import corepkg
from corepkg.util.compression_probe import (
    compression_probe,
    _compressors,
)
...
class BinpkgExtractorAsync(SpawnProcess):
    ...
    _shell_binary = corepkg.const.BASH_BINARY
    ...
    def _xpak_start(self):
        ...
        if decomp is not None:
            ...
            corepkg._unicode_encode(
                self.pkg_path, encoding=corepkg._encodings["fs"], errors="strict"
            )
        ):
            decomp_cmd = "cat"
            decomp = {
                "compress": "cat",
                "package": "app-core/coreutils",
            }
        else:
            decomp_cmd = None
        ...
        pkg_xpak = corepkg.xpak.tbz2(self.pkg_path)
        pkg_xpak.scan()
