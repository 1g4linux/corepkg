...
class PackageTracker:
    ...
    **Behavior**
    ...
    This class is used to model the behavior of a real Gentoo or other system using Corepkg for package management,
    along with the installed and to-be-installed packages. The installed packages are ones that are already on the
    system and recorded in ``/var/db/pkg``, while the to-be-installed packages are a group of packages that Corepkg is
    considering installing on the system, based on the information in Corepkg's dependency graph. Multiple roots are
