...
from corepkg.elog.messages import eerror
from corepkg.output import colorize, create_color_func, red

bad = create_color_func("BAD")
from corepkg._sets import SETPREFIX
from corepkg._sets.base import InternalPackageSet
from corepkg.util import ensure_dirs, writemsg, writemsg_level
...
from corepkg.package.ebuild.doebuild import _check_temp_dir, _prepare_self_update
...
class Scheduler(PollScheduler):
    ...
    class _unknown_internal_error(corepkg.exception.CorepkgException):
        """
    ...
    def __init__(
        ...
        self._running_corepkg = None
        corepkg_match = self._running_root.trees["vartree"].dbapi.match(
            corepkg.const.PORTAGE_PACKAGE_ATOM
        )
        if corepkg_match:
            cpv = corepkg_match.pop()
            self._running_corepkg = self._pkg(
    ...
    def _handle_self_update(self):
        ...
        for x in self._mergelist:
            ...
            if x.root != self._running_root.root:
                continue
            if not corepkg.dep.match_from_list(corepkg.const.PORTAGE_PACKAGE_ATOM, [x]):
                continue
    ...
    class _pkg_failure(corepkg.exception.CorepkgException):
        """
    ...
    def _schedule_setup(self, setup_phase):
        ...
        if (
            ...
            # since the corepkg.locks module does not behave as desired
    ...
    def _check_manifests(self):
        ...
        for myroot, pkgsettings in self.pkgsettings.items():
            quiet_config = corepkg.config(clone=pkgsettings)
            ...
            try:
                ...
                if existing_builddir:
                    if x.built:
                        ...
                        if ebuild_path is None:
                            raise AssertionError(f"ebuild not found for '{x.cpv}'")
                    corepkg.package.ebuild.doebuild.doebuild_environment(
                        ebuild_path,
                        "clean",
                        settings=settings,
                ...
                if x.built:
                    ...
                    except corepkg.exception.SignatureException as e:
                        writemsg(
                ...
                corepkg.elog.elog_process(x.cpv, settings)
    ...
    def merge(self):
        ...
        for root in self.trees:
            ...
            self.pkgsettings[root] = corepkg.config(clone=root_config.settings)
        ...
        printer = corepkg.output.EOutput()
        ...
        if (
            ...
            for mysettings, key, logentries in self._failed_pkgs_die_msgs:
                ...
                print()
                for phase in corepkg.const.EBUILD_PHASES:
    ...
    def _elog_listener(self, mysettings, key, logentries, fulltext):
        errors = corepkg.elog.filter_loglevels(logentries, ["ERROR"])
        if errors:
    ...
    def _merge(self):
        ...
        failed_pkgs = self._failed_pkgs
        corepkg.locks._quiet = self._background
        corepkg.elog.add_listener(self._elog_listener)
        ...
        try:
            ...
            corepkg.locks._quiet = False
            corepkg.elog.remove_listener(self._elog_listener)
    ...
    def _allocate_config(self, root):
        ...
        if self._config_pool[root]:
            ...
            temp_settings = corepkg.config(clone=self.pkgsettings[root])
        ...
        if self._jobs_tmpdir_require_free_gb and hasattr(os, "statvfs"):
            ...
            for root in self.trees:
                ...
                tmpdir = first_existing(
                    os.path.join(settings["PORTAGE_TMPDIR"], "corepkg")
                )
                try:
                    ...
                    if (
                        ...
                        if actual_free_bytes < required_free_bytes:
                            if not self._warned_tmpdir_free_space:
                                from corepkg.util.human_readable import bytes_to_human
    ...
    def _task(self, pkg):
        ...
        if pkg.operation != "uninstall":
            ...
            previous_cpv = [
                x for x in vardb.match(pkg.slot_atom) if corepkg.cpv_getkey(x) == pkg.cp
            ]
    ...
    def _save_resume_list(self):
        ...
        mtimedb["resume"] = {}
        # Stored as a dict starting with corepkg-2.1.6_rc1, and supported
        # by >=corepkg-2.1.3_rc8. Versions <corepkg-2.1.3_rc8 only support
    ...
    def _calc_resume_list(self):
        ...
        for task, atoms in dropped_tasks.items():
            ...
            # not valid here.
            settings.pop("T", None)
            corepkg.elog.elog_process(pkg.cpv, settings)
