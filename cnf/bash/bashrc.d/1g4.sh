...
# Load emoji-aware logging helpers when available.
if [[ -r /etc/emoji-logging.sh ]]; then
  # shellcheck source=/etc/emoji-logging.sh
  source /etc/emoji-logging.sh
fi

# Keep this script usable even when emoji logger is unavailable.
